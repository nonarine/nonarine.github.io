...
}`;let o=Ht(r);if(!o)return console.error("Failed to invert Jacobian for coordinate system:",this.name),console.error("Forward Jacobian was:",r),`
// Velocity transform: ${this.name} \u2192 Cartesian (IDENTITY FALLBACK - ERROR)
...
}`;let s=o.map((c,d)=>{let u=dr(c.condition,e),h=d===0?"if":"else if",g=u==="true"?"":` (${u})`;return`    ${h}${g} {
...
`}function Ni(n,e,t,i,r=null,o=null){let s=Array.from({length:n},(b,_)=>`uniform sampler2D u_pos_${_};`).join(`
`),l=`vec${n}`,c=Array.from({length:n},(b,_)=>`decodeFloat(texture2D(u_pos_${_}, v_texcoord))`).join(", "),d=["x","y","z","w"],u=e.map((b,_)=>`    result.${d[_]} = ${b};`).join(`
...
${l} get_velocity_native(${l} pos_native) {
    ...
    float ${o.nativeVars?o.nativeVars.map((b,_)=>`${b} = pos_native.${d[_]}`).join(", "):d.slice(0,n).map((b,_)=>`${b} = pos_native.${b}`).join(", ")};
${u}
    return result;
}
...
void main() {
    ...
    ${l} pos;
    ${Array.from({length:n},(b,_)=>{let w=["x","y","z","w"][_];return _===0?`pos.${w} = denormalizeFromViewport(decodeFloat(texture2D(u_pos_${_}, texcoord)), u_min.x, u_max.x);`:_===1?`pos.${w} = denormalizeFromViewport(decodeFloat(texture2D(u_pos_${_}, texcoord)), u_min.y, u_max.y);`:`pos.${w} = denormalizeFromViewport(decodeFloat(texture2D(u_pos_${_}, texcoord)), -10.0, 10.0);`}).join(`
    `)}
    ...
    if (should_respawn) {
        ...
        new_pos.y = u_min.y + height * margin + rand2(texcoord * 5.678 + vec2(u_rand_seed * 2.345)) * height * (1.0 - 2.0 * margin);`:Array.from({length:n},(b,_)=>{let w=["x","y","z","w"];return _===0?"new_pos.x = u_min.x + width * margin + rand(texcoord * 1.234 + vec2(u_rand_seed)) * width * (1.0 - 2.0 * margin);":_===1?"new_pos.y = u_min.y + height * margin + rand2(texcoord * 5.678 + vec2(u_rand_seed * 2.345)) * height * (1.0 - 2.0 * margin);":`new_pos.${w[_]} = -10.4 + rand(texcoord * ${_+1}.37 + vec2(u_rand_seed * ${_+3}.5)) * 20.8;`}).join(`
        `)}
    }
    ...
    ${Array.from({length:n},(b,_)=>{let w=["x","y","z","w"][_],F;return _===0?F=`normalizeToViewport(new_pos.${w}, u_min.x, u_max.x)`:_===1?F=`normalizeToViewport(new_pos.${w}, u_min.y, u_max.y)`:F=`normalizeToViewport(new_pos.${w}, -10.0, 10.0)`,_===0?`if (u_out_coordinate == ${_}) {
        gl_FragColor = encodeFloat(${F});
        gl_FragColor.a = new_age; // Store age in alpha channel
    }`:_===n-1?` else {
        gl_FragColor = encodeFloat(${F});
    }`:` else if (u_out_coordinate == ${_}) {
        gl_FragColor = encodeFloat(${F});
    }`}).join("")}
}
`}function Oi(n,e,t,i,r=!1,o=null){let s=o&&o.forwardTransform,l=Array.from({length:n},(x,y)=>`uniform sampler2D u_pos_${y};`).join(`
...
`,i+=`}
`,i}function dt(n,e){let t=e===2?"mat2":e===3?"mat3":"mat4",i=`vec${e}`,r="";for(let l=0;l<e;l++)r+=`    float ${Kr[l]} = pos.${["x","y","z","w"][l]};
`;let o=[];for(let l=0;l<e;l++)for(let c=0;c<e;c++){let d=n[c][l];try{let u=he(d,e);o.push(u)}catch(u){a.warn(`Failed to compile Jacobian element [${c}][${l}]: ${d}`,u),o.push("0.0")}}return`
${je(e)}
...
${r}
    return ${t}(
...
}`}function ge(n,e,t){let i=e(n);return`        ${n} = ${i};`}function Oe(n,e,t){let i=`vec${t}`,r=`${n}_pred`,o=`${n}_mid`,s=e(n),l=e(o);return`        // Predictor: standard fixed-point step
        ${i} ${r} = ${s};
        ...
        ${n} -= delta_${n};`}function He(n,e,t,i="h",r=null){let o=`vec${t}`,s=t===2?"mat2":t===3?"mat3":"mat4",l=`inverse${t}`,c=["x","y","z","w"],d="1e-4",u="",h=[];for(let p=0;p<t;p++){let x=`Df_col${p}_${n}`,y=Array.from({length:t},(w,F)=>F===p?d:"0.0").join(", "),b=e(`${n} + ${o}(${y})`),_=e(n);u+=`
        // Velocity Jacobian column ${p}: d(velocity)/d${c[p]}
        ${o} ${x} = (${b} - ${_}) / ${d};`,h.push(x)}u+=`
        ...
        ${n} -= delta_${n};`}function Yr(n){return{name:"Euler",costFactor:1,code:`
// Euler integration
...
`}}function Zr(n){return{name:"Heun (Explicit Trapezoidal)",costFactor:2,code:`
...
`}}function Yi(n){return{name:"RK4",costFactor:4,code:`
...
`}}function Zi(n,e=3,t="fixed-point",i=null){a.info(`*** Implicit Euler integrator requested: solutionMethod=${t}, hasExpressions=${!!i}`);let r=`vec${n}`,o=n===2?"mat2":n===3?"mat3":"mat4",s="pos + h * get_velocity(pos)",l=g=>`pos + h * get_velocity(${g})`,c=g=>`get_velocity(${g})`,d,u,h="";if(t==="newton"&&i){a.info("Computing Jacobian for Newton's method");let g=ye(i,n);Ne(g)?(a.info("\u2713 Successfully using Newton's method for Implicit Euler"),h=dt(g,n),d=Ge("x_new",x=>`${x} - pos - h * get_velocity(${x})`,x=>`${o}(1.0) - h * computeJacobian(${x})`,n),u="Newton"):(a.warn("\u2717 Failed to compute Jacobian for Newton's method"),a.warn("Falling back to fixed-point iteration"),d=ge("x_new",l,n),u="Fixed-Point")}else t==="newton-fd"?(a.info("\u2713 Using finite difference Newton's method for Implicit Euler"),h=je(n),d=He("x_new",c,n),u="Newton (FD)"):t==="midpoint"?(d=Oe("x_new",l,n),u="Midpoint"):(d=ge("x_new",l,n),u="Fixed-Point");return{name:`Implicit Euler (${u})`,costFactor:1,code:`
${h}
...
`}}function Qr(n,e=4,t="fixed-point",i=null){a.info(`*** Implicit Midpoint integrator requested: solutionMethod=${t}, hasExpressions=${!!i}`);let r=`vec${n}`,o=n===2?"mat2":n===3?"mat3":"mat4",l="pos + h * get_velocity(pos + h * 0.5 * get_velocity(pos))",c=f=>`pos + h * get_velocity((pos + ${f}) * 0.5)`,d=f=>`get_velocity((pos + ${f}) * 0.5)`,u,h,g="";if(t==="newton"&&i){a.info("Computing Jacobian for Newton's method (Implicit Midpoint)");let f=ye(i,n);Ne(f)?(a.info("\u2713 Successfully using Newton's method for Implicit Midpoint"),g=dt(f,n),u=Ge("x_new",y=>`${y} - pos - h * get_velocity((pos + ${y}) * 0.5)`,y=>`${o}(1.0) - (h * 0.5) * computeJacobian((pos + ${y}) * 0.5)`,n),h="Newton"):(a.warn("\u2717 Failed to compute Jacobian for Newton's method (Implicit Midpoint)"),a.warn("Falling back to fixed-point iteration"),u=ge("x_new",c,n),h="Fixed-Point")}else t==="newton-fd"?(a.info("\u2713 Using finite difference Newton's method for Implicit Midpoint"),g=je(n),u=He("x_new",d,n,"h * 0.5"),h="Newton (FD)"):t==="midpoint"?(u=Oe("x_new",c,n),h="Midpoint"):(u=ge("x_new",c,n),h="Fixed-Point");return{name:`Implicit Midpoint (${h})`,costFactor:2,code:`
${g}
...
`}}function to(n,e=5,t="fixed-point",i=null){a.info(`*** Implicit RK4 integrator requested: solutionMethod=${t}, hasExpressions=${!!i}`);let r=`vec${n}`,o=n===2?"mat2":n===3?"mat3":"mat4",s=`
    ...
    ${r} k1 = k1_guess;
    ${r} k2 = k2_guess;`,c=b=>`get_velocity(pos + h * (a11 * ${b} + a12 * k2))`,d=b=>`get_velocity(pos + h * (a21 * k1 + a22 * ${b}))`,u=b=>`get_velocity(pos + h * (a11 * ${b} + a12 * k2))`,h=b=>`get_velocity(pos + h * (a21 * k1 + a22 * ${b}))`,g,f,p,x="";if(t==="newton"&&i){a.info("Computing Jacobian for Newton's method (Implicit RK4 - simplified)");let b=ye(i,n);if(Ne(b)){a.info("\u2713 Successfully using simplified Newton's method for Implicit RK4"),x=dt(b,n);let _=T=>`${T} - get_velocity(pos + h * (a11 * ${T} + a12 * k2))`,w=T=>`${o}(1.0) - (h * a11) * computeJacobian(pos + h * (a11 * ${T} + a12 * k2))`,F=T=>`${T} - get_velocity(pos + h * (a21 * k1 + a22 * ${T}))`,P=T=>`${o}(1.0) - (h * a22) * computeJacobian(pos + h * (a21 * k1 + a22 * ${T}))`;g=Ge("k1",_,w,n),f=Ge("k2",F,P,n),p="Newton"}else a.warn("\u2717 Failed to compute Jacobian for Newton's method (Implicit RK4)"),a.warn("Falling back to fixed-point iteration"),g=ge("k1",c,n),f=ge("k2",d,n),p="Fixed-Point"}else t==="newton-fd"?(a.info("\u2713 Using finite difference Newton's method for Implicit RK4 (simplified)"),x=je(n),g=He("k1",u,n,"h * a11"),f=He("k2",h,n,"h * a22"),p="Newton (FD)"):t==="midpoint"?(g=Oe("k1",c,n),f=Oe("k2",d,n),p="Midpoint"):(g=ge("k1",c,n),f=ge("k2",d,n),p="Fixed-Point");let y=`
...
`}}function Qi(n,e,t={}){let i=t.iterations||3,r=t.solutionMethod||"fixed-point",o=t.expressions||null;switch(n){case"euler":return Yr(e);case"explicit-midpoint":return Ki(e);case"heun":return Zr(e);case"rk4":return Yi(e);case"implicit-euler":return Zi(e,i,r,o);case"implicit-midpoint":return Qr(e,t.iterations||4,r,o);case"trapezoidal":return eo(e,t.iterations||4,r,o);case"implicit-rk4":return to(e,t.iterations||5,r,o);case"rk2":return Ki(e);case"implicit":return Zi(e,i,r,o);default:return Yi(e)}}function ut(n,e,t,i=null){i===null&&(i=t>=3?2:-1);let r=i>=0&&i<t;return{name:"Select",params:{dim1:n,dim2:e,depthDim:i},code:`
...
`}}function io(n,e){let t=n[0].map((o,s)=>`${o.toFixed(6)} * pos[${s}]`).join(" + "),i=n[1].map((o,s)=>`${o.toFixed(6)} * pos[${s}]`).join(" + "),r=n[2]?n[2].map((o,s)=>`${o.toFixed(6)} * pos[${s}]`).join(" + "):null;return{name:"Linear Projection",params:{matrix:n},code:`
...
`}}function ro(n,e){let t=n===0?1:0,i=t+1;return i===n&&i++,{name:"Orthographic",params:{axis:n},code:`
...
`}}function oo(){return{name:"Spherical",params:{},code:`
...
`}}function no(n){return{name:"Stereographic",params:{},code:`
...
`}}function er(n,e,t={}){switch(n){case"select":{let i=t.dim1!==void 0?t.dim1:0,r=t.dim2!==void 0?t.dim2:Math.min(1,e-1),o=t.depthDim!==void 0?t.depthDim:null;return ut(i,r,e,o)}case"project":{let i=t.matrix||[[1,0,...Array(Math.max(0,e-2)).fill(0)],[0,1,...Array(Math.max(0,e-2)).fill(0)]];return io(i,e)}case"orthographic":{let i=t.axis!==void 0?t.axis:e-1;return ro(i,e)}case"spherical":return e===3?oo():ut(0,1,e);case"stereographic":return e>=3?no(e):ut(0,1,e);case"custom":{let i=t.horizontalExpr||"x",r=t.verticalExpr||"y",o=t.depthExpr||"";return so(i,r,o,e)}default:return ut(0,1,e)}}function so(n,e,t,i){let r=he(n||"x",i),o=he(e||"y",i),s=t?he(t,i):"0.0";return{name:"Custom",params:{horizontalExpr:n,verticalExpr:e,depthExpr:t},code:`
// Custom mapper
...
${t} transform_jacobian(${t} x) {
    return ${t}(1.0);
}`}},Xt=class extends le{constructor(){super("power","Power transform (zoom lens effect)")}generateForward(e){let t=`vec${e}`;return`
${t} transform_forward(${t} x) {
...
`)}
    return result;
}`}getParameters(){return[{name:"alpha",label:"Exponent (\u03B1)",type:"slider",min:1e-4,max:25,step:1e-4,default:.5,info:"\u03B1 < 1.0: zoom into origin; \u03B1 > 1.0: compress origin"}]}},Wt=class extends le{constructor(){super("tanh","Hyperbolic tangent (compress infinity)")}generateHelpers(e){return`
...
}`}getParameters(){return[{name:"beta",label:"Compression (\u03B2)",type:"slider",min:1e-4,max:25,step:1e-4,default:1,info:"Higher = more compression. Maps infinite space to [-1,1]"}]}},Jt=class extends le{constructor(){super("sigmoid","Logistic Sigmoid (smooth S-curve)")}generateHelpers(e){return`
...
${t} transform_jacobian(${t} x) {
    ...
    `)}
    return result;
}`}getParameters(){return[{name:"k",label:"Steepness (k)",type:"slider",min:1e-4,max:25,step:1e-4,default:1,info:"Higher = steeper transition. Maps infinite space to [-1,1] with logistic curve."}]}},qt=class extends le{constructor(){super("rational","Rational (bell-shaped Jacobian)")}generateForward(e){let t=`vec${e}`;return`
${t} transform_forward(${t} x) {
...
${t} transform_jacobian(${t} x) {
    ...
    `)}
    return result;
}`}getParameters(){return[{name:"a",label:"Width (a)",type:"slider",min:1e-4,max:25,step:1e-4,default:1,info:"Controls bell curve width. Higher = wider/shallower, lower = narrower/sharper."}]}},Kt=class extends le{constructor(){super("sine","Sine wave distortion (periodic speed bumps)")}generateForward(e){let t=`vec${e}`;return`
${t} transform_forward(${t} x) {
...
}`}getParameters(){return[{name:"amplitude",label:"Amplitude",type:"slider",min:1e-4,max:25,step:1e-4,default:.5,info:"Strength of distortion"},{name:"frequency",label:"Frequency",type:"slider",min:1e-4,max:25,step:1e-4,default:1,info:"Number of waves per unit length"}]}},Yt=class extends le{constructor(){super("radial_power","Radial power (compress/expand by distance)")}generateForward(e){let t=`vec${e}`;return e===2?`
${t} transform_forward(${t} x) {
    float alpha = u_transform_params.x;
    float r = length(x);
    if (r < 1e-8) return x;
...
}`}getParameters(){return[{name:"alpha",label:"Radial Exponent (\u03B1)",type:"slider",min:.1,max:3,step:.1,default:.5,info:"\u03B1 < 1.0: zoom into origin; \u03B1 > 1.0: compress origin"}]}},Zt=class extends le{constructor(){super("log","Logarithmic (stretch near zero)")}generateForward(e){let t=`vec${e}`;return`
${t} transform_forward(${t} x) {
    ${t} result;
...
${t} transform_jacobian(${t} x) {
    ...
    `)}
    return result;
}`}getParameters(){return[]}},Qt=class extends le{constructor(){super("exp","Exponential (compress near zero)")}generateForward(e){let t=`vec${e}`;return`
${t} transform_forward(${t} x) {
    float alpha = u_transform_params.x;
    ${t} result;
...
${t} transform_jacobian(${t} x) {
    ...
    `)}
    return result;
}`}getParameters(){return[{name:"alpha",label:"Growth Rate (\u03B1)",type:"slider",min:.1,max:2,step:.1,default:.5,info:"Higher \u03B1 = stronger exponential growth. Use small values (0.1-0.5) to avoid overflow."}]}},ei=class extends le{constructor(){super("softsign","Soft-sign (smooth compression to [-1,1])")}generateForward(e){let t=`vec${e}`;return`
${t} transform_forward(${t} x) {
    ${t} result;
...
${t} transform_jacobian(${t} x) {
    ...
    `)}
    return result;
}`}getParameters(){return[]}},ti=class extends le{constructor(){super("custom","Custom (user-defined GLSL)"),this.forwardCode="",this.inverseCode="",this.jacobianCode=""}setCode(e,t,i){this.forwardCode=e,this.inverseCode=t,this.jacobianCode=i}generateForward(e){return this.forwardCode||new Pe().generateForward(e)}generateInverse(e){return this.inverseCode||new Pe().generateInverse(e)}generateJacobian(e){return this.jacobianCode||new Pe().generateJacobian(e)}},tr={identity:new Pe,power:new Xt,log:new Zt,exp:new Qt,softsign:new ei,tanh:new Wt,sigmoid:new Jt,rational:new qt,sine:new Kt,radial_power:new Yt,custom:new ti};function mt(n){return tr[n]||tr.identity}function ir(n,e){let t=`vec${e}`,r={white:{name:"Solid White",code:`
...
`}}[n];if(!r)throw new Error(`Unknown color mode: ${n}`);return r}function rr(n,e,t){let i=`vec${n}`,r=[],o=["x","y","z","w","u","v"],s=["dx","dy","dz","dw","du","dv"];for(let l=0;l<n;l++)r.push(`float ${o[l]} = pos.${o[l]};`),r.push(`float ${s[l]} = velocity.${o[l]};`);return`
${t}
...
`}function ii(n){let e=[...n].sort((i,r)=>i.position-r.position);if(e.length<2)throw new Error("Gradient must have at least 2 color stops");e.forEach(i=>{if(i.position<0||i.position>1)throw new Error(`Stop position must be in [0, 1], got ${i.position}`)});let t=`vec3 evaluateGradient(float t) {
    t = clamp(t, 0.0, 1.0);
`;for(let i=0;i<e.length-1;i++){let r=e[i],o=e[i+1],s=nr(r.color),l=nr(o.color),c=r.position.toFixed(6),d=o.position.toFixed(6);i===0?t+=`    if (t <= ${d}) {
`:i===e.length-2?t+=`    } else {
`:t+=`    } else if (t <= ${d}) {
...
`}};J();var ft=class extends De{constructor(e){if(super(e),!e.getExtension("OES_texture_float"))throw new Error("OES_texture_float extension not supported");a.info("Float texture support detected",{extension:"OES_texture_float"})}getName(){return"Float Textures (Direct Storage)"}getTextureFormat(){let e=this.gl;return{internalFormat:e.RGBA,format:e.RGBA,type:e.FLOAT}}getArrayType(){return Float32Array}getComponentsPerValue(){return 4}requiresExtension(){return!0}getExtensionName(){return"OES_texture_float"}encodeValue(e,t,i){let r=new Float32Array(4);return r[0]=e,r[1]=0,r[2]=0,r[3]=1,r}decodeValue(e,t,i){return e[0]}normalizeWorld(e,t,i){return e}denormalizeWorld(e,t,i){return e}getGLSLConstants(){return`
...
2. Enable "Use iterative Newton solver" checkbox`),f.text(p),f.prop("disabled",!1)),$("#coord-preset").val("custom")}catch(x){console.error("Error solving inverse:",x),alert("Error solving inverse: "+x.message),f.text(p),f.prop("disabled",!1)}}),$("#coord-use-iterative").off("change").on("change",function(){let f=$(this).is(":checked");o.useIterativeSolver=f,$(".coord-inverse-transform").prop("disabled",f),f?$(".coord-inverse-transform").css("opacity","0.5"):$(".coord-inverse-transform").css("opacity","1.0"),$("#coord-preset").val("custom")}),$("#coord-apply").off("click").on("click",function(){let f=$(this);f.prop("disabled",!0);let p=f.text();f.text("Applying...");try{if(u()){let y=o.forwardTransforms.map(w=>window.UnicodeAutocomplete&&window.UnicodeAutocomplete.unicodeToAscii?window.UnicodeAutocomplete.unicodeToAscii(w):w),b=o.inverseTransforms?o.inverseTransforms.map(w=>window.UnicodeAutocomplete&&window.UnicodeAutocomplete.unicodeToAscii?window.UnicodeAutocomplete.unicodeToAscii(w):w):null,_=new W(o.name,o.dimensions,o.variables,y,b,o.useIterativeSolver||!1);setTimeout(()=>{i(_),f.prop("disabled",!1),f.text(p)},100)}else alert("Invalid coordinate system. Please check your expressions."),f.prop("disabled",!1),f.text(p)}catch(x){console.error("Error in apply handler:",x),f.prop("disabled",!1),f.text(p)}}),$("#coord-cancel").off("click").on("click",function(){i(null)})}function u(){for(let f=0;f<s;f++)if(!o.variables[f]||!o.variables[f].label||!o.forwardTransforms[f]||o.forwardTransforms[f].trim()==="")return!1;return!0}function h(f){o=f,c()}function g(f){f!==s&&(s=f,o=fe(f),c())}return c(),l}function _r(n,e,t){let i=$("#coordinate-editor-panel");i.length||($("body").append(`
        ...
        `),i=$("#coordinate-editor-panel")),i.show();let r=$o("coordinate-editor-container",n,e,c=>{c===null||t(c),i.hide()});$(".floating-panel-close",i).off("click").on("click",function(){i.hide()});let o=!1,s=0,l=0;return $(".panel-header",i).off("mousedown").on("mousedown",function(c){o=!0,s=c.clientX-i.offset().left,l=c.clientY-i.offset().top,$(document).on("mousemove.panelDrag",function(d){o&&i.css({left:d.clientX-s,top:d.clientY-l})})}),$(document).on("mouseup.panelDrag",function(){o=!1,$(document).off("mousemove.panelDrag")}),r}Le();J();function Tr(n,e){let t=new bt({renderer:n,storageKey:"vectorFieldSettings",debounceTime:300,onApply:m=>{m.implicitIterations!==void 0&&(m.integratorParams={iterations:m.implicitIterations}),n.updateConfig(m);let v=t.getSettings();n&&n.bbox&&(v.bbox={min:[...n.bbox.min],max:[...n.bbox.max]}),n&&n.coordinateSystem&&(v.coordinateSystem=n.coordinateSystem.toJSON()),localStorage.setItem("vectorFieldSettings",JSON.stringify(v)),Fo()}}),i=t.register(new ie("dimensions",2,{min:2,max:6,step:1,displayId:"dim-value",displayFormat:m=>m.toFixed(0),onChange:m=>{a.verbose("Dimensions changed to:",m);let v=t.get("dimension-inputs");v&&n.coordinateSystem?(a.verbose("Updating dimension inputs with dimensions:",m),n.coordinateSystem.dimensions===m?(v.setCoordinateSystem(n.coordinateSystem),v.updateInputs(m)):a.warn(`Coordinate system dimension mismatch: ${n.coordinateSystem.dimensions}D coordinate system with ${m}D expressions - update coordinate system or expressions to match`)):a.verbose("Preserving existing coordinate system:",n.coordinateSystem.name);let E=t.get("mapper-params");E&&E.updateControls()}})),r=t.register(new me("integrator","rk2",{settingsKey:"integratorType",onChange:m=>{let v=m.startsWith("implicit-")||m==="trapezoidal";$("#implicit-iterations-group").toggle(v),$("#solution-method-group").toggle(v),be("#integrator",0)}})),o=t.register(new me("solution-method","fixed-point",{settingsKey:"solutionMethod"})),s=t.register(new xt("timestep",.01,{min:.001,max:2.5,step:.001,smallIncrement:.001,largeIncrement:.01,displayId:"timestep-value",displayFormat:m=>m.toFixed(4),animationMin:.001,animationMax:.1})),l=t.register(new ie("implicit-iterations",4,{settingsKey:"implicitIterations",min:1,max:10,step:1,displayId:"implicit-iterations-value",displayFormat:m=>m.toFixed(0)})),c=t.register(new ie("particles",1e3,{settingsKey:"particleCount",min:100,max:5e6,step:1e4,displayId:"particles-value",displayFormat:m=>m.toFixed(0)})),d=t.register(new Ee("fade",.999,{settingsKey:"fadeOpacity",min:0,max:100,step:.1,displayId:"fade-value",displayFormat:m=>m.toFixed(4),transform:m=>{let v=Math.log(.9),R=(Math.log(.9999)-v)/100;return Math.exp(v+R*m)},inverseTransform:m=>{let v=Math.log(.9),R=(Math.log(.9999)-v)/100;return(Math.log(m)-v)/R},animationMin:.95,animationMax:.9995})),u=t.register(new ie("drop",0,{settingsKey:"dropProbability",min:0,max:.1,step:.001,displayId:"drop-value",displayFormat:m=>m.toFixed(4)})),h=t.register(new q("drop-low-velocity",!1,{settingsKey:"dropLowVelocity"})),g=t.register(new me("transform","identity",{settingsKey:"transformType",onChange:m=>{let v=t.get("transform-params");v&&v.updateControls()}}));t.register(new _t({},{settingsKey:"transformParams"})).setTransformControl(g);let p=t.register(new me("mapper","select",{settingsKey:"mapperType",onChange:m=>{let v=t.get("mapper-params");v&&v.updateControls()}}));t.register(new vt({dim1:0,dim2:1},{settingsKey:"mapperParams"})).setRelatedControls(i,p);let y=t.register(new me("color-mode","velocity_angle",{settingsKey:"colorMode",onChange:m=>{wr(m),ci(m),Er(m)}})),b=t.register(new gt("color-expression","x * y",{settingsKey:"colorExpression"})),_=t.register(new q("use-custom-gradient",!1,{settingsKey:"useCustomGradient"})),w=t.register(new $t(ht(),{settingsKey:"colorGradient"})),F=t.register(new me("velocity-scale-mode","percentile95",{settingsKey:"velocityScaleMode"})),P=t.register(new q("velocity-log-scale",!1,{settingsKey:"velocityLogScale"})),T=t.register(new yt(["-y","x"],{settingsKey:"expressions"}));T.setDimensionsControl(i),T.setCoordinateSystem(n.coordinateSystem,!1);let C=t.register(new q("use-hdr",!0,{settingsKey:"useHDR"})),I=t.register(new q("use-depth-test",!1,{settingsKey:"useDepthTest"})),B=t.register(new ie("supersample-factor",1,{min:.5,max:4,step:.5,displayId:"supersample-factor-value",displayFormat:m=>m.toFixed(1)+"x",settingsKey:"supersampleFactor"})),z=t.register(new q("smaa-enabled",!0,{settingsKey:"smaaEnabled"})),ee=t.register(new pe("smaa-intensity",.75,{displayId:"smaa-intensity-value",displayFormat:m=>m.toFixed(2)})),S=t.register(new pe("smaa-threshold",.1,{displayId:"smaa-threshold-value",displayFormat:m=>m.toFixed(2)})),A=t.register(new q("bilateral-enabled",!1,{settingsKey:"bilateralEnabled"}));class M extends ie{getValue(){let v=$(`#${this.id}`);return parseFloat(v.val())/20}setValue(v){$(`#${this.id}`).val(v*20),this.updateDisplay(v)}}let V=t.register(new M("bilateral-spatial",4,{min:10,max:100,step:1,displayId:"bilateral-spatial-value",displayFormat:m=>m.toFixed(1),settingsKey:"bilateralSpatialSigma"}));class D extends ie{getValue(){let v=$(`#${this.id}`);return parseFloat(v.val())/100}setValue(v){$(`#${this.id}`).val(v*100),this.updateDisplay(v)}}let H=t.register(new D("bilateral-intensity",.2,{min:1,max:50,step:.1,displayId:"bilateral-intensity-value",displayFormat:m=>m.toFixed(2),settingsKey:"bilateralIntensitySigma"})),U=t.register(new me("tonemap-operator","aces",{settingsKey:"tonemapOperator",onChange:m=>{wt(m)}})),j=t.register(new ue("exposure",1,{minValue:1e-4,maxValue:10,displayId:"exposure-value",displayFormat:m=>m.toFixed(4)})),te=t.register(new ue("gamma",2.2,{minValue:.2,maxValue:10,displayId:"gamma-value",displayFormat:m=>m.toFixed(2)})),Re=t.register(new ue("luminance-gamma",1,{settingsKey:"luminanceGamma",minValue:.2,maxValue:10,displayId:"luminance-gamma-value",displayFormat:m=>m.toFixed(2)})),Rt=t.register(new ue("highlight-compression",0,{settingsKey:"highlightCompression",minValue:0,maxValue:10,displayId:"highlight-compression-value",displayFormat:m=>m.toFixed(2)})),pi=t.register(new ue("compression-threshold",1,{settingsKey:"compressionThreshold",minValue:1e-4,maxValue:1e5,displayId:"compression-threshold-value",displayFormat:m=>m.toFixed(2)})),gi=t.register(new ue("white-point",2,{settingsKey:"whitePoint",minValue:1,maxValue:1e3,displayId:"white-point-value",displayFormat:m=>m.toFixed(2)})),Mo=t.register(new ue("particle-intensity",1,{settingsKey:"particleIntensity",minValue:.001,maxValue:100,displayId:"particle-intensity-value",displayFormat:m=>m.toFixed(3)})),Po=t.register(new ue("particle-size",1,{settingsKey:"particleSize",minValue:.1,maxValue:10,displayId:"particle-size-value",displayFormat:m=>m.toFixed(1)})),Lo=t.register(new me("particle-render-mode","points",{settingsKey:"particleRenderMode"})),Do=t.register(new pe("color-saturation",1,{settingsKey:"colorSaturation",displayId:"color-saturation-value",displayFormat:m=>m.toFixed(2)})),Bo=t.register(new pe("brightness-desat",0,{settingsKey:"brightnessDesaturation",displayId:"brightness-desat-value",displayFormat:m=>m.toFixed(2)})),Io=t.register(new pe("saturation-buildup",0,{settingsKey:"brightnessSaturation",displayId:"saturation-buildup-value",displayFormat:m=>m.toFixed(2)})),Uo=t.register(new q("bloom-enabled",!1,{settingsKey:"bloomEnabled"})),zo=t.register(new pe("bloom-intensity",.3,{settingsKey:"bloomIntensity",min:0,max:200,displayId:"bloom-intensity-value",displayFormat:m=>m.toFixed(2)})),Vo=t.register(new ie("bloom-radius",1,{settingsKey:"bloomRadius",min:.5,max:3,step:.1,displayId:"bloom-radius-value",displayFormat:m=>m.toFixed(1)})),ko=t.register(new pe("bloom-alpha",1,{settingsKey:"bloomAlpha",displayId:"bloom-alpha-value",displayFormat:m=>m.toFixed(2)})),bi=t.register(new q("frame-limit-enabled",!1,{settingsKey:"frameLimitEnabled",onChange:m=>{if(window.renderer&&(window.renderer.frameLimitEnabled=m,m)){let v=Ct.getValue();window.renderer.frameLimit=v,a.info(`Frame limit enabled: will stop at ${v} frames`)}}})),Ct=t.register(new ue("frame-limit",1e3,{minValue:100,maxValue:1e5,displayId:"frame-limit-value",displayFormat:m=>m>=1e3?`${Math.floor(m/1e3)}k`:Math.floor(m).toString(),settingsKey:"frameLimit",onChange:m=>{window.renderer&&(window.renderer.frameLimit=m,a.verbose(`Frame limit updated to: ${m} frames`))}})),$e=!1,Ce=null,Mt=1,xi=10,Ie=0,Je=!1,yi=95,Pt=.002,vi=yi/(100-yi)*Pt,Rr=3,Cr=10,re=null,Lt=null,Dt=0,qe=0,Te=null,Fe=null,oe=null,Ke=null,No=new Set(["expressions","dimensions","integratorType","solutionMethod","transformType","transformParams","mapperType","mapperParams","colorMode"]),Bt=$("#animation-alpha-container");a.info(`Animation alpha container found: ${Bt.length>0}`),Bt.length?(Bt.html(`
            <div class="control-group">
        ...
        `),a.info("Animation alpha control HTML injected")):a.warn("Animation alpha container not found!");let $i=$("#animation-speed-container");$i.length&&$i.html(`
            <div class="control-group">
        ...
        `);let _i=t.register(new pe("animation-alpha",0,{settingsKey:"animationAlpha",displayId:"animation-alpha-value",displayFormat:m=>`${(m*100).toFixed(2)}%`,onChange:m=>{t.controls.forEach(E=>{E instanceof Ee&&E.animationEnabled&&E.updateFromAlpha(m)});let v=t.controls.get("transform-params");v&&v.parameterControls&&v.parameterControls.forEach(E=>{E.animationEnabled&&E.updateFromAlpha(m)}),window.renderer&&(window.renderer.setAnimationAlpha(m),$("#animation-clear-particles").is(":checked")&&window.renderer.resetParticles(),$("#animation-clear-screen").is(":checked")&&window.renderer.clearRenderBuffer())}})),Oo=t.register(new ie("animation-speed",10,{min:1,max:200,displayId:"animation-speed-value",displayFormat:m=>(xi=Math.round(m),Math.round(m).toString())}));t.register(new q("animation-clear-particles",!1,{settingsKey:"animationClearParticles"})),t.register(new q("animation-clear-screen",!1,{settingsKey:"animationClearScreen"})),t.register(new q("animation-smooth-timing",!1,{settingsKey:"animationSmoothTiming"})),t.register(new q("animation-lock-shaders",!1,{settingsKey:"animationLockShaders"})),a.info("Animation alpha control registered");let Go=t.register(new me("theme-selector","dark",{settingsKey:"theme",onChange:m=>{m==="light"?$("body").addClass("light-theme"):$("body").removeClass("light-theme"),t.saveToStorage()}}));$("#preset-selector").on("change",function(){let m=$(this).val();m&&(ui(m,t),$(this).val(""),We()[m]?($("#preset-name-input").val(m),$("#delete-preset-btn").show()):($("#preset-name-input").val(""),$("#delete-preset-btn").hide()))}),$("#save-preset-btn").on("click",function(){let m=$("#preset-name-input").val().trim();if(!m){alert("Please enter a preset name");return}let E={...t.getSettings(),name:m};Ao(m,E),di(),$("#delete-preset-btn").show(),a.info("Preset saved: "+m)}),$("#delete-preset-btn").on("click",function(){let m=$("#preset-name-input").val().trim();m&&confirm(`Delete preset "${m}"?`)&&(Ro(m),di(),$("#preset-name-input").val(""),$("#delete-preset-btn").hide(),a.info("Preset deleted: "+m))}),di(),$("#default-settings").on("click",function(){t.resetAll(),t.clearStorage(),t.apply()}),$("#share-url").on("click",function(){let m=t.getSettings();To(m)}),$("#reset").on("click",function(){let m=n.gl.canvas,R=10*(m.width/m.height);n.updateConfig({bbox:{min:[-R/2,-5],max:[R/2,5]},reinitializeParticles:!0})}),$("#reset-particles").on("click",function(){n.clearScreen()}),$("#save-image").on("click",async function(){try{let m=await n.captureRenderBuffer(),v=URL.createObjectURL(m),E=document.createElement("a"),R=new Date().toISOString().replace(/[:.]/g,"-").slice(0,-5),K=`${n.renderWidth}x${n.renderHeight}`;E.download=`vector-field-${R}-${K}.png`,E.href=v,document.body.appendChild(E),E.click(),document.body.removeChild(E),URL.revokeObjectURL(v),a.info("Image saved: "+E.download)}catch(m){a.error("Failed to save image:",m),alert("Failed to save image. Check console for details.")}});let Mr=new URLSearchParams(window.location.search).get("storage")||"float";$("#storage-strategy").val(Mr),$("#storage-strategy").on("change",function(){let m=$(this).val(),v=t.getSettings();n&&n.bbox&&(v.bbox={min:[...n.bbox.min],max:[...n.bbox.max]}),n&&n.coordinateSystem&&(v.coordinateSystem=n.coordinateSystem.toJSON()),localStorage.setItem("vectorFieldSettings",JSON.stringify(v));let E=new URL(window.location);E.searchParams.set("storage",m),window.location.href=E.toString()}),$(document).on("click",".slider-btn",function(){let m=$(this).data("slider"),v=$(this).data("action");if(m==="mobile-timestep"){let R=t.get("timestep");R&&R.handleButtonAction&&R.handleButtonAction(v);return}let E=t.get(m);if(E&&E.handleButtonAction){E.handleButtonAction(v);return}if(m.startsWith("transform-param-")){let R=t.get("transform-params");R&&R.handleTransformParamButton&&R.handleTransformParamButton(m,v);return}console.warn(`No button handler found for slider: ${m}`)});let Ye=null;function Pr(){let m=$("#gradient-panel");ci(t.get("color-mode").getValue()),m.show(),Ye?Ye.setGradient(w.getValue()):(Ye=vr("main-gradient-editor",w.getValue(),v=>{w.notifyChange(v)}),w.setGradientEditor(Ye))}function wi(){$("#gradient-panel").hide()}$("#open-gradient-editor").on("click",function(){Pr()}),$("#configure-coordinates").on("click",function(){let m=n.dimensions,v=n.coordinateSystem;(!v||v.dimensions!==m)&&(console.warn("Coordinate system dimension mismatch, resetting to Cartesian"),v=fe(m),n.coordinateSystem=v),_r(m,v,E=>{n.updateConfig({coordinateSystem:E}),T.setCoordinateSystem(E),t.apply()})}),$(document).on("click",function(m){let v=$("#gradient-panel"),E=$("#open-gradient-editor");$(m.target).is(E)||$(m.target).closest("#gradient-panel").length>0||v.is(":visible")&&wi()}),$("#gradient-panel").on("click",function(m){m.stopPropagation()});function Lr(){$("#rendering-panel").show(),wt(t.get("tonemap-operator").getValue())}function Ei(){$("#rendering-panel").hide()}$("#open-rendering-settings").on("click",function(){Lr()}),$("#gradient-panel .floating-panel-close").on("click",function(){wi()}),$("#rendering-panel .floating-panel-close").on("click",function(){window.mobilePanelManager&&window.mobilePanelManager.getCurrentPanel()==="rendering"?window.mobilePanelManager.hidePanel("rendering"):Ei()}),$(document).on("click","#menu-settings",function(){window.appModal&&window.appModal.show()}),$(document).on("click","#menu-docs",function(){window.appModal&&window.appModal.show("docs")}),$(document).on("keydown",function(m){(m.ctrlKey||m.metaKey)&&m.key===","&&(m.preventDefault(),window.appModal&&window.appModal.show())}),$("#animation-section-toggle").on("click",function(){let m=$("#animation-section-content"),v=$("#animation-section-arrow"),E=$("#histogram-panel"),R=$("#cursor-position");m.is(":visible")?(m.slideUp(200),v.text("\u25BC"),E.removeClass("shifted"),R.removeClass("shifted")):(m.slideDown(200),v.text("\u25B2"),E.addClass("shifted"),R.addClass("shifted"))});let xe=[],Ze=null,It=0,Qe=0,Ut=.01;function Ti(m={}){let{captureFrames:v=!1,totalFrames:E=0,loops:R=1,halfLoops:K=!1,onProgress:k=null,onComplete:O=null}=m;if($e)return;v?(Ze="fixed",It=E,Qe=0,xe=[],Ut=R*(K?1:2)/E):(Ze="continuous",Ut=.01),$e=!0,window.renderer&&window.renderer.stop(),Ie=0,re=null,Lt=null,Dt=0,qe=0,oe=null,$("#animation-lock-shaders").is(":checked")&&n?(n.lockShaderRecompilation=!0,Je=!0,a.info("Shader recompilation lock ENABLED (animation started)",null,!1)):Je=!1,Ke=a.verbosity,a.setVerbosity("silent"),Te=[],t.controls.forEach(Y=>{Y instanceof Ee&&Te.push(Y)}),Fe=[];let ce=t.controls.get("transform-params");ce&&ce.parameterControls&&ce.parameterControls.forEach(Y=>{Y instanceof Be&&Fe.push(Y)});let _e=()=>{if(!$e)return;let Y=$("#animation-clear-screen").is(":checked"),Se=$("#animation-smooth-timing").is(":checked");if(Ie++,Ie===1&&(window.renderer&&window.renderer.shadersJustRecompiled&&(window.renderer.shadersJustRecompiled=!1),Lt=performance.now()),Ie>=xi){Ie=0,Y&&window.renderer&&window.renderer.render(!0);let se=performance.now()-Lt;Dt<Rr?Dt++:re===null?re=se:se>re?re=vi*se+(1-vi)*re:re=Pt*se+(1-Pt)*re,Se?(qe++,qe>=Cr&&(qe=0,re===null?$("#step-time-counter").text(`Step: ${se.toFixed(1)}ms (warming up...)`):$("#step-time-counter").text(`Step: ${se.toFixed(1)}ms (EMA: ${re.toFixed(1)}ms)`),$("#step-time-counter").show())):$("#step-time-counter").hide();let X=_i.getValue();X+=Mt*Ut,X>=1?(X=1,Mt=-1):X<=0&&(X=0,Mt=1),_i.setValue(X);for(let G=0;G<Te.length;G++)Te[G].updateFromAlpha(X);for(let G=0;G<Fe.length;G++)Fe[G].updateFromAlpha(X);if(window.renderer){window.renderer.setAnimationAlpha(X);let G={};for(let ae=0;ae<Te.length;ae++){let Me=Te[ae],Ue=Me.getValue();(oe===null||Ue!==oe[Me.settingsKey])&&(G[Me.settingsKey]=Ue)}for(let ae=0;ae<Fe.length;ae++){let Me=Fe[ae],Ue=Me.getValue(),Mi=Me.parameterName;G.transformParams||(G.transformParams=oe?.transformParams?{...oe.transformParams}:{}),(oe===null||Ue!==oe.transformParams?.[Mi])&&(G.transformParams[Mi]=Ue)}if(Object.keys(G).length>0)try{window.renderer.updateConfig(G),oe===null&&(oe={}),Object.assign(oe,G)}catch(ae){a.error("Failed to apply animation settings:",ae)}else oe===null&&(oe={});$("#animation-clear-particles").is(":checked")&&window.renderer.resetParticles(),Y||window.renderer.render(!0),Y&&window.renderer.clearRenderBuffer(!0),Ze==="fixed"&&window.renderer.captureRenderBuffer().then(function(ae){xe.push(ae),Qe++,k&&k(Qe,It,X),Qe>=It&&(et(),O&&O(xe))}).catch(function(ae){console.error("Failed to capture animation frame:",ae),et(),alert("Failed to capture animation frame. Check console for details.")})}if(Se&&re!==null&&se<re){let G=re-se;setTimeout(()=>{$e&&(Ce=requestAnimationFrame(_e))},G);return}}else window.renderer&&(Y?window.renderer.render(!1):window.renderer.render(!0));$e&&(Ce=requestAnimationFrame(_e))};Ce=requestAnimationFrame(_e)}function et(){$e=!1,Ce!==null&&(cancelAnimationFrame(Ce),Ce=null),window.renderer&&!window.renderer.isRunning&&window.renderer.start(),$("#step-time-counter").hide(),Te=null,Fe=null,oe=null,Ke!==null&&(a.setVerbosity(Ke),Ke=null),Je&&n&&(n.lockShaderRecompilation=!1,Je=!1,a.info("Shader recompilation lock DISABLED (animation stopped)")),Ze=null}$("#animation-alpha-animate-btn").on("click",function(m){m.stopPropagation();let v=$(this);$e?(et(),v.text("\u25B6"),v.css("background","#4CAF50"),v.attr("title","Auto-animate")):(Ti(),v.text("\u23F8"),v.css("background","#FFA726"),v.attr("title","Stop auto-animate"))}),$("#animation-half-loops").on("change",function(){let v=$(this).is(":checked")?"Number of half cycles (1 = 0.0 \u2192 1.0, 2 = 0.0 \u2192 1.0 \u2192 0.0)":"Number of complete alpha cycles (0.0 \u2192 1.0 \u2192 0.0)";$("#animation-loops-info").text(v)}),$("#animation-create-btn").on("click",function(){let m=$(this);if(m.data("stop-animation")){et(),m.text("\u25B6 Create Animation"),m.css("background","#4CAF50"),m.data("stop-animation",!1),$("#animation-frames").prop("disabled",!1),$("#animation-loops").prop("disabled",!1),$("#animation-half-loops").prop("disabled",!1),$("#animation-download-btn").prop("disabled",xe.length===0),$("#progress-text").text(`Stopped: ${xe.length} frames captured`),a.info(`Animation stopped early: ${xe.length} frames captured`);return}if($e)return;let v=$("#animation-frames"),E=$("#animation-loops"),R=$("#animation-half-loops"),K=$("#animation-download-btn"),k=$("#animation-progress"),O=$("#progress-bar"),ne=$("#progress-text"),ce=$("#progress-alpha"),_e=parseInt(v.val())||100,Y=parseInt(E.val())||1,Se=R.is(":checked");m.prop("disabled",!1),m.text("\u23F9 Stop Animation"),m.css("background","#f44336"),m.data("stop-animation",!0),v.prop("disabled",!0),E.prop("disabled",!0),R.prop("disabled",!0),K.prop("disabled",!0),k.show(),O.css("width","0%"),ne.text(`Frame 0 / ${_e}`),ce.text("\u03B1: 0.00"),Ti({captureFrames:!0,totalFrames:_e,loops:Y,halfLoops:Se,onProgress:(de,se,X)=>{let G=de/se*100;O.css("width",`${G}%`),ne.text(`Frame ${de} / ${se}`),ce.text(`\u03B1: ${X.toFixed(2)}`)},onComplete:de=>{m.prop("disabled",!1),m.text("\u25B6 Create Animation"),m.css("background","#4CAF50"),m.data("stop-animation",!1),v.prop("disabled",!1),E.prop("disabled",!1),R.prop("disabled",!1),K.prop("disabled",!1),O.css("width","100%"),ne.text(`Complete: ${de.length} frames`),a.info(`Animation creation complete: ${de.length} frames captured`)}})}),$("#animation-download-btn").on("click",async function(){if(xe.length===0){alert("No frames to download. Create an animation first.");return}let m=$(this);m.prop("disabled",!0),m.text("\u23F3 Creating ZIP...");try{let E=xe.length,R=Math.ceil(E/300),K=new Date().toISOString().replace(/[:.]/g,"-").slice(0,-5);a.info(`Splitting ${E} frames into ${R} ZIP file(s)`);for(let k=0;k<R;k++){let O=k*300,ne=Math.min((k+1)*300,E);m.text(`\u23F3 Creating ZIP ${k+1}/${R}...`);let ce=new JSZip,_e=ce.folder("frames");for(let X=O;X<ne;X++){let G=String(X).padStart(5,"0");_e.file(`frame_${G}.png`,xe[X])}let Y=await ce.generateAsync({type:"blob"}),Se=URL.createObjectURL(Y),de=document.createElement("a"),se=R>1?`.${k+1}`:"";de.download=`animation-${K}${se}.zip`,de.href=Se,document.body.appendChild(de),de.click(),document.body.removeChild(de),URL.revokeObjectURL(Se),a.info(`Part ${k+1}/${R} downloaded: frames ${O}-${ne-1}`),k<R-1&&await new Promise(X=>setTimeout(X,500))}a.info(`Animation download complete: ${E} frames in ${R} ZIP file(s)`),R>1&&alert(`Animation split into ${R} ZIP files. Use create-video.sh with the base filename (without .1, .2, etc) to reconstruct.`)}catch(v){a.error("Failed to create ZIP:",v),alert("Failed to create ZIP file: "+v.message)}finally{m.prop("disabled",!1),m.text("\u{1F4BE} Download Animation (ZIP)")}}),$("#export-animation-json").on("click",function(){let m=t.getSettings();n&&n.bbox&&(m.bbox={min:[n.bbox.min[0],n.bbox.min[1]],max:[n.bbox.max[0],n.bbox.max[1]]}),delete m.animationAlpha;let v={name:"Untitled Animation",description:"Animation created from current settings",fps:30,baseSettings:m,timeline:[{time:0,settings:{},easing:"linear"},{time:10,settings:{},easing:"linear"}],frameConfig:{burnInSteps:5e3,clearAfterBurnIn:!0,accumulationSteps:2e3}},E=new Blob([JSON.stringify(v,null,2)],{type:"application/json"}),R=URL.createObjectURL(E),K=document.createElement("a");K.href=R,K.download="animation-template.json",document.body.appendChild(K),K.click(),document.body.removeChild(K),URL.revokeObjectURL(R),a.info("Exported animation template JSON")}),$(document).on("click",function(m){let v=$("#rendering-panel"),E=$("#open-rendering-settings"),R=$("#mobile-menu-rendering");$(m.target).is(E)||$(m.target).is(R)||$(m.target).closest("#mobile-menu-rendering").length>0||$(m.target).closest("#rendering-panel").length>0||v.is(":visible")&&Ei()}),t.initializeControls();let N=_o();if(window.innerWidth<=768&&(!N||N.frameLimitEnabled===void 0&&N.frameLimit===void 0)&&(bi.setValue(!0),Ct.setValue(500),window.renderer&&(window.renderer.frameLimitEnabled=!0,window.renderer.frameLimit=500),a.info("Mobile detected: frame limit enabled at 500 frames")),N){if(N.coordinateSystem&&N.dimensions)try{let m=N.coordinateSystem;m.forwardTransforms&&window.UnicodeAutocomplete&&(m.forwardTransforms=m.forwardTransforms.map(E=>typeof E=="string"&&window.UnicodeAutocomplete.unicodeToAscii?window.UnicodeAutocomplete.unicodeToAscii(E):E));let v=W.fromJSON(m);if(v.dimensions===N.dimensions)n.coordinateSystem=v,T.setCoordinateSystem(v,!1),console.log("Restored coordinate system:",v.name);else{console.warn(`Coordinate system dimension mismatch: system has ${v.dimensions}D but settings use ${N.dimensions}D. Resetting to Cartesian.`);let E=fe(N.dimensions);n.coordinateSystem=E,T.setCoordinateSystem(E,!1)}}catch(m){if(console.warn("Failed to restore coordinate system:",m),N.dimensions){let v=fe(N.dimensions);n.coordinateSystem=v,T.setCoordinateSystem(v,!1)}}if(N.expressions&&Array.isArray(N.expressions)&&window.UnicodeAutocomplete&&(N.expressions=N.expressions.map(m=>typeof m=="string"&&window.UnicodeAutocomplete.unicodeToAscii?window.UnicodeAutocomplete.unicodeToAscii(m):m)),N.mapperParams){let m=N.mapperParams;m.dim1!==void 0&&m.dim2!==void 0&&m.dim1===m.dim2&&(console.warn("Invalid mapper params: dim1 and dim2 are the same, fixing to default"),N.mapperParams={dim1:0,dim2:1})}t.applySettings(N),t.apply()}n&&(n.frameLimitEnabled=bi.getValue(),n.frameLimit=Ct.getValue()),wt(t.get("tonemap-operator").getValue()),wr(t.get("color-mode").getValue()),ci(t.get("color-mode").getValue()),Er(t.get("color-mode").getValue());let Fi=t.get("integrator").getValue()||"rk2",Si=Fi.startsWith("implicit-")||Fi==="trapezoidal";$("#implicit-iterations-group").toggle(Si),$("#solution-method-group").toggle(Si),t.get("theme-selector").getValue()==="light"&&$("body").addClass("light-theme"),So(),window.unicodeAutocomplete&&(window.unicodeAutocomplete.setEnabled(!0),window.unicodeAutocomplete.attachToAll('[id^="expr-"]'),window.unicodeAutocomplete.attachToAll("#mapper-expression"),window.unicodeAutocomplete.attachToAll("#color-expression"),window.unicodeAutocomplete.attachToAll("#custom-color-code"),window.unicodeAutocomplete.attachToAll("#custom-integrator-code"),console.log("Unicode autocomplete enabled for all math inputs"));function Dr(){let m=t.getSettings();return n&&n.bbox&&(m.bbox={min:[...n.bbox.min],max:[...n.bbox.max]}),n&&n.coordinateSystem&&(m.coordinateSystem=n.coordinateSystem.toJSON()),localStorage.setItem("vectorFieldSettings",JSON.stringify(m)),m}function Br(){let m={controls:$("#controls"),rendering:$("#rendering-panel")},v=null;function E(k){v&&R(v);let O=m[k];if(!O){a.warn(`Mobile panel not found: ${k}`);return}if(k==="rendering"){O.show();try{let ne=t.get("tonemap-operator");ne&&wt(ne.getValue())}catch(ne){a.warn("Could not update white point visibility:",ne)}}else O.addClass("mobile-overlay active"),O.find(".mobile-overlay-close").show();$(`#mobile-menu-${k}`).addClass("active"),v=k}function R(k){let O=m[k];O&&(k==="rendering"?O.hide():(O.removeClass("mobile-overlay active"),O.find(".mobile-overlay-close").hide()),$(`#mobile-menu-${k}`).removeClass("active"),v===k&&(v=null))}function K(){Object.keys(m).forEach(R)}return $("#mobile-menu-controls").on("click",function(){v==="controls"?R("controls"):E("controls")}),$("#mobile-menu-rendering").on("click",function(){v==="rendering"?R("rendering"):E("rendering")}),$("#mobile-menu-save-image").on("click",function(){$("#save-image").click()}),$(".mobile-overlay-close").on("click",function(){let O=$(this).closest("[id]").attr("id"),ce={controls:"controls"}[O];ce&&R(ce)}),{showPanel:E,hidePanel:R,hideAllPanels:K,getCurrentPanel:()=>v}}window.mobilePanelManager=Br();let Ai=$("#mobile-timestep"),Ri=$("#mobile-timestep-value");function Ci(){let m=s.getValue(),v=Math.log10(m);Ai.val(v),Ri.text(m.toFixed(4))}if($("#timestep").on("input change",Ci),Ai.on("input",function(){let m=parseFloat($(this).val()),v=Math.pow(10,m);$("#timestep").val(v),s.updateDisplay(v),Ri.text(v.toFixed(4)),s.onChange&&s.onChange(v),t.debouncedApply()}),Ci(),e&&e({manager:t,state:t.getSettings(),saveSettings:Dr}),N&&N.bbox){let m=n?.gl?.canvas,v=Fr(N.bbox,m);n.updateConfig({bbox:v,reinitializeParticles:!1}),a.verbose("Restored bbox expanded for aspect ratio")}return t}function Fr(n,e){if(!e)return n;let t=e.width/e.height,i=n.max[0]-n.min[0],r=n.max[1]-n.min[1],o=i/r,s=(n.min[0]+n.max[0])/2,l=(n.min[1]+n.max[1])/2,c=i,d=r;return t>o?c=r*t:d=i/t,{min:[s-c/2,l-d/2],max:[s+c/2,l+d/2]}}function wt(n){let t=["reinhard_extended","uncharted2","hable","luminance_extended"].includes(n);$("#white-point-control").toggle(t)}function wr(n){n==="expression"?$("#expression-controls").show():$("#expression-controls").hide()}function ci(n){n==="expression"||n==="velocity_magnitude"||n==="velocity_angle"||n==="velocity_combined"?$("#gradient-button-container").show():$("#gradient-button-container").hide(),n==="expression"?$("#gradient-preset-toggle").hide():$("#gradient-preset-toggle").show()}function Er(n){n==="velocity_magnitude"||n==="velocity_combined"?($("#velocity-scaling-container").show(),$("#velocity-log-container").show()):($("#velocity-scaling-container").hide(),$("#velocity-log-container").hide())}function _o(){try{let n=new URLSearchParams(window.location.search),e=n.get("s"),t=null;if(e){if(t=wo(e),t){console.log("Loaded settings from URL parameter"),localStorage.setItem("vectorFieldSettings",JSON.stringify(t)),n.delete("s");let i=n.toString(),r=window.location.pathname+(i?"?"+i:"");window.history.replaceState({},"",r),console.log("URL cleaned, settings now in localStorage")}}else{let i=localStorage.getItem("vectorFieldSettings");i&&(t=JSON.parse(i),console.log("Loaded settings from localStorage"))}return t}catch(n){return console.warn("Failed to load settings:",n),null}}function wo(n){try{let e=n.replace(/-/g,"+").replace(/_/g,"/");for(;e.length%4;)e+="=";let t=atob(e);return JSON.parse(t)}catch(e){return console.error("Failed to decode settings:",e),null}}function Eo(n){try{let e=JSON.stringify(n);return btoa(e).replace(/\+/g,"-").replace(/\//g,"_").replace(/=+$/,"")}catch(e){return console.error("Failed to encode settings:",e),null}}function To(n){let e=Eo(n);if(!e){alert("Failed to encode settings");return}let t=new URL(window.location.href);t.search="";let i=new URLSearchParams(window.location.search).get("storage");i&&t.searchParams.set("storage",i),t.searchParams.set("s",e);let r=t.toString();if(navigator.clipboard&&navigator.clipboard.writeText)navigator.clipboard.writeText(r).then(()=>{let o=$("#share-url"),s=o.text();o.text("URL Copied!"),setTimeout(()=>{o.text(s)},2e3)}).catch(o=>{console.error("Failed to copy URL to clipboard:",o),alert(`Share this URL:
...
`}navigator.clipboard.writeText(i).then(()=>{this.logger.info("Log copied to clipboard ("+t.length+" entries)")}).catch(r=>{this.logger.error("Failed to copy log to clipboard",r)})}),e.querySelector("#debug-clear").addEventListener("click",()=>{this.logger.clear()}),e.querySelector("#debug-flush-buffer").addEventListener("click",()=>{this.logger.flush(),this._updateBufferStatus()}),e.querySelector("#debug-clear-buffer").addEventListener("click",()=>{this.logger.clearSilencedBuffer(),this._updateBufferStatus()}),e.querySelector("#debug-log-update-shader").addEventListener("click",()=>{window.renderer&&typeof window.renderer.logUpdateShader=="function"?window.renderer.logUpdateShader():this.logger.warn("Renderer not initialized - cannot log update shader")}),e.querySelector("#debug-log-draw-shader").addEventListener("click",()=>{window.renderer&&typeof window.renderer.logDrawShader=="function"?window.renderer.logDrawShader():this.logger.warn("Renderer not initialized - cannot log draw shader")}),e.querySelector("#debug-log-screen-shader").addEventListener("click",()=>{window.renderer&&typeof window.renderer.logScreenShader=="function"?window.renderer.logScreenShader():this.logger.warn("Renderer not initialized - cannot log screen shader")}),e.querySelector("#debug-log-stats-shaders").addEventListener("click",()=>{window.renderer&&typeof window.renderer.logStatsShaders=="function"?window.renderer.logStatsShaders():this.logger.warn("Renderer not initialized - cannot log stats shaders")}),e.querySelector("#debug-buffer-stats").addEventListener("click",()=>{window.renderer&&typeof window.renderer.logBufferStats=="function"?window.renderer.logBufferStats():this.logger.warn("Renderer not initialized - cannot log buffer stats")}),e.querySelector("#debug-enable-stats").addEventListener("change",t=>{if(window.renderer){let i=t.target.checked;window.renderer.enableDebugStats=i,this.logger.info(`GPU debug stats ${i?"ENABLED":"DISABLED"}`)}})}_updateBufferStatus(){let e=this.logger.getBufferStats();this.bufferSizeSpan.textContent=e.bufferSize,this.bufferPercentSpan.textContent=e.bufferUsagePercent,e.bufferUsagePercent>80?this.bufferSizeSpan.style.color="#EF5350":e.bufferUsagePercent>50?this.bufferSizeSpan.style.color="#FFA726":this.bufferSizeSpan.style.color="#4CAF50"}onActivate(){this._bufferUpdateInterval||(this._bufferUpdateInterval=setInterval(()=>{this.logger.isSilenced()&&this._isActive&&this._updateBufferStatus()},1e3))}onDeactivate(){this._bufferUpdateInterval&&(clearInterval(this._bufferUpdateInterval),this._bufferUpdateInterval=null)}destroy(){this._bufferUpdateInterval&&(clearInterval(this._bufferUpdateInterval),this._bufferUpdateInterval=null),super.destroy()}};var At=class extends ve{constructor(){super("docs","Documentation"),this.sections=[],this.sectionDefinitions=[{id:"display-options",title:"Display Options & UI Controls",file:"docs/display-options.md"},{id:"vector-fields",title:"Vector Fields",file:"docs/vector-fields.md"},{id:"integrators",title:"Integration Methods",file:"docs/integrators.md"},{id:"color-modes",title:"Color Modes",file:"docs/color-modes.md"},{id:"projection",title:"2D Projection",file:"docs/projection.md"},{id:"coordinate-systems",title:"Coordinate Systems",file:"docs/coordinate-systems.md"},{id:"domain-transforms",title:"Domain Transforms",file:"docs/domain-transforms.md"},{id:"hdr-rendering",title:"HDR & Tone Mapping",file:"docs/hdr-rendering.md"},{id:"rendering-effects",title:"Rendering Effects",file:"docs/rendering-effects.md"},{id:"particle-settings",title:"Particle Settings",file:"docs/particle-settings.md"},{id:"animation",title:"Animation System",file:"docs/animation.md"},{id:"preset-management",title:"Preset Management",file:"docs/preset-management.md"},{id:"storage-strategy",title:"Storage Strategy",file:"docs/storage-strategy.md"},{id:"keyboard-shortcuts",title:"Keyboard Shortcuts",file:"docs/keyboard-shortcuts.md"}]}async _loadMarkdown(e){try{let t=await fetch(e);if(!t.ok)throw new Error(`Failed to load ${e}: ${t.statusText}`);let i=await t.text();return typeof marked>"u"?(console.error("marked.js not loaded"),"<p>Error: Markdown parser not available</p>"):marked.parse(i)}catch(t){return console.error("Error loading markdown:",t),`<p>Error loading documentation: ${t.message}</p>`}}render(e){let t=document.createElement("div");return t.id="docs-tab-content",t.className="modal-tab-content",t.style.display="none",t.innerHTML=`
        ...
        `}_setupCollapsibles(e){e.querySelectorAll(".docs-section-header").forEach(i=>{i.addEventListener("click",()=>{let r=i.parentElement,o=r.querySelector(".docs-section-content"),s=i.querySelector(".docs-toggle");r.classList.contains("collapsed")?(r.classList.remove("collapsed"),o.style.maxHeight="5000px",s.textContent="\u25BC",setTimeout(()=>{r.classList.contains("collapsed")||(o.style.maxHeight=o.scrollHeight+"px")},350)):(r.classList.add("collapsed"),o.style.maxHeight="0",s.textContent="\u25B6")})})}openToSection(e){if(!this._contentElement)return;let t=this._contentElement.querySelector(`#docs-${e}`);if(!t){console.warn(`Documentation section '${e}' not found`);return}t.classList.contains("collapsed")&&t.querySelector(".docs-section-header").click(),setTimeout(()=>{t.scrollIntoView({behavior:"smooth",block:"start"})},400)}onActivate(){this._pendingSection&&(this.openToSection(this._pendingSection),this._pendingSection=null)}setPendingSection(e){this._pendingSection=e}};window.MathParser={setCustomFunctions:Ui,getCustomFunctions:zi};$(document).ready(function(){window.logger=a,a.hookConsole(),a.info("Logger initialized");function n(l){let c=document.getElementById("canvas"),d=null;function u(){if(c.width=window.innerWidth,c.height=window.innerHeight,d){d.resize(c.width,c.height);let h=c.width/c.height,g=d.bbox,f=(g.min[0]+g.max[0])/2,p=(g.min[1]+g.max[1])/2,y=(g.max[1]-g.min[1])*h;g.min[0]=f-y/2,g.max[0]=f+y/2,d.updateConfig({bbox:g})}}u(),window.addEventListener("resize",u);try{let g=new URLSearchParams(window.location.search).get("storage")||"float";d=new pt(c,{storageStrategy:g}),l(d,c)}catch(h){a.error("Failed to initialize renderer",h),alert("Failed to initialize WebGL renderer: "+(h.message||h))}}function e(l,c){let d=Tr(l,function(u){let{manager:h,state:g,saveSettings:f}=u,p=h.getSettings();l.updateConfig(p),l.start(),o(l,h),window.renderer=l,window.loadPreset=ui,window.logger=a,window.saveSettings=f,window.manager=h;let x=Ar(),y=new St(a);x.registerTab(y);let b=h.debouncedApply.bind(h),_=new Ft(window.MathParser,b);x.registerTab(_);let w=new At;x.registerTab(w),window.appModal=x,a.info("N-Dimensional Vector Field Renderer initialized!"),a.info("Available presets: "+Object.keys(window.presets).join(", ")),a.info('Try: loadPreset("3d_lorenz")')})}function t(l,c){let d=!1,u=0,h=0,g=null;function f(){g&&clearTimeout(g),g=setTimeout(()=>{window.saveSettings&&window.saveSettings()},500)}c.addEventListener("mousedown",b=>{b.target===c&&(d=!0,u=b.clientX,h=b.clientY)}),c.addEventListener("mousemove",b=>{if(d){let _=b.clientX-u,w=b.clientY-h,F=l.bbox,P=F.max[0]-F.min[0],T=F.max[1]-F.min[1],C=P/c.width;F.min[0]-=_*C,F.max[0]-=_*C,F.min[1]+=w*C,F.max[1]+=w*C,l.updateConfig({bbox:F}),u=b.clientX,h=b.clientY}}),c.addEventListener("mouseup",()=>{d=!1,f()}),c.addEventListener("mouseleave",()=>{d=!1,d||f()}),c.addEventListener("wheel",b=>{b.preventDefault();let _=b.deltaY>0?1.1:.9,w=c.getBoundingClientRect(),F=b.clientX-w.left,P=b.clientY-w.top,T=l.bbox,C=T.max[0]-T.min[0],I=T.max[1]-T.min[1],B=T.min[0]+C*(F/c.width),z=T.max[1]-I*(P/c.height),ee=I*_,S=c.width/c.height,A=ee*S,M=(B-T.min[0])/C,V=(T.max[1]-z)/I;T.min[0]=B-A*M,T.max[0]=B+A*(1-M),T.min[1]=z-ee*(1-V),T.max[1]=z+ee*V,l.updateConfig({bbox:T}),f()});let p=0;c.addEventListener("touchstart",b=>{if(b.touches.length===1)d=!0,u=b.touches[0].clientX,h=b.touches[0].clientY;else if(b.touches.length===2){d=!1;let _=b.touches[1].clientX-b.touches[0].clientX,w=b.touches[1].clientY-b.touches[0].clientY;p=Math.sqrt(_*_+w*w)}b.preventDefault()}),c.addEventListener("touchmove",b=>{if(b.touches.length===1&&d){let _=b.touches[0].clientX-u,w=b.touches[0].clientY-h,F=l.bbox,P=F.max[0]-F.min[0],T=F.max[1]-F.min[1],C=P/c.width;F.min[0]-=_*C,F.max[0]-=_*C,F.min[1]+=w*C,F.max[1]+=w*C,l.updateConfig({bbox:F}),u=b.touches[0].clientX,h=b.touches[0].clientY}else if(b.touches.length===2){let _=b.touches[1].clientX-b.touches[0].clientX,w=b.touches[1].clientY-b.touches[0].clientY,F=Math.sqrt(_*_+w*w);if(p>0){let P=p/F,T=l.bbox,C=T.max[1]-T.min[1],I=(T.min[0]+T.max[0])/2,B=(T.min[1]+T.max[1])/2,z=C*P,ee=c.width/c.height,S=z*ee;T.min[0]=I-S/2,T.max[0]=I+S/2,T.min[1]=B-z/2,T.max[1]=B+z/2,l.updateConfig({bbox:T})}p=F}b.preventDefault()}),c.addEventListener("touchend",()=>{d=!1,p=0,f()});function x(b){let _=l.bbox,w=_.max[0]-_.min[0],F=_.max[1]-_.min[1],P=(_.min[0]+_.max[0])/2,T=(_.min[1]+_.max[1])/2,C=F*b,I=c.width/c.height,B=C*I;_.min[0]=P-B/2,_.max[0]=P+B/2,_.min[1]=T-C/2,_.max[1]=T+C/2,l.updateConfig({bbox:_}),f()}function y(b,_){let w=l.bbox,F=w.max[0]-w.min[0],P=w.max[1]-w.min[1],T=F*b*.1,C=P*_*.1;w.min[0]+=T,w.max[0]+=T,w.min[1]+=C,w.max[1]+=C,l.updateConfig({bbox:w}),f()}$("#zoom-in").on("click",()=>x(.9)),$("#zoom-out").on("click",()=>x(1.11)),$("#pan-up").on("click",()=>y(0,1)),$("#pan-down").on("click",()=>y(0,-1)),$("#pan-left").on("click",()=>y(-1,0)),$("#pan-right").on("click",()=>y(1,0)),$("#pan-up-left").on("click",()=>y(-1,1)),$("#pan-up-right").on("click",()=>y(1,1)),$("#pan-down-left").on("click",()=>y(-1,-1)),$("#pan-down-right").on("click",()=>y(1,-1)),$("#pan-reset").on("click",function(){let b=l.bbox,_=b.max[0]-b.min[0],w=b.max[1]-b.min[1];l.updateConfig({bbox:{min:[-_/2,-w/2],max:[_/2,w/2]},reinitializeParticles:!1}),f()}),$("#mobile-zoom-in").on("click",()=>x(.9)),$("#mobile-zoom-out").on("click",()=>x(1.11)),$("#mobile-center").on("click",function(){let b=l.bbox,_=b.max[0]-b.min[0],w=b.max[1]-b.min[1];l.updateConfig({bbox:{min:[-_/2,-w/2],max:[_/2,w/2]},reinitializeParticles:!1}),f()})}function i(l,c){let d=document.getElementById("grid-canvas"),u=d.getContext("2d"),h=$("#cursor-position"),g=$("#show-grid").is(":checked"),f=0,p=0;function x(){d.width=c.width,d.height=c.height,w()}function y(S,A){let M=l.bbox,V=M.max[0]-M.min[0],D=M.max[1]-M.min[1],H=M.min[0]+S/c.width*V,U=M.max[1]-A/c.height*D;return{x:H,y:U}}function b(S,A){let M=l.bbox,V=M.max[0]-M.min[0],D=M.max[1]-M.min[1],H=(S-M.min[0])/V*c.width,U=(M.max[1]-A)/D*c.height;return{x:H,y:U}}function _(){let S=l.bbox,V=(S.max[0]-S.min[0])/10,D=Math.pow(10,Math.floor(Math.log10(V))),H=V/D,U;return H<1.5?U=1*D:H<3.5?U=2*D:H<7.5?U=5*D:U=10*D,U}function w(){if(!g){u.clearRect(0,0,d.width,d.height);return}u.clearRect(0,0,d.width,d.height);let S=l.bbox,A=_(),M=Math.floor(S.min[0]/A)*A,V=Math.ceil(S.max[0]/A)*A;u.strokeStyle="rgba(255, 255, 255, 0.1)",u.lineWidth=1,u.font="10px Courier New",u.fillStyle="rgba(255, 255, 255, 0.5)";for(let j=M;j<=V;j+=A){let te=b(j,0);if(u.beginPath(),u.moveTo(te.x,0),u.lineTo(te.x,d.height),u.stroke(),Math.abs(j)<1e-4)continue;let Re=j.toFixed(Math.max(0,-Math.floor(Math.log10(A))));u.fillText(Re,te.x+3,d.height-5)}let D=Math.floor(S.min[1]/A)*A,H=Math.ceil(S.max[1]/A)*A;for(let j=D;j<=H;j+=A){let te=b(0,j);if(u.beginPath(),u.moveTo(0,te.y),u.lineTo(d.width,te.y),u.stroke(),Math.abs(j)<1e-4)continue;let Re=j.toFixed(Math.max(0,-Math.floor(Math.log10(A))));u.fillText(Re,5,te.y-3)}let U=b(0,0);u.strokeStyle="rgba(255, 255, 255, 0.3)",u.lineWidth=2,U.y>=0&&U.y<=d.height&&(u.beginPath(),u.moveTo(0,U.y),u.lineTo(d.width,U.y),u.stroke(),u.fillText("0",U.x+3,U.y-3)),U.x>=0&&U.x<=d.width&&(u.beginPath(),u.moveTo(U.x,0),u.lineTo(U.x,d.height),u.stroke())}function F(){let S=y(f,p),A=c.getBoundingClientRect(),M=f-A.left,V=p-A.top,D=l.readPixelAt(M,V),H=`x: ${S.x.toFixed(3)}, y: ${S.y.toFixed(3)}`;if(D){if(D.hdr){let[U,j,te]=D.hdr;H+=`
HDR: (${U.toFixed(3)}, ${j.toFixed(3)}, ${te.toFixed(3)})`}if(D.ldr){let[U,j,te]=D.ldr;H+=`
LDR: (${U.toFixed(3)}, ${j.toFixed(3)}, ${te.toFixed(3)})`}}h.html(H.replace(/\n/g,"<br>"))}c.addEventListener("mousemove",S=>{f=S.clientX,p=S.clientY,F()}),$("#show-grid").on("change",function(){g=$(this).is(":checked"),w()}),window.addEventListener("resize",x),setInterval(w,100);let P=document.querySelector("#fps-counter .fps-display");function T(S){if(S>=1e3){let A=S/1e3;return A%1===0?`${Math.floor(A)}k`:`${A.toFixed(1)}k`}return S.toString()}function C(){if(l&&l.fps!==void 0){let S=l.totalFrames!==void 0?` | ${T(l.totalFrames)} frames`:"";P.textContent=`FPS: ${l.fps}${S}`}}setInterval(C,500);let I=$("#histogram-panel"),B=document.getElementById("histogram-canvas"),z=B.getContext("2d");$("#histogram-header").on("click",function(){I.toggleClass("collapsed"),$("#histogram-expand").text(I.hasClass("collapsed")?"\u25B6":"\u25BC")});function ee(){if(!l)return;let S=l.getBufferStats();if(!S||!S.histogram||S.histogram.length===0)return;let A=B,M=z,V=window.devicePixelRatio||1;A.width=A.clientWidth*V,A.height=A.clientHeight*V,M.scale(V,V);let D=A.clientWidth,H=A.clientHeight;M.fillStyle="rgba(0, 0, 0, 0.3)",M.fillRect(0,0,D,H);let U=Math.max(...S.histogram);if(U===0)return;let j=D/S.histogram.length;M.fillStyle="#4CAF50",S.histogram.forEach((te,Re)=>{let Rt=te/U*(H-4),pi=Re*j,gi=H-Rt;M.fillRect(pi,gi,j-1,Rt)}),$("#hist-avg").text(S.avgBrightness.toFixed(1)),$("#hist-max").text(S.maxBrightness.toFixed(0)),S.maxVelocity!==void 0&&S.maxVelocity!==null?$("#hist-vel").text(S.maxVelocity.toFixed(2)):$("#hist-vel").text("--")}setInterval(ee,1e3),x(),F()}function r(l){document.addEventListener("keydown",c=>{if(c.key==="Escape"){if(window.mobilePanelManager&&window.mobilePanelManager.getCurrentPanel()){c.preventDefault(),window.mobilePanelManager.hideAllPanels(),a.verbose("Mobile panel closed via Escape key");return}let d=$(".floating-panel:visible");if(d.length>0){c.preventDefault(),d.hide(),a.verbose("Floating panel closed via Escape key");return}}if(c.key==="c"&&!c.ctrlKey&&!c.altKey&&!c.metaKey&&!c.shiftKey){if(c.target.tagName==="INPUT"||c.target.tagName==="TEXTAREA")return;c.preventDefault(),l.clearScreen(),a.info("Screen cleared via keyboard shortcut (C)")}})}function o(l,c){let d=null;$("#open-animation-panel").on("click",function(){$("#animation-panel").toggle()}),$("#animation-script-input").on("change",function(u){let h=u.target.files[0];if(!h)return;let g=new FileReader;g.onload=function(f){try{let p=JSON.parse(f.target.result);d=new Et(l,c);let x=d.loadScript(p);$("#anim-name").text(x.name);let y=x.timeline[x.timeline.length-1].time,b=Math.ceil(y*x.fps);$("#anim-param").text(`${x.timeline.length} keyframes`),$("#anim-range").text(`0s \u2192 ${y}s`),$("#anim-frames").text(b),$("#anim-fps").text(x.fps),$("#anim-duration").text(`${y.toFixed(1)}s`),$("#animation-info").show(),$("#animation-run-btn").prop("disabled",!1),$("#animation-download-btn").prop("disabled",!0),a.info(`Animation script loaded: ${x.name}`)}catch(p){alert("Failed to load animation script: "+p.message),a.error("Animation script load failed",p)}},g.readAsText(h)}),$("#animation-run-btn").on("click",async function(){if(d)try{$("#animation-run-btn").prop("disabled",!0),$("#animation-pause-btn").prop("disabled",!1),$("#animation-stop-btn").prop("disabled",!1),$("#animation-script-input").prop("disabled",!0),$("#animation-progress").show(),await d.run((u,h,g)=>{let f=u/h*100;$("#progress-bar").css("width",f+"%"),$("#progress-text").text(`Frame ${u} / ${h}`),$("#progress-param").text(`Time: ${g.toFixed(2)}s`)}),a.info("Animation complete!"),alert("Animation complete! "+d.frames.length+" frames captured."),$("#animation-download-btn").prop("disabled",!1)}catch(u){a.error("Animation failed",u),alert("Animation failed: "+u.message)}finally{$("#animation-run-btn").prop("disabled",!1),$("#animation-pause-btn").prop("disabled",!0),$("#animation-stop-btn").prop("disabled",!0),$("#animation-script-input").prop("disabled",!1)}}),$("#animation-pause-btn").on("click",function(){d&&(d.isPaused?(d.resume(),$(this).text("\u23F8 Pause"),a.info("Animation resumed")):(d.pause(),$(this).text("\u25B6 Resume"),a.info("Animation paused")))}),$("#animation-stop-btn").on("click",function(){d&&(d.stop(),a.info("Animation stopped"),$("#animation-run-btn").prop("disabled",!1),$("#animation-pause-btn").prop("disabled",!0),$("#animation-stop-btn").prop("disabled",!0),$("#animation-script-input").prop("disabled",!1))}),$("#animation-download-btn").on("click",async function(){if(!(!d||d.frames.length===0))try{$(this).prop("disabled",!0),$(this).text("\u{1F4BE} Downloading..."),await d.downloadFrames(),$(this).text("\u2713 Downloaded"),setTimeout(()=>{$(this).text("\u{1F4BE} Download Frames (ZIP)"),$(this).prop("disabled",!1)},2e3)}catch(u){a.error("Download failed",u),alert("Download failed: "+u.message),$(this).text("\u{1F4BE} Download Frames (ZIP)"),$(this).prop("disabled",!1)}}),a.info("Animation panel initialized")}function s(){let l="accordionState",c={};try{let d=localStorage.getItem(l);d&&(c=JSON.parse(d))}catch(d){console.warn("Failed to load accordion state",d)}$("#controls h2").each(function(d){let u=$(this),h=u.next(".accordion-section");if(h.length===0)return;let g=u.text().trim();(c.hasOwnProperty(g)?c[g]:d>0)?(u.addClass("collapsed"),h.addClass("collapsed")):h.css("max-height",h[0].scrollHeight+"px"),u.on("click",function(){let x=$(this),y=x.next(".accordion-section"),b=x.hasClass("collapsed");b?(x.removeClass("collapsed"),y.css("max-height",y[0].scrollHeight+"px"),y.removeClass("collapsed")):(y.css("max-height",y[0].scrollHeight+"px"),y[0].offsetHeight,y.css("max-height","0"),x.addClass("collapsed"),y.addClass("collapsed")),c[g]=!b;try{localStorage.setItem(l,JSON.stringify(c))}catch(_){console.warn("Failed to save accordion state",_)}});let p=()=>{h.hasClass("collapsed")||h.css("max-height",h[0].scrollHeight+"px")};typeof ResizeObserver<"u"&&new ResizeObserver(p).observe(h[0])}),a.info("Accordion initialized with "+$("#controls h2").length+" sections")}s(),n(function(l,c){t(l,c),i(l,c),r(l),e(l,c)})});})();
