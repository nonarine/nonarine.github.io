var DeRender=(()=>{var jt=Object.defineProperty;var ln=Object.getOwnPropertyDescriptor;var cn=Object.getOwnPropertyNames;var un=Object.prototype.hasOwnProperty;var G=(n,e)=>()=>(n&&(e=n(n=0)),e);var Xt=(n,e)=>{for(var t in e)jt(n,t,{get:e[t],enumerable:!0})},dn=(n,e,t,i)=>{if(e&&typeof e=="object"||typeof e=="function")for(let r of cn(e))!un.call(n,r)&&r!==t&&jt(n,r,{get:()=>e[r],enumerable:!(i=ln(e,r))||i.enumerable});return n};var hn=n=>dn(jt({},"__esModule",{value:!0}),n);var qt,l,M=G(()=>{qt=class{constructor(){this.enabled=!0,this.verbosity="info",this.maxLogs=100,this.logs=[],this.consoleHooked=!1,this.originalConsole={},this.outputElement=null,this.showLineNumbers=!1;try{let e=localStorage.getItem("debugShowLineNumbers");this.showLineNumbers=e==="true"}catch{}this.silenced=!1,this.silencedBuffer=[],this.maxSilencedLogs=5e3}setShowLineNumbers(e){this.showLineNumbers=e;try{localStorage.setItem("debugShowLineNumbers",e?"true":"false")}catch{}}_getCallerInfo(){if(!this.showLineNumbers)return null;try{let e=new Error().stack;if(!e)return null;let t=e.split(`
`);for(let i=0;i<t.length;i++){let r=t[i];if(r.includes("debug-logger.js"))continue;let o=r.match(/([^\/]+\.js):(\d+):\d+\)?$/);if(o)return`${o[1]}:${o[2]}`}return null}catch{return null}}setOutputElement(e){this.outputElement=e}setEnabled(e){this.enabled=e}setVerbosity(e){let t=this.verbosity==="silent";this.verbosity=e;let i=e==="silent";t&&!i?this.unsilence():!t&&i&&this.silence()}silence(){this.silenced||(this.silenced=!0,this.info("Entering silent mode - logs will be buffered",null,!1))}unsilence(){if(!this.silenced)return;let e=this.silencedBuffer.length;this.silenced=!1,this.flush(),this.info(`Exited silent mode - flushed ${e} buffered logs`,null,!1)}flush(){if(this.silencedBuffer.length===0)return;let e=this.silencedBuffer.length;this.info(`Flushing ${e} silenced logs...`,null,!1);let t=this.consoleHooked?this.originalConsole.log:console.log;for(let i of this.silencedBuffer){let r=i.caller?`[${i.caller}] `:"",o=`[${i.timestamp}] ${r}${i.message}`;i.data?t(o,i.data):t(o),i.stack&&t("Stack trace:",i.stack)}this.logs.push(...this.silencedBuffer),this.logs.length>this.maxLogs&&(this.logs=this.logs.slice(-this.maxLogs)),this.silencedBuffer=[],this.updateUI()}clearSilencedBuffer(){let e=this.silencedBuffer.length;this.silencedBuffer=[],this.info(`Cleared ${e} buffered logs`,null,!1)}shouldLog(e){if(!this.enabled)return!1;let t={verbose:0,debug:1,info:2,warn:3,error:4,silent:0};return t[e]>=t[this.verbosity]}log(e,t,i=null,r=!0){if(!this.shouldLog(e))return;let o=new Date().toLocaleTimeString(),s=this._getCallerInfo(),a=null;i instanceof Error&&(a=i.stack,i={message:i.message,name:i.name,stack:i.stack});let c={timestamp:o,level:e,message:t,data:i,stack:a,caller:s};if(this.silenced&&r){this.silencedBuffer.push(c),this.silencedBuffer.length>this.maxSilencedLogs&&this.silencedBuffer.shift();return}this.logs.push(c),this.logs.length>this.maxLogs&&this.logs.shift();let u=this.consoleHooked?this.originalConsole.log:console.log,d=s?`[${s}] `:"",h=`[${o}] ${d}${t}`;i?u(h,i):u(h),a&&u("Stack trace:",a),this.updateUI()}verbose(e,t=null,i=!0){this.log("verbose",e,t,i)}debug(e,t=null,i=!0){this.log("debug",e,t,i)}info(e,t=null,i=!0){this.log("info",e,t,i)}warn(e,t=null,i=!0){this.log("warn",e,t,i)}error(e,t=null,i=!0){this.log("error",e,t,i)}clear(){this.logs=[],this.updateUI()}updateUI(){let e=this.outputElement?$(this.outputElement):$("#debug-output");if(!(!e||e.length===0)){e.empty();for(let t of this.logs){let i=$('<div class="debug-log"></div>').addClass(t.level),r=$('<span class="debug-timestamp"></span>').text(t.timestamp);if(t.caller){let s=$('<span class="debug-caller"></span>').text(`[${t.caller}] `).css({color:"#64B5F6","font-family":"monospace","font-size":"11px"});i.append(r).append(s)}else i.append(r);let o=$("<span></span>").text(t.message);if(i.append(o),t.data){let s=typeof t.data=="object"?JSON.stringify(t.data,null,2):String(t.data),a=$("<span></span>").text(" | "+s).css("color","#888");i.append(a)}if(t.stack){let s=$("<span></span>").text(" [stack]").css({color:"#ff6b6b",cursor:"pointer","text-decoration":"underline"}),a=$("<pre></pre>").text(t.stack).css({display:"none",color:"#ff6b6b","font-size":"9px",margin:"4px 0 0 20px",padding:"4px",background:"#2a0000","border-left":"2px solid #ff6b6b","overflow-x":"auto"});s.on("click",function(){a.toggle()}),i.append(s),i.append(a)}e.append(i)}e.scrollTop(e[0].scrollHeight)}}getLogs(){return this.logs}getSilencedBuffer(){return this.silencedBuffer}isSilenced(){return this.silenced}getBufferStats(){return{silenced:this.silenced,bufferSize:this.silencedBuffer.length,maxBufferSize:this.maxSilencedLogs,bufferUsagePercent:Math.round(this.silencedBuffer.length/this.maxSilencedLogs*100)}}hookConsole(){if(this.consoleHooked){this.warn("Console already hooked");return}this.originalConsole={log:console.log,info:console.info,warn:console.warn,error:console.error};let e=this;console.log=function(...t){e.originalConsole.log.apply(console,t);let i=t.map(r=>typeof r=="object"?JSON.stringify(r):String(r)).join(" ");e.debug("[console.log] "+i)},console.info=function(...t){e.originalConsole.info.apply(console,t);let i=t.map(r=>typeof r=="object"?JSON.stringify(r):String(r)).join(" ");e.debug("[console.info] "+i)},console.warn=function(...t){e.originalConsole.warn.apply(console,t);let i=t.map(r=>typeof r=="object"?JSON.stringify(r):String(r)).join(" ");e.warn("[console] "+i)},console.error=function(...t){e.originalConsole.error.apply(console,t);let i=t.map(r=>typeof r=="object"?JSON.stringify(r):String(r)).join(" ");e.error("[console] "+i)},this.consoleHooked=!0,this.info("Console hooked - browser console calls will echo to debug console")}unhookConsole(){if(!this.consoleHooked){this.warn("Console not hooked");return}console.log=this.originalConsole.log,console.info=this.originalConsole.info,console.warn=this.originalConsole.warn,console.error=this.originalConsole.error,this.consoleHooked=!1,this.info("Console unhooked - restored original behavior")}},l=new qt});function nt(n){let e=[],t=0,i=null;for(;t<n.length;){let r=n[t];if(/\s/.test(r)){t++;continue}if(/\d/.test(r)||r==="."&&/\d/.test(n[t+1])){let o="";for(;t<n.length&&/[\d.]/.test(n[t]);)o+=n[t++];let s={type:k.NUMBER,value:parseFloat(o)};e.push(s),i=s;continue}if(/[a-zA-Z_]/.test(r)){let o="";for(;t<n.length&&/[a-zA-Z_0-9]/.test(n[t]);)o+=n[t++];let s;Ii.has(o)||ee.hasOwnProperty(o)?s={type:k.FUNCTION,value:o}:Yt.hasOwnProperty(o)?s={type:k.NUMBER,value:Yt[o],isConstant:!0}:s={type:k.VARIABLE,value:o},e.push(s),i=s;continue}if(tt.hasOwnProperty(r)){if(r==="-"&&(i===null||i.type===k.OPERATOR||i.type===k.UNARY_MINUS||i.type===k.LPAREN||i.type===k.COMMA)){let s={type:k.UNARY_MINUS};e.push(s),i=s,t++;continue}let o={type:k.OPERATOR,value:r};e.push(o),i=o,t++;continue}if(r==="("){let o={type:k.LPAREN};e.push(o),i=o,t++;continue}if(r===")"){let o={type:k.RPAREN};e.push(o),i=o,t++;continue}if(r===","){let o={type:k.COMMA};e.push(o),i=o,t++;continue}throw new Error(`Unexpected character: ${r} at position ${t}`)}return e.push({type:k.EOF}),e}function ot(n){let e=[],t=[],i=0,r=()=>n[i],o=()=>n[i++];for(;r().type!==k.EOF;){let s=o();if(s.type===k.NUMBER)e.push(s);else if(s.type===k.VARIABLE)e.push(s);else if(s.type===k.FUNCTION)t.push(s);else if(s.type===k.UNARY_MINUS){for(;t.length>0&&t[t.length-1].type===k.UNARY_MINUS;)e.push(t.pop());t.push(s)}else if(s.type===k.COMMA)for(;t.length>0&&t[t.length-1].type!==k.LPAREN;)e.push(t.pop());else if(s.type===k.OPERATOR){let a=s;for(;t.length>0;){let c=t[t.length-1];if(c.type===k.OPERATOR){let u=tt[a.value],d=tt[c.value];if(u.associativity==="L"&&u.precedence<=d.precedence||u.associativity==="R"&&u.precedence<d.precedence)e.push(t.pop());else break}else if(c.type===k.UNARY_MINUS)if(tt[a.value].precedence<=2)e.push(t.pop());else break;else break}t.push(a)}else if(s.type===k.LPAREN)t.push(s);else if(s.type===k.RPAREN){for(;t.length>0&&t[t.length-1].type!==k.LPAREN;)e.push(t.pop());if(t.length===0)throw new Error("Mismatched parentheses");t.pop(),t.length>0&&t[t.length-1].type===k.FUNCTION&&e.push(t.pop())}}for(;t.length>0;){let s=t.pop();if(s.type===k.LPAREN)throw new Error("Mismatched parentheses");e.push(s)}return mn(e)}function mn(n){let e=[];for(let t of n)if(t.type===k.NUMBER)e.push(new it(t.value,t.isConstant));else if(t.type===k.VARIABLE)e.push(new rt(t.value));else if(t.type===k.UNARY_MINUS){if(e.length<1)throw new Error("Invalid expression");let i=e.pop();e.push(new be("-",i))}else if(t.type===k.OPERATOR){if(e.length<2)throw new Error("Invalid expression");let i=e.pop(),r=e.pop();e.push(new ge(t.value,r,i))}else if(t.type===k.FUNCTION){let i=gn(t.value);if(e.length<i)throw new Error(`Not enough arguments for ${t.value}`);let r=[];for(let o=0;o<i;o++)r.unshift(e.pop());e.push(new xe(t.value,r))}if(e.length!==1)throw new Error("Invalid expression");return e[0]}function re(n){if(n.type==="number")return new it(n.value,n.isConstant);if(n.type==="variable")return new rt(n.name);if(n.type==="unaryOp")return new be(n.operator,re(n.operand));if(n.type==="binaryOp")return new ge(n.operator,re(n.left),re(n.right));if(n.type==="functionCall")return new xe(n.name,n.args.map(e=>re(e)));throw new Error(`Unknown node type: ${n.type}`)}function Ve(n,e){if(n.type==="number")return re(n);if(n.type==="variable")return e[n.name]?re(e[n.name]):re(n);if(n.type==="unaryOp")return new be(n.operator,Ve(n.operand,e));if(n.type==="binaryOp")return new ge(n.operator,Ve(n.left,e),Ve(n.right,e));if(n.type==="functionCall"){let t=n.args.map(i=>Ve(i,e));return new xe(n.name,t)}throw new Error(`Unknown node type: ${n.type}`)}function fe(n,e){if(n.type==="number"||n.type==="variable")return re(n);if(n.type==="unaryOp")return new be(n.operator,fe(n.operand,e));if(n.type==="binaryOp")return new ge(n.operator,fe(n.left,e),fe(n.right,e));if(n.type==="functionCall")if(e[n.name]){let t=e[n.name],i=n.args.map(o=>fe(o,e)),r={};return t.params.forEach((o,s)=>{r[o]=i[s]}),Ve(t.bodyAST,r)}else{let t=n.args.map(i=>fe(i,e));return new xe(n.name,t)}throw new Error(`Unknown node type: ${n.type}`)}function Ne(n,e=0){let t="  ".repeat(e);if(n.type==="number")return n.isConstant?`${t}NumberNode(${n.value} [constant])`:`${t}NumberNode(${n.value})`;if(n.type==="variable")return`${t}VariableNode(${n.name})`;if(n.type==="unaryOp"){let i=Ne(n.operand,e+1);return`${t}UnaryOpNode(${n.operator})
${i}`}else if(n.type==="binaryOp"){let i=Ne(n.left,e+1),r=Ne(n.right,e+1);return`${t}BinaryOpNode(${n.operator})
${i}
${r}`}else if(n.type==="functionCall"){let i=n.args.map(r=>Ne(r,e+1)).join(`
`);return`${t}FunctionCallNode(${n.name})
${i}`}return`${t}Unknown(${n.type})`}function ki(n,e){let t=new Set(e);["dx","dy","dz","dw","du","dv"].forEach(o=>t.add(o)),t.add("a");function r(o){if(o.type==="number")return o.isConstant&&{PI:"Math.PI",E:"Math.E"}[o.value]||o.value.toString();if(o.type==="variable"){if(t.has(o.name))return o.name;throw new Error(`Unknown variable: ${o.name}. Available: ${e.join(", ")}, dx, dy, dz, dw, du, dv, a`)}else{if(o.type==="unaryOp")return`(-${r(o.operand)})`;if(o.type==="binaryOp"){let s=r(o.left),a=r(o.right);return o.operator==="^"?`Math.pow(${s}, ${a})`:`(${s} ${o.operator} ${a})`}else if(o.type==="functionCall"){let s=o.args.map(u=>r(u));return`${{mod:"%",fract:"(x => x - Math.floor(x))",mix:"(a, b, t) => a * (1 - t) + b * t"}[o.name]||`Math.${o.name}`}(${s.join(", ")})`}}throw new Error(`Unknown node type: ${o.type}`)}return r(n)}function pn(n,e){let t=e.match(/^\(*(\-?[\d]+)\.?0?\)*$/);if(t){let i=parseInt(t[1]);return i===0?"1.0":i===1?n:i===-1?`(1.0/(${n}))`:`(${i<0?"1.0/":""}(${Array(Math.abs(i)).fill(n).join("*")}))`}return`pow(${n}, ${e})`}function Di(n,e,t=!1,i="pos"){let r={};if(t)e.forEach(s=>{r[s]=s});else{let s=["x","y","z","w","u","v"],a=["dx","dy","dz","dw","du","dv"];e.forEach((c,u)=>{u<6?r[c]=`${i}.${s[u]}`:r[c]=`${i}[${u}]`}),a.forEach((c,u)=>{u<6?r[c]=`velocity.${s[u]}`:r[c]=`velocity[${u}]`}),r.a="u_alpha"}function o(s){if(s.type==="number"){if(s.isConstant)return{PI:"3.14159265359",E:"2.71828182846"}[s.value]||s.value.toString();{let a=s.value.toString();return!a.includes(".")&&!a.includes("e")&&!a.includes("E")&&(a+=".0"),a}}else if(s.type==="variable"){if(r.hasOwnProperty(s.name))return r[s.name];throw new Error(`Unknown variable: ${s.name}. Available: ${e.join(", ")}`)}else{if(s.type==="unaryOp")return`(-${o(s.operand)})`;if(s.type==="binaryOp"){let a=o(s.left),c=o(s.right);return s.operator==="^"?pn(a,c):s.operator==="%"?`mod(${a}, ${c})`:`(${a} ${s.operator} ${c})`}else if(s.type==="functionCall"){let a=s.args.map(u=>o(u)),c=s.name;return s.name==="atan2"&&(c="atan"),`${c}(${a.join(", ")})`}}throw new Error(`Unknown node type: ${s.type}`)}return o(n)}function fn(n,e){let t=new Set(e);["dx","dy","dz","dw","du","dv"].forEach(a=>t.add(a)),t.add("a");function r(a){return a.type==="binaryOp"||a.type==="unaryOp"}function o(a){return a.type==="binaryOp"&&(a.operator==="+"||a.operator==="-")}function s(a){if(a.type==="number")return a.isConstant&&{PI:"\\pi",E:"e"}[a.value]||a.value.toString();if(a.type==="variable"){if(t.has(a.name))return a.name;throw new Error(`Unknown variable: ${a.name}`)}else if(a.type==="unaryOp"){let c=s(a.operand);return c.includes(" ")||c.includes("+")||c.includes("-")?`-(${c})`:`-${c}`}else if(a.type==="binaryOp"){let c=s(a.left),u=s(a.right);if(a.operator==="^")return`{${r(a.left)?`\\left(${c}\\right)`:c}}^{${u}}`;if(a.operator==="*"){let d=o(a.left)?`\\left(${c}\\right)`:c,h=o(a.right)?`\\left(${u}\\right)`:u;return`${d} \\cdot ${h}`}else return a.operator==="/"?`\\frac{${c}}{${u}}`:a.operator==="-"?`${c} - ${u}`:a.operator==="+"?`${c} + ${u}`:`${c} ${a.operator} ${u}`}else if(a.type==="functionCall"){let c=a.args.map(u=>s(u));return a.name==="sqrt"?`\\sqrt{${c[0]}}`:a.name==="sin"?`\\sin(${c[0]})`:a.name==="cos"?`\\cos(${c[0]})`:a.name==="tan"?`\\tan(${c[0]})`:a.name==="exp"?`\\exp(${c[0]})`:a.name==="log"?`\\log(${c[0]})`:a.name==="abs"?`\\left|${c[0]}\\right|`:`${a.name}(${c.join(", ")})`}throw new Error(`Unknown node type: ${a.type}`)}return s(n)}function gn(n){return ee.hasOwnProperty(n)?ee[n].params.length:new Set(["min","max","pow","mod","dot","atan2"]).has(n)?2:1}function bn(n){let e="";for(let[t,i]of Object.entries(ee)){let r=nt(i.body),o=ot(r),s=Di(o,i.params,!0);e+=`float ${t}(`,e+=i.params.map(a=>`float ${a}`).join(", "),e+=`) {
`,e+=`    return ${s};
`,e+=`}

`}return e}function Bi(n,e,t=null){let i=t||["x","y","z","w","u","v"].slice(0,e);try{let r=nt(n),o=ot(r);return l.verbose(`Parsed expression: "${n}"`),l.verbose(`AST tree:
`+Ne(o)),new Jt(o)}catch(r){throw new Error(`Parse error: ${r.message}`)}}function W(n,e,t=null,i="pos"){let r=t||["x","y","z","w","u","v"].slice(0,e);return Bi(n,e,t).toGLSL(r,{useDirectMapping:!1,posVarName:i})}function st(){return bn([])}function Vi(n,e=null,t="pos"){let i=n.length;return n.map((r,o)=>{try{return W(r.trim(),i,e,t)}catch(s){throw new Error(`Error in dimension ${o}: ${s.message}`)}})}function Kt(n,e=null){let t=n.length,i=e||["x","y","z","w","u","v"].slice(0,t);return n.map((r,o)=>{try{let s=nt(r.trim()),a=ot(s),u=`return ${ki(a,i)};`;return new Function(...i,u)}catch(s){throw new Error(`Error creating evaluator for dimension ${o}: ${s.message}`)}})}function Ni(n,e,t=null){let i=t||["x","y","z","w","u","v"].slice(0,e);return Bi(n,e,t).toTeX(i)}function zi(n){for(let t in ee)delete ee[t];if(!n||!n.trim())return;let e=n.split(`
`);for(let t=0;t<e.length;t++){let i=e[t].trim();if(!i||i.startsWith("//")||i.startsWith("#"))continue;let r=i.match(/^([a-zA-Z_][a-zA-Z_0-9]*)\s*\(([^)]*)\)\s*=\s*(.+)$/);if(!r)throw new Error(`Line ${t+1}: Invalid function definition syntax. Expected: functionName(arg1, arg2) = expression`);let[,o,s,a]=r;if(Ii.has(o))throw new Error(`Line ${t+1}: Cannot override built-in function '${o}'`);if(Yt.hasOwnProperty(o))throw new Error(`Line ${t+1}: Cannot use constant name '${o}' as function name`);let c=s.split(",").map(u=>u.trim()).filter(u=>u);for(let u of c)if(!/^[a-zA-Z_][a-zA-Z_0-9]*$/.test(u))throw new Error(`Line ${t+1}: Invalid parameter name '${u}'`);try{let u=nt(a);ot(u)}catch(u){throw new Error(`Line ${t+1}: Error parsing function body: ${u.message}`)}ee[o]={params:c,body:a}}}function Ui(){return{...ee}}var Wt,Jt,de,it,rt,ge,be,xe,k,tt,Ii,ee,Yt,se=G(()=>{M();Wt=class{toGLSL(e,t={}){throw new Error("Must implement toGLSL()")}toTeX(e){throw new Error("Must implement toTeX()")}toJS(e){throw new Error("Must implement toJS()")}clone(){throw new Error("Must implement clone()")}substitute(e){throw new Error("Must implement substitute()")}},Jt=class n extends Wt{constructor(e){super(),this.ast=e}toGLSL(e,t={}){return Di(this.ast,e,t.useDirectMapping||!1,t.posVarName||"pos")}toTeX(e){return fn(this.ast,e)}toJS(e){return ki(this.ast,e)}clone(){return new n(re(this.ast))}substitute(e){let t={};for(let[r,o]of Object.entries(e))o instanceof n&&ee[r]&&(t[r]={params:ee[r].params,bodyAST:o.ast});let i=fe(this.ast,t);return new n(i)}},de=class{constructor(e){this.type=e}},it=class extends de{constructor(e,t=!1){super("number"),this.value=e,this.isConstant=t}},rt=class extends de{constructor(e){super("variable"),this.name=e}},ge=class extends de{constructor(e,t,i){super("binaryOp"),this.operator=e,this.left=t,this.right=i}},be=class extends de{constructor(e,t){super("unaryOp"),this.operator=e,this.operand=t}},xe=class extends de{constructor(e,t){super("functionCall"),this.name=e,this.args=t}},k={NUMBER:"NUMBER",VARIABLE:"VARIABLE",FUNCTION:"FUNCTION",OPERATOR:"OPERATOR",UNARY_MINUS:"UNARY_MINUS",LPAREN:"LPAREN",RPAREN:"RPAREN",COMMA:"COMMA",EOF:"EOF"},tt={"+":{precedence:1,associativity:"L"},"-":{precedence:1,associativity:"L"},"*":{precedence:2,associativity:"L"},"/":{precedence:2,associativity:"L"},"^":{precedence:3,associativity:"R"},"%":{precedence:2,associativity:"L"}},Ii=new Set(["sin","cos","tan","asin","acos","atan","atan2","sinh","cosh","tanh","exp","log","log2","sqrt","abs","floor","ceil","fract","sign","min","max","pow","mod","length","normalize","dot"]),ee={},Yt={pi:"PI",e:"E",PI:"PI",E:"E"}});function Yi(n){Z=n,l.info("Jacobian: Notebook set (CAS engine:",Z.casEngine.getName()+")")}function ne(n,e){if(l.verbosity="verbose",l.info("=== JACOBIAN COMPUTATION START ==="),l.info("Timestamp:",new Date().toISOString()),l.info("Notebook:",Z?"set":"not set"),l.info("CAS Engine:",Z?Z.casEngine.getName():"not available"),l.info("Input expressions:",n.toString()),l.info("Dimensions:",e),!Z)return l.error("Notebook not set - cannot compute Jacobian"),null;if(!Z.casEngine.isReady())return l.error("CAS engine not ready - cannot compute Jacobian"),null;try{let t=n.map(r=>{let o=r.replace(/atan2\s*\(\s*([^,]+)\s*,\s*([^)]+)\s*\)/g,"atan(($1)/($2))");return o!==r&&l.verbose(`Replaced atan2 for differentiation: "${r}" \u2192 "${o}"`),o}),i=[];for(let r=0;r<e;r++){let o=[],s=t[r];l.verbose(`Row ${r}: differentiating expression "${s}"`);for(let a=0;a<e;a++){let c=$n[a];try{l.verbose(`  Computing \u2202(${s})/\u2202${c}...`);let u=Z.differentiate(s,c);l.verbose(`  Derivative: ${u}`),o.push(u)}catch(u){l.warn(`Failed to differentiate expression "${s}" w.r.t. ${c}: ${u.message}`),o.push("0")}}i.push(o)}l.info("=== JACOBIAN COMPUTATION COMPLETE ==="),l.info("Final Jacobian matrix:");for(let r=0;r<i.length;r++)l.info(`  Row ${r}: [${i[r].join(", ")}]`);return i}catch(t){return l.error("Failed to compute Jacobian matrix",t),null}}function ze(n){if(!n||!Array.isArray(n))return!1;for(let e of n){if(!Array.isArray(e))return!1;for(let t of e)if(typeof t!="string"||t==="")return!1}return!0}function Zt(n){if(!n||!Array.isArray(n))return l.error("Invalid Jacobian for inversion"),null;if(!Z)return l.error("Notebook not set - cannot invert Jacobian"),null;let e=n.length;if(e<2||e>4)return l.error(`Jacobian inversion only supported for 2x2, 3x3, and 4x4 matrices (got ${e}x${e})`),null;try{l.verbose("Inverting Jacobian matrix using",Z.casEngine.getName());let t=Z.invertMatrix(n);if(t){l.verbose("Inverted Jacobian:");for(let i=0;i<t.length;i++)l.verbose(`  Row ${i}: [${t[i].join(", ")}]`)}return t}catch(t){return l.error("Failed to invert Jacobian:",t.message),null}}var Z,$n,ht=G(()=>{M();Z=null;$n=["x","y","z","w","u","v"]});function Vn(n){let e=[],t=/atan2\s*\(\s*([^,]+)\s*,\s*([^)]+)\s*\)/g,i;for(;(i=t.exec(n))!==null;)e.push({fullMatch:i[0],numerator:i[1].trim(),denominator:i[2].trim()});return e}function cr(n,e){let{numerator:t,denominator:i,fullMatch:r}=e,o=`PI/2 - atan2(${i}, ${t})`,s=n.map(c=>c.replace(r,o));return{condition:`abs(${i}) <= abs(${t})`,forwardTransforms:s}}function ur(n,e){let t=[],i=[];if(n.forEach((r,o)=>{Vn(r).forEach(a=>{i.push({...a,transformIndex:o})})}),i.length===0)return[{condition:"true",forwardTransforms:n}];if(e===2||e===3&&i.length===1){let r=i[0];return t.push({condition:`abs(${r.denominator}) >= abs(${r.numerator})`,forwardTransforms:n}),t.push(cr(n,r)),t}if(e===3&&i.length>=2){let r=i[i.length-1];return t.push({condition:`abs(${r.denominator}) >= abs(${r.numerator})`,forwardTransforms:n}),t.push(cr(n,r)),t}return[{condition:"true",forwardTransforms:n}]}function dr(n,e){if(n==="true")return"true";let t=n;return e.forEach((i,r)=>{let o=["x","y","z","w"][r],s=new RegExp(`\\b${i}\\b`,"g");t=t.replace(s,`pos.${o}`)}),t=t.replace(/\bPI\b/g,"3.14159265359"),t}var hr=G(()=>{});var pr={};Xt(pr,{CoordinateSystem:()=>O,PRESET_COORDINATE_SYSTEMS:()=>he,createCartesianSystem:()=>mr,getCartesianSystem:()=>j,getCoordinateSystem:()=>Nn,getPresetsForDimension:()=>di});function Nn(n){return he[n]||null}function di(n){let e=[];for(let[t,i]of Object.entries(he))i.dimensions===n&&e.push({key:t,system:i});return e}function j(n){let e=`cartesian${n}d`;return he[e]||mr(n)}function mr(n){let e=["x","y","z","w","u","v"].slice(0,n);return new O(`Cartesian ${n}D`,n,e.map(t=>({label:t,displayLabel:t})),e)}var O,he,ae=G(()=>{ht();hr();O=class n{constructor(e,t,i,r,o=null,s=!1,a=null){this.name=e,this.dimensions=t,this.variables=i,this.forwardTransforms=r,this.inverseTransforms=o,this.useIterativeSolver=s,this.charts=a||ur(r,t)}getVariableNames(){return this.variables.map(e=>e.label)}getDisplayLabels(){return this.variables.map(e=>e.displayLabel)}generateForwardTransformGLSL(e,t){let i=this.getVariableNames(),r=`vec${this.dimensions}`,o=this.forwardTransforms.map((s,a)=>{let c=t(s,e);return`    result.${["x","y","z","w"][a]} = ${c};`}).join(`
`);return`
...
}`;let o=Zt(r);if(!o)return console.error("Failed to invert Jacobian for coordinate system:",this.name),console.error("Forward Jacobian was:",r),`
// Velocity transform: ${this.name} \u2192 Cartesian (IDENTITY FALLBACK - ERROR)
...
}`;let s=`vec${this.dimensions}`,a=["x","y","z","w"],c=[];for(let u=0;u<this.dimensions;u++){let h=o[u].map((p,m)=>`(${t(p,e)}) * vel_native.${a[m]}`);c.push(`    result.${a[u]} = ${h.join(" + ")};`)}return`
...
}`;let s=o.map((c,u)=>{let d=dr(c.condition,e),h=u===0?"if":"else if",p=d==="true"?"":` (${d})`;return`    ${h}${p} {
        // Chart ${u+1}: Avoid singularities when ${c.condition}
${c.matrixMultCode.join(`
`)}
    }`}).join(" "),a=o.map((c,u)=>`// Chart ${u+1}: condition=${c.condition}
...
${s}
    return result;
}`}generateInverseTransformGLSL(e,t){let i=`vec${this.dimensions}`,r=["x","y","z","w"],o=["x","y","z","w","u","v"].slice(0,this.dimensions);if(this.inverseTransforms&&this.inverseTransforms.length===this.dimensions){let s=this.inverseTransforms.map((a,c)=>`    result.${r[c]} = ${a};`).join(`
`);return`
// Inverse transform: ${this.name} (Native \u2192 Cartesian)
// Using explicit inverse transform (Tier 1)
${i} transformToCartesian(${i} native_pos) {
    ${i} result;
    float ${e.map((a,c)=>`${a} = native_pos.${r[c]}`).join(", ")};
${s}
    return result;
...
}`)}generateNewtonSolverGLSL(e,t,i){let r=`vec${this.dimensions}`,o=`mat${this.dimensions}`,s=["x","y","z","w"],a=this.forwardTransforms.map((d,h)=>{let p=i(d,e);return`        native.${s[h]} = ${p};`}).join(`
...
`}function Hi(n,e,t,i,r=null,o=null){let s=Array.from({length:n},(g,y)=>`uniform sampler2D u_pos_${y};`).join(`
`),a=`vec${n}`,c=Array.from({length:n},(g,y)=>`decodeFloat(texture2D(u_pos_${y}, v_texcoord))`).join(", "),u=["x","y","z","w"],d=e.map((g,y)=>`    result.${u[y]} = ${g};`).join(`
...
`}function ji(n,e,t,i,r=!1,o=null){let s=o&&o.forwardTransform,a=Array.from({length:n},(b,x)=>`uniform sampler2D u_pos_${x};`).join(`
...
`}var lt=class{constructor(e,t,i,r={}){this.gl=e,this.width=t,this.height=i,this.enabled=r.enabled!==void 0?r.enabled:!0,this.intensity=r.intensity!==void 0?r.intensity:.75,this.threshold=r.threshold!==void 0?r.threshold:.1,this.edgesFramebuffer=null,this.edgesTexture=null,this.blendFramebuffer=null,this.blendTexture=null,this.edgeDetectionProgram=null,this.blendingWeightProgram=null,this.neighborhoodBlendProgram=null,this.quadBuffer=null,this.initialize(),l.info("SMAAManager initialized",{size:`${t}x${i}`,enabled:this.enabled,intensity:this.intensity,threshold:this.threshold})}initialize(){let e=this.gl;if(this.createFramebuffers(),this.compileShaders(),!this.quadBuffer){let t=new Float32Array([0,0,1,0,0,1,0,1,1,0,1,1]);this.quadBuffer=e.createBuffer(),e.bindBuffer(e.ARRAY_BUFFER,this.quadBuffer),e.bufferData(e.ARRAY_BUFFER,t,e.STATIC_DRAW)}}createFramebuffers(){let e=this.gl;this.edgesTexture=this.createTexture(),this.edgesFramebuffer=this.createFramebuffer(this.edgesTexture),this.blendTexture=this.createTexture(),this.blendFramebuffer=this.createFramebuffer(this.blendTexture)}createTexture(){let e=this.gl,t=e.createTexture();return e.bindTexture(e.TEXTURE_2D,t),e.texParameteri(e.TEXTURE_2D,e.TEXTURE_MIN_FILTER,e.LINEAR),e.texParameteri(e.TEXTURE_2D,e.TEXTURE_MAG_FILTER,e.LINEAR),e.texParameteri(e.TEXTURE_2D,e.TEXTURE_WRAP_S,e.CLAMP_TO_EDGE),e.texParameteri(e.TEXTURE_2D,e.TEXTURE_WRAP_T,e.CLAMP_TO_EDGE),e.texImage2D(e.TEXTURE_2D,0,e.RGBA,this.width,this.height,0,e.RGBA,e.UNSIGNED_BYTE,null),t}createFramebuffer(e){let t=this.gl,i=t.createFramebuffer();t.bindFramebuffer(t.FRAMEBUFFER,i),t.framebufferTexture2D(t.FRAMEBUFFER,t.COLOR_ATTACHMENT0,t.TEXTURE_2D,e,0);let r=t.checkFramebufferStatus(t.FRAMEBUFFER);return r!==t.FRAMEBUFFER_COMPLETE&&l.error("SMAA framebuffer incomplete",{status:r}),t.bindFramebuffer(t.FRAMEBUFFER,null),i}compileShaders(){let e=this.gl,t=`
attribute vec2 a_pos;
varying vec2 v_uv;
...
`;return this.reductionProgram=this.createProgram(t,i),this.reductionProgram?(this.initialized=!0,!0):(l.error("Failed to create reduction shader"),!1)}createProgram(e,t){let i=this.gl,r=i.createShader(i.VERTEX_SHADER);if(i.shaderSource(r,e),i.compileShader(r),!i.getShaderParameter(r,i.COMPILE_STATUS))return l.error("Vertex shader compile error:",i.getShaderInfoLog(r)),null;let o=i.createShader(i.FRAGMENT_SHADER);if(i.shaderSource(o,t),i.compileShader(o),!i.getShaderParameter(o,i.COMPILE_STATUS))return l.error("Fragment shader compile error:",i.getShaderInfoLog(o)),null;let s=i.createProgram();return i.attachShader(s,r),i.attachShader(s,o),i.linkProgram(s),i.getProgramParameter(s,i.LINK_STATUS)?s:(l.error("Program link error:",i.getProgramInfoLog(s)),null)}compute(e,t,i,r=!0){if(!this.initialized&&!this.initialize())return this.stats;let o=this.gl,s=r?16:2;return this.computeCPUSampled(e,t,i,s)}computeCPUSampled(e,t,i,r){let o=this.gl,s=o.createFramebuffer();if(o.bindFramebuffer(o.FRAMEBUFFER,s),o.framebufferTexture2D(o.FRAMEBUFFER,o.COLOR_ATTACHMENT0,o.TEXTURE_2D,e,0),o.checkFramebufferStatus(o.FRAMEBUFFER)!==o.FRAMEBUFFER_COMPLETE)return l.error("Framebuffer not complete for stats readback"),o.deleteFramebuffer(s),this.stats;let a=4,c=16,u=a*a*c*c,d=new Float32Array(u*4),h=0;for(let v=0;v<a;v++)for(let C=0;C<a;C++){let w=Math.floor(t/a*C+(t/a-c)/2),T=Math.floor(i/a*v+(i/a-c)/2),R=new Float32Array(c*c*4);o.readPixels(w,T,c,c,o.RGBA,o.FLOAT,R),d.set(R,h*4),h+=c*c}o.deleteFramebuffer(s),o.bindFramebuffer(o.FRAMEBUFFER,null);let p=1/0,m=-1/0,f=0,b=1/0,x=-1/0,g=0,y=1/0,_=-1/0,S=0,F=u;for(let v=0;v<F;v++){let C=v*4,w=d[C],T=d[C+1],R=d[C+2];p=Math.min(p,w),m=Math.max(m,w),f+=w,b=Math.min(b,T),x=Math.max(x,T),g+=T,y=Math.min(y,R),_=Math.max(_,R),S+=R}let E=F>0?f/F:0,L=F>0?g/F:0,D=F>0?S/F:0,A=20,B=new Array(A).fill(0),N=Math.max(m,x,_);if(N>0){let v=Math.log2(N+1);for(let C=0;C<F;C++){let w=C*4,T=Math.max(d[w],d[w+1],d[w+2]),R=Math.log2(T+1),P=Math.min(Math.floor(R/v*A),A-1);B[P]++}}return this.stats={red:{min:p,max:m,avg:E},green:{min:b,max:x,avg:L},blue:{min:y,max:_,avg:D},maxBrightness:Math.max(m,x,_),avgBrightness:(E+L+D)/3,histogram:B,timestamp:Date.now(),sampledPixels:F,sampleRate:r},this.stats}getCached(){return this.stats}dispose(){let e=this.gl;this.quadBuffer&&e.deleteBuffer(this.quadBuffer),this.reductionProgram&&e.deleteProgram(this.reductionProgram),this.finalProgram&&e.deleteProgram(this.finalProgram),this.reductionFBOs.forEach(t=>e.deleteFramebuffer(t)),this.reductionTextures.forEach(t=>e.deleteTexture(t)),this.reductionFBOs=[],this.reductionTextures=[]}};M();var ut=class{constructor(e){this.gl=e,this.program=null,this.quadBuffer=null,this.resultTexture=null,this.resultFBO=null,this.stats={maxVelocity:5,avgVelocity:2,sampleCount:0,timestamp:0},this.initialized=!1,this.shaderSource=null}initialize(e,t,i=null){let r=this.gl;if(this.quadBuffer=r.createBuffer(),r.bindBuffer(r.ARRAY_BUFFER,this.quadBuffer),r.bufferData(r.ARRAY_BUFFER,new Float32Array([0,0,1,0,0,1,0,1,1,0,1,1]),r.STATIC_DRAW),this.resultTexture=r.createTexture(),r.bindTexture(r.TEXTURE_2D,this.resultTexture),r.texImage2D(r.TEXTURE_2D,0,r.RGBA,64,1,0,r.RGBA,r.FLOAT,null),r.texParameteri(r.TEXTURE_2D,r.TEXTURE_MIN_FILTER,r.NEAREST),r.texParameteri(r.TEXTURE_2D,r.TEXTURE_MAG_FILTER,r.NEAREST),r.texParameteri(r.TEXTURE_2D,r.TEXTURE_WRAP_S,r.CLAMP_TO_EDGE),r.texParameteri(r.TEXTURE_2D,r.TEXTURE_WRAP_T,r.CLAMP_TO_EDGE),this.resultFBO=r.createFramebuffer(),r.bindFramebuffer(r.FRAMEBUFFER,this.resultFBO),r.framebufferTexture2D(r.FRAMEBUFFER,r.COLOR_ATTACHMENT0,r.TEXTURE_2D,this.resultTexture,0),r.checkFramebufferStatus(r.FRAMEBUFFER)!==r.FRAMEBUFFER_COMPLETE)return l.error("Velocity stats framebuffer not complete"),this.dispose(),!1;r.bindFramebuffer(r.FRAMEBUFFER,null);let o=`vec${e}`,s=["x","y","z","w"],a=t.map((f,b)=>`    result.${s[b]} = ${f};`).join(`
...
`,i+=`}
`,i}function mt(n,e){let t=e===2?"mat2":e===3?"mat3":"mat4",i=`vec${e}`,r="";for(let a=0;a<e;a++)r+=`    float ${Sn[a]} = pos.${["x","y","z","w"][a]};
`;let o=[];for(let a=0;a<e;a++)for(let c=0;c<e;c++){let u=n[c][a];try{let d=W(u,e);o.push(d)}catch(d){l.warn(`Failed to compile Jacobian element [${c}][${a}]: ${u}`,d),o.push("0.0")}}return`
${He(e)}
...
${r}
    return ${t}(
...
}`}function te(n,e,t){let i=e(n);return`        ${n} = ${i};`}function Ue(n,e,t){let i=`vec${t}`,r=`${n}_pred`,o=`${n}_mid`,s=e(n),a=e(o);return`        // Predictor: standard fixed-point step
        ${i} ${r} = ${s};

        // Corrector: evaluate at midpoint between current and predictor
        ${i} ${o} = (${n} + ${r}) * 0.5;
        ${n} = ${a};`}function Oe(n,e,t,i){let r=`vec${i}`,o=i===2?"mat2":i===3?"mat3":"mat4",s=`inverse${i}`,a=e(n),c=t(n);return`        ${r} F_${n} = ${a};
        ${o} J_${n} = ${c};

        // Newton step: x -= J^(-1) * F
        ${r} delta_${n} = ${s}(J_${n}) * F_${n};
        ${n} -= delta_${n};`}function Ge(n,e,t,i="h",r=null){let o=`vec${t}`,s=t===2?"mat2":t===3?"mat3":"mat4",a=`inverse${t}`,c=["x","y","z","w"],u="1e-4",d="",h=[];for(let f=0;f<t;f++){let b=`Df_col${f}_${n}`,x=Array.from({length:t},(_,S)=>S===f?u:"0.0").join(", "),g=e(`${n} + ${o}(${x})`),y=e(n);d+=`
        ...
        // Newton step: x -= J^(-1) * F
        ${o} delta_${n} = ${a}(J_${n}) * F_${n};
        ${n} -= delta_${n};`}function Tn(n){return{name:"Euler",costFactor:1,code:`
// Euler integration
vec${n} integrate(vec${n} pos, float h) {
    vec${n} velocity = get_velocity(pos);
    return pos + h * velocity;
}
`}}function Ki(n){return{name:"Explicit Midpoint",costFactor:2,code:`
...
`}}function Cn(n){return{name:"Heun (Explicit Trapezoidal)",costFactor:2,code:`
...
`}}function Zi(n){return{name:"RK4",costFactor:4,code:`
...
`}}function Qi(n,e=3,t="fixed-point",i=null){l.info(`*** Implicit Euler integrator requested: solutionMethod=${t}, hasExpressions=${!!i}`);let r=`vec${n}`,o=n===2?"mat2":n===3?"mat3":"mat4",s="pos + h * get_velocity(pos)",a=p=>`pos + h * get_velocity(${p})`,c=p=>`get_velocity(${p})`,u,d,h="";if(t==="newton"&&i){l.info("Computing Jacobian for Newton's method");let p=ne(i,n);ze(p)?(l.info("\u2713 Successfully using Newton's method for Implicit Euler"),h=mt(p,n),u=Oe("x_new",b=>`${b} - pos - h * get_velocity(${b})`,b=>`${o}(1.0) - h * computeJacobian(${b})`,n),d="Newton"):(l.warn("\u2717 Failed to compute Jacobian for Newton's method"),l.warn("Falling back to fixed-point iteration"),u=te("x_new",a,n),d="Fixed-Point")}else t==="newton-fd"?(l.info("\u2713 Using finite difference Newton's method for Implicit Euler"),h=He(n),u=Ge("x_new",c,n),d="Newton (FD)"):t==="midpoint"?(u=Ue("x_new",a,n),d="Midpoint"):(u=te("x_new",a,n),d="Fixed-Point");return{name:`Implicit Euler (${d})`,costFactor:1,code:`
${h}
...
`}}function An(n,e=4,t="fixed-point",i=null){l.info(`*** Implicit Midpoint integrator requested: solutionMethod=${t}, hasExpressions=${!!i}`);let r=`vec${n}`,o=n===2?"mat2":n===3?"mat3":"mat4",a="pos + h * get_velocity(pos + h * 0.5 * get_velocity(pos))",c=m=>`pos + h * get_velocity((pos + ${m}) * 0.5)`,u=m=>`get_velocity((pos + ${m}) * 0.5)`,d,h,p="";if(t==="newton"&&i){l.info("Computing Jacobian for Newton's method (Implicit Midpoint)");let m=ne(i,n);ze(m)?(l.info("\u2713 Successfully using Newton's method for Implicit Midpoint"),p=mt(m,n),d=Oe("x_new",x=>`${x} - pos - h * get_velocity((pos + ${x}) * 0.5)`,x=>`${o}(1.0) - (h * 0.5) * computeJacobian((pos + ${x}) * 0.5)`,n),h="Newton"):(l.warn("\u2717 Failed to compute Jacobian for Newton's method (Implicit Midpoint)"),l.warn("Falling back to fixed-point iteration"),d=te("x_new",c,n),h="Fixed-Point")}else t==="newton-fd"?(l.info("\u2713 Using finite difference Newton's method for Implicit Midpoint"),p=He(n),d=Ge("x_new",u,n,"h * 0.5"),h="Newton (FD)"):t==="midpoint"?(d=Ue("x_new",c,n),h="Midpoint"):(d=te("x_new",c,n),h="Fixed-Point");return{name:`Implicit Midpoint (${h})`,costFactor:2,code:`
${p}
...
`}}function Fn(n,e=4,t="fixed-point",i=null){l.info(`*** Trapezoidal integrator requested: solutionMethod=${t}, hasExpressions=${!!i}`);let r=`vec${n}`,o=n===2?"mat2":n===3?"mat3":"mat4",s="pos + h * f0",a=p=>`pos + h * 0.5 * (f0 + get_velocity(${p}))`,c=p=>`get_velocity(${p})`,u,d,h="";if(t==="newton"&&i){l.info("Computing Jacobian for Newton's method (Trapezoidal)");let p=ne(i,n);ze(p)?(l.info("\u2713 Successfully using Newton's method for Trapezoidal"),h=mt(p,n),u=Oe("x_new",b=>`${b} - pos - h * 0.5 * (f0 + get_velocity(${b}))`,b=>`${o}(1.0) - (h * 0.5) * computeJacobian(${b})`,n),d="Newton"):(l.warn("\u2717 Failed to compute Jacobian for Newton's method (Trapezoidal)"),l.warn("Falling back to fixed-point iteration"),u=te("x_new",a,n),d="Fixed-Point")}else t==="newton-fd"?(l.info("\u2713 Using finite difference Newton's method for Trapezoidal"),h=He(n),u=Ge("x_new",c,n,"h * 0.5","pos + h * 0.5 * (f0 + "+c("x_new")+")"),d="Newton (FD)"):t==="midpoint"?(u=Ue("x_new",a,n),d="Midpoint"):(u=te("x_new",a,n),d="Fixed-Point");return{name:`Trapezoidal (${d})`,costFactor:2,code:`
${h}
...
`}}function Rn(n,e=5,t="fixed-point",i=null){l.info(`*** Implicit RK4 integrator requested: solutionMethod=${t}, hasExpressions=${!!i}`);let r=`vec${n}`,o=n===2?"mat2":n===3?"mat3":"mat4",s=`
    ...
    ${r} k1 = k1_guess;
    ${r} k2 = k2_guess;`,c=g=>`get_velocity(pos + h * (a11 * ${g} + a12 * k2))`,u=g=>`get_velocity(pos + h * (a21 * k1 + a22 * ${g}))`,d=g=>`get_velocity(pos + h * (a11 * ${g} + a12 * k2))`,h=g=>`get_velocity(pos + h * (a21 * k1 + a22 * ${g}))`,p,m,f,b="";if(t==="newton"&&i){l.info("Computing Jacobian for Newton's method (Implicit RK4 - simplified)");let g=ne(i,n);if(ze(g)){l.info("\u2713 Successfully using simplified Newton's method for Implicit RK4"),b=mt(g,n);let y=E=>`${E} - get_velocity(pos + h * (a11 * ${E} + a12 * k2))`,_=E=>`${o}(1.0) - (h * a11) * computeJacobian(pos + h * (a11 * ${E} + a12 * k2))`,S=E=>`${E} - get_velocity(pos + h * (a21 * k1 + a22 * ${E}))`,F=E=>`${o}(1.0) - (h * a22) * computeJacobian(pos + h * (a21 * k1 + a22 * ${E}))`;p=Oe("k1",y,_,n),m=Oe("k2",S,F,n),f="Newton"}else l.warn("\u2717 Failed to compute Jacobian for Newton's method (Implicit RK4)"),l.warn("Falling back to fixed-point iteration"),p=te("k1",c,n),m=te("k2",u,n),f="Fixed-Point"}else t==="newton-fd"?(l.info("\u2713 Using finite difference Newton's method for Implicit RK4 (simplified)"),b=He(n),p=Ge("k1",d,n,"h * a11"),m=Ge("k2",h,n,"h * a22"),f="Newton (FD)"):t==="midpoint"?(p=Ue("k1",c,n),m=Ue("k2",u,n),f="Midpoint"):(p=te("k1",c,n),m=te("k2",u,n),f="Fixed-Point");let x=`
...
`}}function er(n,e,t={}){let i=t.iterations||3,r=t.solutionMethod||"fixed-point",o=t.expressions||null;switch(n){case"euler":return Tn(e);case"explicit-midpoint":return Ki(e);case"heun":return Cn(e);case"rk4":return Zi(e);case"implicit-euler":return Qi(e,i,r,o);case"implicit-midpoint":return An(e,t.iterations||4,r,o);case"trapezoidal":return Fn(e,t.iterations||4,r,o);case"implicit-rk4":return Rn(e,t.iterations||5,r,o);case"rk2":return Ki(e);case"implicit":return Qi(e,i,r,o);default:return Zi(e)}}se();function pt(n,e,t,i=null){i===null&&(i=t>=3?2:-1);let r=i>=0&&i<t;return{name:"Select",params:{dim1:n,dim2:e,depthDim:i},code:`
...
`}}function Ln(n,e){let t=n[0].map((o,s)=>`${o.toFixed(6)} * pos[${s}]`).join(" + "),i=n[1].map((o,s)=>`${o.toFixed(6)} * pos[${s}]`).join(" + "),r=n[2]?n[2].map((o,s)=>`${o.toFixed(6)} * pos[${s}]`).join(" + "):null;return{name:"Linear Projection",params:{matrix:n},code:`
...
`}}function Mn(n,e){let t=n===0?1:0,i=t+1;return i===n&&i++,{name:"Orthographic",params:{axis:n},code:`
...
`}}function Pn(){return{name:"Spherical",params:{},code:`
...
`}}function In(n){return{name:"Stereographic",params:{},code:`
...
`}}function tr(n,e,t={}){switch(n){case"select":{let i=t.dim1!==void 0?t.dim1:0,r=t.dim2!==void 0?t.dim2:Math.min(1,e-1),o=t.depthDim!==void 0?t.depthDim:null;return pt(i,r,e,o)}case"project":{let i=t.matrix||[[1,0,...Array(Math.max(0,e-2)).fill(0)],[0,1,...Array(Math.max(0,e-2)).fill(0)]];return Ln(i,e)}case"orthographic":{let i=t.axis!==void 0?t.axis:e-1;return Mn(i,e)}case"spherical":return e===3?Pn():pt(0,1,e);case"stereographic":return e>=3?In(e):pt(0,1,e);case"custom":{let i=t.horizontalExpr||"x",r=t.verticalExpr||"y",o=t.depthExpr||"";return kn(i,r,o,e)}default:return pt(0,1,e)}}function kn(n,e,t,i){let r=W(n||"x",i),o=W(e||"y",i),s=t?W(t,i):"0.0";return{name:"Custom",params:{horizontalExpr:n,verticalExpr:e,depthExpr:t},code:`
// Custom mapper
...
${t} transform_jacobian(${t} x) {
    return ${t}(1.0);
}`}},Qt=class extends J{constructor(){super("power","Power transform (zoom lens effect)")}generateForward(e){let t=`vec${e}`;return`
${t} transform_forward(${t} x) {
...
`)}
    return result;
}`}getParameters(){return[{name:"alpha",label:"Exponent (\u03B1)",type:"slider",min:1e-4,max:25,step:1e-4,default:.5,info:"\u03B1 < 1.0: zoom into origin; \u03B1 > 1.0: compress origin"}]}},ei=class extends J{constructor(){super("tanh","Hyperbolic tangent (compress infinity)")}generateHelpers(e){return`
...
}`}getParameters(){return[{name:"beta",label:"Compression (\u03B2)",type:"slider",min:1e-4,max:25,step:1e-4,default:1,info:"Higher = more compression. Maps infinite space to [-1,1]"}]}},ti=class extends J{constructor(){super("sigmoid","Logistic Sigmoid (smooth S-curve)")}generateHelpers(e){return`
...
${t} transform_jacobian(${t} x) {
    ...
    `)}
    return result;
}`}getParameters(){return[{name:"k",label:"Steepness (k)",type:"slider",min:1e-4,max:25,step:1e-4,default:1,info:"Higher = steeper transition. Maps infinite space to [-1,1] with logistic curve."}]}},ii=class extends J{constructor(){super("rational","Rational (bell-shaped Jacobian)")}generateForward(e){let t=`vec${e}`;return`
${t} transform_forward(${t} x) {
...
${t} transform_jacobian(${t} x) {
    ...
    `)}
    return result;
}`}getParameters(){return[{name:"a",label:"Width (a)",type:"slider",min:1e-4,max:25,step:1e-4,default:1,info:"Controls bell curve width. Higher = wider/shallower, lower = narrower/sharper."}]}},ri=class extends J{constructor(){super("sine","Sine wave distortion (periodic speed bumps)")}generateForward(e){let t=`vec${e}`;return`
${t} transform_forward(${t} x) {
...
}`}getParameters(){return[{name:"amplitude",label:"Amplitude",type:"slider",min:1e-4,max:25,step:1e-4,default:.5,info:"Strength of distortion"},{name:"frequency",label:"Frequency",type:"slider",min:1e-4,max:25,step:1e-4,default:1,info:"Number of waves per unit length"}]}},ni=class extends J{constructor(){super("radial_power","Radial power (compress/expand by distance)")}generateForward(e){let t=`vec${e}`;return e===2?`
${t} transform_forward(${t} x) {
    float alpha = u_transform_params.x;
    float r = length(x);
    if (r < 1e-8) return x;
...
}`}getParameters(){return[{name:"alpha",label:"Radial Exponent (\u03B1)",type:"slider",min:.1,max:3,step:.1,default:.5,info:"\u03B1 < 1.0: zoom into origin; \u03B1 > 1.0: compress origin"}]}},oi=class extends J{constructor(){super("log","Logarithmic (stretch near zero)")}generateForward(e){let t=`vec${e}`;return`
${t} transform_forward(${t} x) {
    ${t} result;
...
${t} transform_jacobian(${t} x) {
    ...
    `)}
    return result;
}`}getParameters(){return[]}},si=class extends J{constructor(){super("exp","Exponential (compress near zero)")}generateForward(e){let t=`vec${e}`;return`
${t} transform_forward(${t} x) {
    float alpha = u_transform_params.x;
    ${t} result;
...
${t} transform_jacobian(${t} x) {
    ...
    `)}
    return result;
}`}getParameters(){return[{name:"alpha",label:"Growth Rate (\u03B1)",type:"slider",min:.1,max:2,step:.1,default:.5,info:"Higher \u03B1 = stronger exponential growth. Use small values (0.1-0.5) to avoid overflow."}]}},ai=class extends J{constructor(){super("softsign","Soft-sign (smooth compression to [-1,1])")}generateForward(e){let t=`vec${e}`;return`
${t} transform_forward(${t} x) {
    ${t} result;
...
${t} transform_jacobian(${t} x) {
    ...
    `)}
    return result;
}`}getParameters(){return[]}},li=class extends J{constructor(){super("custom","Custom (user-defined GLSL)"),this.forwardCode="",this.inverseCode="",this.jacobianCode=""}setCode(e,t,i){this.forwardCode=e,this.inverseCode=t,this.jacobianCode=i}generateForward(e){return this.forwardCode||new ye().generateForward(e)}generateInverse(e){return this.inverseCode||new ye().generateInverse(e)}generateJacobian(e){return this.jacobianCode||new ye().generateJacobian(e)}},ir={identity:new ye,power:new Qt,log:new oi,exp:new si,softsign:new ai,tanh:new ei,sigmoid:new ti,rational:new ii,sine:new ri,radial_power:new ni,custom:new li};function ft(n){return ir[n]||ir.identity}function rr(n,e){let t=`vec${e}`,r={white:{name:"Solid White",code:`
...
`}}[n];if(!r)throw new Error(`Unknown color mode: ${n}`);return r}function nr(n,e,t){let i=`vec${n}`,r=[],o=["x","y","z","w","u","v"],s=["dx","dy","dz","dw","du","dv"];for(let a=0;a<n;a++)r.push(`float ${o[a]} = pos.${o[a]};`),r.push(`float ${s[a]} = velocity.${o[a]};`);return`
${t}
...
`}function or(n,e,t){let i=`vec${e}`,r;switch(n){case"velocity_magnitude":r=`
...
`}function ci(n){let e=[...n].sort((i,r)=>i.position-r.position);if(e.length<2)throw new Error("Gradient must have at least 2 color stops");e.forEach(i=>{if(i.position<0||i.position>1)throw new Error(`Stop position must be in [0, 1], got ${i.position}`)});let t=`vec3 evaluateGradient(float t) {
    t = clamp(t, 0.0, 1.0);
`;for(let i=0;i<e.length-1;i++){let r=e[i],o=e[i+1],s=sr(r.color),a=sr(o.color),c=r.position.toFixed(6),u=o.position.toFixed(6);i===0?t+=`    if (t <= ${u}) {
`:i===e.length-2?t+=`    } else {
`:t+=`    } else if (t <= ${u}) {
...
`}};M();var gt=class extends we{constructor(e){if(super(e),!e.getExtension("OES_texture_float"))throw new Error("OES_texture_float extension not supported");l.info("Float texture support detected",{extension:"OES_texture_float"})}getName(){return"Float Textures (Direct Storage)"}getTextureFormat(){let e=this.gl;return{internalFormat:e.RGBA,format:e.RGBA,type:e.FLOAT}}getArrayType(){return Float32Array}getComponentsPerValue(){return 4}requiresExtension(){return!0}getExtensionName(){return"OES_texture_float"}encodeValue(e,t,i){let r=new Float32Array(4);return r[0]=e,r[1]=0,r[2]=0,r[3]=1,r}decodeValue(e,t,i){return e[0]}normalizeWorld(e,t,i){return e}denormalizeWorld(e,t,i){return e}getGLSLConstants(){return`
...
`}};var bt=class{constructor(e,t={}){if(this.canvas=e,this.gl=e.getContext("webgl",{alpha:!1,depth:!1,stencil:!1,antialias:!1,preserveDrawingBuffer:!0}),!this.gl)throw new Error("WebGL not supported");let i=this.gl;i.disable(i.DEPTH_TEST),i.disable(i.STENCIL_TEST),i.enable(i.BLEND);let r=t.storageStrategy||"rgba";try{r==="float"?(this.strategy=new gt(i),l.info("Using Float texture strategy")):(this.strategy=new je(i),l.info("Using RGBA encoded texture strategy"))}catch(b){l.warn(`Failed to initialize ${r} strategy: ${b.message}`),l.info("Falling back to RGBA strategy"),this.strategy=new je(i)}this.dimensions=2,this.expressions=["-y","x"],this.coordinateSystem=j(this.dimensions);try{let b=this.coordinateSystem.getVariableNames();this.velocityEvaluators=Kt(this.expressions,b),l.verbose("Initial velocity evaluators created",{coordinateSystem:this.coordinateSystem.name,variables:b})}catch(b){l.warn("Failed to create initial velocity evaluators",b),this.velocityEvaluators=null}this.integratorType="rk4",this.integratorParams={iterations:3},this.integratorCostFactor=1,this.solutionMethod="fixed-point",this.transformType="identity",this.transformParams={},this.mapperType="select",this.mapperParams={dim1:0,dim2:1},this.colorMode="white",this.colorExpression="x * y",this.colorGradient=ve(),this.velocityScaleMode="percentile95",this.velocityLogScale=!1,this.useHDR=t.useHDR!==void 0?t.useHDR:!0,this.tonemapOperator=t.tonemapOperator||"aces",this.exposure=t.exposure!==void 0?t.exposure:1,this.gamma=t.gamma!==void 0?t.gamma:2.2,this.luminanceGamma=t.luminanceGamma!==void 0?t.luminanceGamma:1,this.highlightCompression=t.highlightCompression!==void 0?t.highlightCompression:0,this.compressionThreshold=t.compressionThreshold!==void 0?t.compressionThreshold:1,this.whitePoint=t.whitePoint!==void 0?t.whitePoint:2,this.particleIntensity=t.particleIntensity!==void 0?t.particleIntensity:1,this.particleSize=t.particleSize!==void 0?t.particleSize:1,this.particleRenderMode=t.particleRenderMode||"points",this.useDepthTest=t.useDepthTest!==void 0?t.useDepthTest:!1,this.colorSaturation=t.colorSaturation!==void 0?t.colorSaturation:1,this.brightnessDesaturation=t.brightnessDesaturation!==void 0?t.brightnessDesaturation:0,this.brightnessSaturation=t.brightnessSaturation!==void 0?t.brightnessSaturation:0,this.smaaEnabled=t.smaaEnabled!==void 0?t.smaaEnabled:!0,this.smaaIntensity=t.smaaIntensity!==void 0?t.smaaIntensity:1,this.smaaThreshold=t.smaaThreshold!==void 0?t.smaaThreshold:.1,this.bilateralEnabled=t.bilateralEnabled!==void 0?t.bilateralEnabled:!1,this.bilateralSpatialSigma=t.bilateralSpatialSigma!==void 0?t.bilateralSpatialSigma:4,this.bilateralIntensitySigma=t.bilateralIntensitySigma!==void 0?t.bilateralIntensitySigma:.2;let o=t.supersampleFactor!==void 0?t.supersampleFactor:1;(typeof o!="number"||isNaN(o)||o<=0)&&(l.warn(`Invalid supersampleFactor: ${o}, using default 1.0`),o=1),this.supersampleFactor=Math.max(.5,Math.min(4,o)),l.info(`Initial render scale factor set to: ${this.supersampleFactor}`),this.timestep=.01,this.fadeOpacity=.99,this.dropProbability=.003,this.respawnMargin=.2,this.animationAlpha=0,this.lockShaderRecompilation=!1,this.enableDebugStats=!1,this.maxVelocity=5,this.prevMaxVelocity=5,this.velocityEMAAlpha=.1,this.velocitySampleInterval=10,this.velocitySampleIntervalMin=5,this.velocitySampleIntervalMax=60,this.framesSinceLastSample=0,this.lowVelocityThreshold=.02;let c=10*(e.width/e.height);this.bbox={min:[-c/2,-5],max:[c/2,5]},this.particleSystem=new dt(1e4,this.dimensions,this.bbox,this.strategy),this.textureManager=new De(i,this.dimensions,this.particleSystem.getResolution(),this.strategy),this.particleRenderMode==="lines"&&this.textureManager.setLineMode(!0),this.updateFramebuffer=i.createFramebuffer();let u=Math.max(1,e.width||800),d=Math.max(1,e.height||600);this.renderWidth=Math.max(1,Math.floor(u*this.supersampleFactor)),this.renderHeight=Math.max(1,Math.floor(d*this.supersampleFactor)),l.verbose("Render dimensions calculated",{canvasSize:`${u}x${d}`,renderSize:`${this.renderWidth}x${this.renderHeight}`,supersampleFactor:this.supersampleFactor}),l.info("Initializing HDR rendering system...",{requested:this.useHDR,canvasSize:`${e.width}x${e.height}`,renderSize:`${this.renderWidth}x${this.renderHeight}`,supersampleFactor:this.supersampleFactor}),this.framebufferManager=new Be(i,this.renderWidth,this.renderHeight,{useHDR:this.useHDR,usePingPong:!0});let h=this.framebufferManager.getHDRSupport();l.info("HDR rendering system initialized",{hdrEnabled:h.enabled,hdrSupported:h.supported,textureFormat:h.format,extensions:{floatColorBuffer:h.hasFloatColorBuffer,halfFloatColorBuffer:h.hasHalfFloatColorBuffer,linearFiltering:h.hasLinearFiltering},exposure:this.exposure,gamma:this.gamma}),this.useHDR&&!h.supported&&l.warn("HDR rendering requested but not supported by device. Using LDR fallback."),this.smaaManager=new lt(i,this.renderWidth,this.renderHeight,{enabled:this.smaaEnabled,intensity:this.smaaIntensity,threshold:this.smaaThreshold}),l.info("SMAA system initialized",{enabled:this.smaaManager.isEnabled(),intensity:this.smaaIntensity,threshold:this.smaaThreshold}),this.bilateralManager=new at(i,this.renderWidth,this.renderHeight,{enabled:this.bilateralEnabled,spatialSigma:this.bilateralSpatialSigma,intensitySigma:this.bilateralIntensitySigma}),l.info("Bilateral filter initialized",{enabled:this.bilateralManager.isEnabled(),spatialSigma:this.bilateralSpatialSigma,intensitySigma:this.bilateralIntensitySigma}),this.ldrFramebuffer=i.createFramebuffer(),this.ldrTexture=i.createTexture(),i.bindTexture(i.TEXTURE_2D,this.ldrTexture),i.texParameteri(i.TEXTURE_2D,i.TEXTURE_MIN_FILTER,i.LINEAR),i.texParameteri(i.TEXTURE_2D,i.TEXTURE_MAG_FILTER,i.LINEAR),i.texParameteri(i.TEXTURE_2D,i.TEXTURE_WRAP_S,i.CLAMP_TO_EDGE),i.texParameteri(i.TEXTURE_2D,i.TEXTURE_WRAP_T,i.CLAMP_TO_EDGE),i.texImage2D(i.TEXTURE_2D,0,i.RGBA,this.renderWidth,this.renderHeight,0,i.RGBA,i.UNSIGNED_BYTE,null),i.bindFramebuffer(i.FRAMEBUFFER,this.ldrFramebuffer),i.framebufferTexture2D(i.FRAMEBUFFER,i.COLOR_ATTACHMENT0,i.TEXTURE_2D,this.ldrTexture,0);let p=i.checkFramebufferStatus(i.FRAMEBUFFER);p!==i.FRAMEBUFFER_COMPLETE&&l.error("LDR framebuffer incomplete",{status:p}),i.bindFramebuffer(i.FRAMEBUFFER,null),l.info("LDR framebuffer created for tone mapping output"),this.finalFramebuffer=i.createFramebuffer(),this.finalTexture=i.createTexture(),i.bindTexture(i.TEXTURE_2D,this.finalTexture),i.texParameteri(i.TEXTURE_2D,i.TEXTURE_MIN_FILTER,i.LINEAR),i.texParameteri(i.TEXTURE_2D,i.TEXTURE_MAG_FILTER,i.LINEAR),i.texParameteri(i.TEXTURE_2D,i.TEXTURE_WRAP_S,i.CLAMP_TO_EDGE),i.texParameteri(i.TEXTURE_2D,i.TEXTURE_WRAP_T,i.CLAMP_TO_EDGE),i.texImage2D(i.TEXTURE_2D,0,i.RGBA,this.renderWidth,this.renderHeight,0,i.RGBA,i.UNSIGNED_BYTE,null),i.bindFramebuffer(i.FRAMEBUFFER,this.finalFramebuffer),i.framebufferTexture2D(i.FRAMEBUFFER,i.COLOR_ATTACHMENT0,i.TEXTURE_2D,this.finalTexture,0);let m=i.checkFramebufferStatus(i.FRAMEBUFFER);m!==i.FRAMEBUFFER_COMPLETE&&l.error("Final framebuffer incomplete",{status:m}),i.bindFramebuffer(i.FRAMEBUFFER,null),l.info("Final framebuffer created for supersampling output"),this.bufferStatsManager=new ct(i),this.velocityStatsManager=new ut(i),this.statsUpdateInterval=60,this.statsUpdateIntervalSlow=120,this.statsCoarseMode=!0,this.quadBuffer=this.createBuffer(new Float32Array([0,0,1,0,0,1,0,1,1,0,1,1])),this.smaaManager.setQuadBuffer(this.quadBuffer),this.bilateralManager.setQuadBuffer(this.quadBuffer),l.info("Compiling shaders...",{dimensions:this.dimensions,integrator:this.integratorType});let f=this.compileShaders();if(f)throw l.error("Shader compilation failed",f),new Error(`Shader compilation failed: ${f}`);l.info("Shaders compiled successfully"),l.info("Initializing particle textures...",{resolution:this.particleSystem.getResolution(),particleCount:this.particleSystem.getActualParticleCount()}),this.textureManager.initializeData(this.particleSystem.getAllData()),this.indexBufferPoints=this.createBuffer(this.particleSystem.getIndices()),this.indexBufferLines=this.createLineIndexBuffer(this.particleSystem.getActualParticleCount()),this.vertexIdBufferLines=this.createLineVertexIdBuffer(this.particleSystem.getActualParticleCount()),this.isRunning=!1,this.frame=0,this.hdrLogged=!1}createBuffer(e){let t=this.gl,i=t.createBuffer();return t.bindBuffer(t.ARRAY_BUFFER,i),t.bufferData(t.ARRAY_BUFFER,e,t.STATIC_DRAW),i}createLineIndexBuffer(e){let t=this.gl,i=new Float32Array(e*2);for(let o=0;o<e;o++)i[o*2]=o,i[o*2+1]=o;let r=t.createBuffer();return t.bindBuffer(t.ARRAY_BUFFER,r),t.bufferData(t.ARRAY_BUFFER,i,t.STATIC_DRAW),r}createLineVertexIdBuffer(e){let t=this.gl,i=new Float32Array(e*2);for(let o=0;o<e*2;o++)i[o]=o%2;let r=t.createBuffer();return t.bindBuffer(t.ARRAY_BUFFER,r),t.bufferData(t.ARRAY_BUFFER,i,t.STATIC_DRAW),r}compileShaders(){let e=this.gl;try{let t=this.coordinateSystem.getVariableNames(),i=this.coordinateSystem.name.includes("Cartesian"),r;if(this.velocityGLSL)r=this.velocityGLSL,l.verbose("Using pre-generated velocity GLSL",{glsl:r});else{let E=i?"pos":"pos_native";r=Vi(this.expressions,t,E),l.verbose("Generated velocity GLSL from expressions",{expressions:this.expressions,coordinateSystem:this.coordinateSystem.name,variables:t,posVarName:E,glsl:r})}let o=null;if(!i){let E=["x","y","z","w","u","v"].slice(0,this.dimensions),L=this.coordinateSystem.getVariableNames();o={name:this.coordinateSystem.name,nativeVars:L,forwardTransform:this.coordinateSystem.generateForwardTransformGLSL(E,(D,A)=>W(D,A.length,A)),inverseTransform:this.coordinateSystem.generateInverseTransformGLSL(L,(D,A)=>W(D,A.length,A))},l.verbose("Generated coordinate system code",{name:o.name,forward:o.forwardTransform,inverse:o.inverseTransform})}let s={...this.integratorParams,expressions:this.expressions,solutionMethod:this.solutionMethod},a=er(this.integratorType,this.dimensions,s);this.integratorCostFactor=a.costFactor||1;let c=tr(this.mapperType,this.dimensions,this.mapperParams),u=null;if(this.transformType!=="identity"){let E=ft(this.transformType);u={helpers:E.generateHelpers(this.dimensions),forward:E.generateForward(this.dimensions),inverse:E.generateInverse(this.dimensions),jacobian:E.generateJacobian(this.dimensions)}}let d=rr(this.colorMode,this.dimensions),h=d.code,p=d.usesMaxVelocity||!1;if(this.colorMode==="expression"){let E=W(this.colorExpression,this.dimensions),L=ci(this.colorGradient);h=nr(this.dimensions,E,L)}else if(d.usesGradient){let E=ci(this.colorGradient);h=or(this.colorMode,this.dimensions,E)}let m=Gi(),f=Hi(this.dimensions,r,a.code,this.strategy,u,o);this.updateProgram=K(e,m,f);let b=this.particleRenderMode==="lines",x=ji(this.dimensions,c.code,r,this.strategy,b,o),g=Xi(this.dimensions,h,p);this.drawProgram=K(e,x,g),this.usesMaxVelocity=p,this.shaderSource={updateVertex:m,updateFragment:f,drawVertex:x,drawFragment:g,velocityField:r},this.dumpShadersOnCompile&&(console.log(`
...
=================================
`),this.dumpShadersOnCompile=!1),this.velocityStatsManager&&(this.velocityStatsManager.dispose(),this.velocityStatsManager.initialize(this.dimensions,r,o)?l.verbose("Velocity stats manager initialized"):l.warn("Failed to initialize velocity stats manager"));let y=qi(),_=Wi(),S=lr(this.tonemapOperator,{exposure:this.exposure,gamma:this.gamma,luminanceGamma:this.luminanceGamma,whitePoint:this.whitePoint}),F=Ji(S);return this.screenFadeProgram=K(e,y,_),this.tonemapProgram=K(e,y,F),this.shaderSource.screenFade=_,this.shaderSource.tonemap=F,l.verbose("Tone mapping operator compiled",{operator:this.tonemapOperator,exposure:this.exposure,gamma:this.gamma,whitePoint:this.whitePoint}),this.shadersJustRecompiled=!0,null}catch(t){return t.message}}updatePositions(){let e=this.gl,t=this.updateProgram;this.enableDebugStats&&this.frame%60===0&&this.sampleParticleData(),this.frame%600===0&&l.verbose(`Frame ${this.frame}: Shader uniforms`,{u_min:[this.bbox.min[0],this.bbox.min[1]],u_max:[this.bbox.max[0],this.bbox.max[1]],u_h:this.timestep*(this.integratorCostFactor||1),timestep_base:this.timestep,costFactor:this.integratorCostFactor||1,u_drop_rate:this.dropProbability}),e.useProgram(t);let i=e.getAttribLocation(t,"a_pos");e.bindBuffer(e.ARRAY_BUFFER,this.quadBuffer),e.enableVertexAttribArray(i),e.vertexAttribPointer(i,2,e.FLOAT,!1,0,0),this.textureManager.bindReadTextures(t);let r=this.particleSystem.getResolution(),o=Math.random();e.uniform2f(e.getUniformLocation(t,"u_min"),this.bbox.min[0],this.bbox.min[1]),e.uniform2f(e.getUniformLocation(t,"u_max"),this.bbox.max[0],this.bbox.max[1]),e.uniform1f(e.getUniformLocation(t,"u_h"),this.timestep*(this.integratorCostFactor||1)),e.uniform1f(e.getUniformLocation(t,"u_rand_seed"),o),e.uniform1f(e.getUniformLocation(t,"u_drop_rate"),this.dropProbability),e.uniform1f(e.getUniformLocation(t,"u_respawn_margin"),this.respawnMargin),e.uniform1f(e.getUniformLocation(t,"u_particles_res"),r);let s=this.getVelocityScale();e.uniform1f(e.getUniformLocation(t,"u_max_velocity"),s);let a=e.getUniformLocation(t,"u_alpha");if(a!==null&&e.uniform1f(a,this.animationAlpha),this.transformType!=="identity"){let c=this.transformParams||{},u=[c.alpha||c.beta||c.k||c.amplitude||.5,c.frequency||0,0,0];e.uniform4f(e.getUniformLocation(t,"u_transform_params"),u[0],u[1],u[2],u[3])}for(let c=0;c<this.dimensions;c++){e.uniform1i(e.getUniformLocation(t,"u_out_coordinate"),c);let u=this.textureManager.getWriteTexture(c);e.bindFramebuffer(e.FRAMEBUFFER,this.updateFramebuffer),e.framebufferTexture2D(e.FRAMEBUFFER,e.COLOR_ATTACHMENT0,e.TEXTURE_2D,u,0),e.viewport(0,0,r,r),e.drawArrays(e.TRIANGLES,0,6)}this.textureManager.swap(),this.frame++}drawParticles(){let e=this.gl,t=this.drawProgram;if(e.useProgram(t),e.blendFunc(e.SRC_ALPHA,e.ONE),this.framesSinceLastSample++,this.framesSinceLastSample>=this.velocitySampleInterval){if(this.framesSinceLastSample=0,this.velocityStatsManager&&this.velocityStatsManager.initialized){let u=this.textureManager.getReadTextures(),d=this.velocityStatsManager.compute(u,this.bbox,this.particleSystem.getResolution(),this.animationAlpha);if(this.frame%600===0&&d.sampleCount>0&&l.debug(`GPU velocity stats: avg=${d.avgVelocity.toFixed(3)}, max=${d.maxVelocity.toFixed(3)} (${d.sampleCount} samples)`),d.maxVelocity>0){let h=d.maxVelocity>this.maxVelocity?this.velocityEMAAlpha*2:this.velocityEMAAlpha;this.prevMaxVelocity=this.maxVelocity,this.maxVelocity=h*d.maxVelocity+(1-h)*this.maxVelocity}}else this.velocityStatsWarningLogged||(l.warn("Velocity stats manager not initialized - max velocity will not be computed"),this.velocityStatsWarningLogged=!0);let s=Math.abs(this.maxVelocity-this.prevMaxVelocity)/Math.max(this.maxVelocity,.1),a=.05,c=.01;if(s>a)this.velocitySampleInterval=this.velocitySampleIntervalMin;else if(s<c)this.velocitySampleInterval=this.velocitySampleIntervalMax;else{let u=(s-c)/(a-c);this.velocitySampleInterval=Math.round(this.velocitySampleIntervalMax*(1-u)+this.velocitySampleIntervalMin*u)}}this.maxVelocity=Math.max(this.maxVelocity,.5);let i=e.getAttribLocation(t,"a_index"),r=this.particleRenderMode==="lines"?this.indexBufferLines:this.indexBufferPoints;if(e.bindBuffer(e.ARRAY_BUFFER,r),e.enableVertexAttribArray(i),e.vertexAttribPointer(i,1,e.FLOAT,!1,0,0),this.particleRenderMode==="lines"){let s=e.getAttribLocation(t,"a_vertex_id");s!==-1&&(e.bindBuffer(e.ARRAY_BUFFER,this.vertexIdBufferLines),e.enableVertexAttribArray(s),e.vertexAttribPointer(s,1,e.FLOAT,!1,0,0))}this.textureManager.bindReadTextures(t),this.textureManager.bindPrevTextures(t),e.uniform1f(e.getUniformLocation(t,"u_particles_res"),this.particleSystem.getResolution()),e.uniform2f(e.getUniformLocation(t,"u_min"),this.bbox.min[0],this.bbox.min[1]),e.uniform2f(e.getUniformLocation(t,"u_max"),this.bbox.max[0],this.bbox.max[1]),e.uniform1f(e.getUniformLocation(t,"u_particle_intensity"),this.particleIntensity),e.uniform1f(e.getUniformLocation(t,"u_particle_size"),this.particleSize),e.uniform2f(e.getUniformLocation(t,"u_viewport_size"),this.renderWidth,this.renderHeight),e.uniform2f(e.getUniformLocation(t,"u_canvas_size"),this.canvas.width,this.canvas.height),e.uniform1f(e.getUniformLocation(t,"u_color_saturation"),this.colorSaturation);let o=e.getUniformLocation(t,"u_alpha");if(o!==null&&e.uniform1f(o,this.animationAlpha),this.usesMaxVelocity){let s=this.getVelocityScale();e.uniform1f(e.getUniformLocation(t,"u_max_velocity"),s),e.uniform1f(e.getUniformLocation(t,"u_velocity_log_scale"),this.velocityLogScale?1:0)}this.particleRenderMode==="lines"?e.drawArrays(e.LINES,0,this.particleSystem.getActualParticleCount()*2):e.drawArrays(e.POINTS,0,this.particleSystem.getActualParticleCount())}getVelocityScale(){if(!this.velocityStatsManager||!this.velocityStatsManager.initialized)return this.maxVelocity;let e=this.velocityStatsManager.getCached();switch(this.velocityScaleMode){case"max":return e.maxVelocity||this.maxVelocity;case"average":return e.avgVelocity||this.maxVelocity;case"percentile90":return e.percentile90||e.avgVelocity||this.maxVelocity;case"percentile95":return e.percentile95||e.avgVelocity||this.maxVelocity;default:return this.maxVelocity}}fadeScreen(){let e=this.gl,t=this.framebufferManager.getCurrentTexture();this.framebufferManager.swap(),this.framebufferManager.bind(),e.viewport(0,0,this.renderWidth,this.renderHeight),e.disable(e.BLEND),e.useProgram(this.screenFadeProgram);let i=e.getAttribLocation(this.screenFadeProgram,"a_pos");e.bindBuffer(e.ARRAY_BUFFER,this.quadBuffer),e.enableVertexAttribArray(i),e.vertexAttribPointer(i,2,e.FLOAT,!1,0,0),e.activeTexture(e.TEXTURE0),e.bindTexture(e.TEXTURE_2D,t),e.uniform1i(e.getUniformLocation(this.screenFadeProgram,"u_screen"),0),e.uniform1f(e.getUniformLocation(this.screenFadeProgram,"u_fade"),this.fadeOpacity),e.drawArrays(e.TRIANGLES,0,6),e.enable(e.BLEND)}downsampleToCanvas(){let e=this.gl;e.bindFramebuffer(e.FRAMEBUFFER,null),e.viewport(0,0,this.canvas.width,this.canvas.height),e.useProgram(this.screenFadeProgram);let t=e.getAttribLocation(this.screenFadeProgram,"a_pos");e.bindBuffer(e.ARRAY_BUFFER,this.quadBuffer),e.enableVertexAttribArray(t),e.vertexAttribPointer(t,2,e.FLOAT,!1,0,0),e.activeTexture(e.TEXTURE0),e.bindTexture(e.TEXTURE_2D,this.finalTexture),e.uniform1i(e.getUniformLocation(this.screenFadeProgram,"u_screen"),0),e.uniform1f(e.getUniformLocation(this.screenFadeProgram,"u_fade"),1),e.drawArrays(e.TRIANGLES,0,6)}render(e=!0){let t=this.gl;if(!this.hdrLogged){let a=this.framebufferManager.getHDRSupport();l.info("First frame render",{renderingMode:a.enabled?"HDR":"LDR",textureFormat:a.format,exposure:this.exposure,gamma:this.gamma}),this.hdrLogged=!0}this.updatePositions(),this.fadeScreen(),this.framebufferManager.bind(),this.useDepthTest&&(t.enable(t.DEPTH_TEST),t.depthFunc(t.LESS),t.clear(t.DEPTH_BUFFER_BIT)),this.drawParticles(),this.useDepthTest&&t.disable(t.DEPTH_TEST),t.bindFramebuffer(t.FRAMEBUFFER,this.ldrFramebuffer),t.viewport(0,0,this.renderWidth,this.renderHeight),t.disable(t.BLEND),t.useProgram(this.tonemapProgram);let i=t.getAttribLocation(this.tonemapProgram,"a_pos");t.bindBuffer(t.ARRAY_BUFFER,this.quadBuffer),t.enableVertexAttribArray(i),t.vertexAttribPointer(i,2,t.FLOAT,!1,0,0),t.activeTexture(t.TEXTURE0),t.bindTexture(t.TEXTURE_2D,this.framebufferManager.getCurrentTexture()),t.uniform1i(t.getUniformLocation(this.tonemapProgram,"u_screen"),0);let r=this.bufferStatsManager.getCached();t.uniform1f(t.getUniformLocation(this.tonemapProgram,"u_exposure"),this.exposure),t.uniform1f(t.getUniformLocation(this.tonemapProgram,"u_gamma"),this.gamma),t.uniform1f(t.getUniformLocation(this.tonemapProgram,"u_whitePoint"),this.whitePoint),t.uniform1f(t.getUniformLocation(this.tonemapProgram,"u_highlight_compression"),this.highlightCompression),t.uniform1f(t.getUniformLocation(this.tonemapProgram,"u_compression_threshold"),this.compressionThreshold),t.uniform1f(t.getUniformLocation(this.tonemapProgram,"u_brightness_desat"),this.brightnessDesaturation),t.uniform1f(t.getUniformLocation(this.tonemapProgram,"u_brightness_sat"),this.brightnessSaturation),t.uniform1f(t.getUniformLocation(this.tonemapProgram,"u_hdr_max_brightness"),r.maxBrightness),t.uniform1f(t.getUniformLocation(this.tonemapProgram,"u_hdr_avg_brightness"),r.avgBrightness),t.drawArrays(t.TRIANGLES,0,6);let o=this.bilateralManager.apply(this.ldrTexture);this.smaaManager.applyToFramebuffer(o,this.finalFramebuffer,this.quadBuffer),e&&this.downsampleToCanvas(),this.frame||(this.frame=0),this.frame++;let s=this.isRunning?this.statsUpdateInterval:this.statsUpdateIntervalSlow;if(this.frame%s===0){let a=this.framebufferManager.getCurrentTexture(),c=this.bufferStatsManager.compute(a,this.renderWidth,this.renderHeight,!0);this.enableDebugStats&&this.frame%(s*10)===0&&l.info("HDR Buffer Stats (sampled)",{avgBrightness:c.avgBrightness.toFixed(2),maxBrightness:c.maxBrightness.toFixed(2),sampledPixels:c.sampledPixels,sampleRate:c.sampleRate})}}getBufferStats(){let e=this.bufferStatsManager.getCached(),t=this.velocityStatsManager?this.velocityStatsManager.getCached():null;return e.maxVelocity=this.maxVelocity,e.avgVelocity=t?t.avgVelocity:0,e.velocitySampleCount=t?t.sampleCount:0,e}calculateParticleStatistics_REMOVED(){let e=this.particleSystem.getResolution(),t=[];for(let c=0;c<this.dimensions;c++)t.push(this.textureManager.readTexture(c));let i=[],r=[],o=this.strategy.getComponentsPerValue(),s=this.strategy.getArrayType(),a=[];for(let c=0;c<this.particleSystem.getActualParticleCount();c++){let u=[];for(let d=0;d<this.dimensions;d++){let h=t[d],p=c*o,m=new s(o);for(let g=0;g<o;g++)m[g]=h[p+g];let f=d===0?this.bbox.min[0]:d===1?this.bbox.min[1]:-10,b=d===0?this.bbox.max[0]:d===1?this.bbox.max[1]:10,x=this.strategy.decodeValue(m,f,b);u.push(x)}a.push(u)}for(let c=0;c<this.dimensions;c++){let u=1/0,d=-1/0,h=0;for(let m of a)u=Math.min(u,m[c]),d=Math.max(d,m[c]),h+=m[c];let p=["x","y","z","w"][c]||`dim${c}`;i.push({dim:p,min:u.toFixed(3),max:d.toFixed(3),avg:(h/a.length).toFixed(3)})}if(this.velocityEvaluators)for(let c=0;c<this.dimensions;c++){let u=1/0,d=-1/0,h=0;try{for(let m of a){let f=this.velocityEvaluators[c](...m);u=Math.min(u,f),d=Math.max(d,f),h+=f}let p=["x","y","z","w"][c]||`dim${c}`;r.push({dim:`v${p}`,min:u.toFixed(3),max:d.toFixed(3),avg:(h/a.length).toFixed(3)})}catch(p){l.warn(`Error computing velocity for dim ${c}`,p)}}l.verbose(`Frame ${this.frame}: Particle statistics`,{bounds:`[${this.bbox.min[0].toFixed(3)}, ${this.bbox.min[1].toFixed(3)}] to [${this.bbox.max[0].toFixed(3)}, ${this.bbox.max[1].toFixed(3)}]`,position:i,velocity:r.length>0?r:"N/A"})}sampleParticleVelocity(){let e=this.gl,t=[];for(let c=0;c<this.dimensions;c++)t.push(this.textureManager.readTexture(c));let i=this.strategy.getComponentsPerValue(),r=this.strategy.getArrayType(),s=Math.floor(Math.random()*this.particleSystem.getActualParticleCount())*i,a=[];for(let c=0;c<this.dimensions;c++){let u=t[c],d=new r(i);for(let f=0;f<i;f++)d[f]=u[s+f];let h=c===0?this.bbox.min[0]:c===1?this.bbox.min[1]:-10,p=c===0?this.bbox.max[0]:c===1?this.bbox.max[1]:10,m=this.strategy.decodeValue(d,h,p);a.push(m)}if(!this.velocityEvaluators||this.velocityEvaluators.length===0)return 0;try{let c=[];for(let h=0;h<this.dimensions;h++){let p=this.velocityEvaluators[h](...a);c.push(p)}let u=c.reduce((h,p)=>h+p*p,0),d=Math.sqrt(u);return isFinite(d)?d:(this.velocityNaNWarningLogged||(l.warn("Invalid velocity magnitude detected",{position:a,velocity:c,magnitude:d}),this.velocityNaNWarningLogged=!0),0)}catch(c){return this.velocityErrorLogged||(l.error("Error computing velocity",c),this.velocityErrorLogged=!0),0}}sampleParticleData(){let t=this.particleSystem.getResolution(),i=[],r=[];for(let a=0;a<this.dimensions;a++)r.push(this.textureManager.readTexture(a));let o=this.strategy.getComponentsPerValue(),s=this.strategy.getArrayType();for(let a=0;a<5;a++){let c=Math.floor(Math.random()*this.particleSystem.getActualParticleCount()),u=c*o,d=[];for(let m=0;m<this.dimensions;m++){let f=r[m],b=new s(o);for(let _=0;_<o;_++)b[_]=f[u+_];let x=m===0?this.bbox.min[0]:m===1?this.bbox.min[1]:-10,g=m===0?this.bbox.max[0]:m===1?this.bbox.max[1]:10,y=this.strategy.decodeValue(b,x,g);d.push(y.toFixed(3))}let h=r[0],p=o===4?h[u+3]:1;i.push({particle:c,position:d.join(", "),age:p.toFixed(2)})}l.verbose(`Frame ${this.frame}: Sampled 5 particles (with age)`,{bounds:`[${this.bbox.min[0].toFixed(3)}, ${this.bbox.min[1].toFixed(3)}] to [${this.bbox.max[0].toFixed(3)}, ${this.bbox.max[1].toFixed(3)}]`,samples:i})}decodeFloatRGBA(e,t,i,r){return((e<<24|t<<16|i<<8|r)>>>0)/4294967295}start(){this.isRunning=!0,this.frameCount=0,this.totalFrames=0,this.lastFpsUpdate=performance.now(),this.fps=0,this.frameLimitEnabled===void 0&&(this.frameLimitEnabled=!1),this.frameLimit===void 0&&(this.frameLimit=1e3),l.verbose(`Starting render loop (frameLimitEnabled: ${this.frameLimitEnabled}, frameLimit: ${this.frameLimit})`);let e=()=>{if(!this.isRunning)return;this.frameCount++,this.totalFrames++;let t=performance.now(),i=t-this.lastFpsUpdate;if(i>=500&&(this.fps=Math.round(this.frameCount*1e3/i),this.frameCount=0,this.lastFpsUpdate=t),this.render(),this.frameLimitEnabled&&this.totalFrames>=this.frameLimit){let o=performance.now()-this.lastFpsUpdate;o>0&&(this.fps=Math.round(this.frameCount*1e3/o)),l.info(`Frame limit reached (${this.frameLimit} frames), stopping render loop`),this.stop();return}requestAnimationFrame(e)};e()}stop(){this.isRunning=!1}setAnimationAlpha(e){this.animationAlpha=Math.max(0,Math.min(1,e))}clearRenderBuffer(e=!1){this.framebufferManager.clearAll(0,0,0,1),this.totalFrames=0,!this.isRunning&&!e&&(l.info("Restarting render loop after render buffer clear"),this.start()),l.verbose("Render buffer cleared (particles preserved)")}resetParticles(e=!1){if(this.particleSystem.initializeParticles(),this.textureManager.initializeData(this.particleSystem.getAllData()),l.verbose("Particles reinitialized to random positions"),e){l.info("Recompiling shaders after particle reset...");try{this.compileShaders(),l.info("Shaders recompiled successfully")}catch(t){l.error("Failed to recompile shaders:",t)}}}step(e=1){for(let t=0;t<e;t++)this.updatePositions()}clearScreen(){this.framebufferManager.clearAll(0,0,0,1),this.particleSystem.initializeParticles(),this.textureManager.initializeData(this.particleSystem.getAllData()),this.totalFrames=0,this.isRunning||(l.info("Restarting render loop after screen clear"),this.start()),l.verbose("Screen cleared and particles reinitialized")}captureRenderBuffer(){return new Promise((e,t)=>{let i=this.gl;try{i.bindFramebuffer(i.FRAMEBUFFER,this.finalFramebuffer);let r=new Uint8Array(this.renderWidth*this.renderHeight*4);i.readPixels(0,0,this.renderWidth,this.renderHeight,i.RGBA,i.UNSIGNED_BYTE,r),i.bindFramebuffer(i.FRAMEBUFFER,null);let o=document.createElement("canvas");o.width=this.renderWidth,o.height=this.renderHeight;let s=o.getContext("2d"),a=s.createImageData(this.renderWidth,this.renderHeight);a.data.set(r),s.save(),s.scale(1,-1),s.translate(0,-this.renderHeight),s.putImageData(a,0,0),s.restore(),o.toBlob(c=>{c?e(c):t(new Error("Failed to create blob from canvas"))},"image/png")}catch(r){l.error("Failed to capture render buffer:",r),t(r)}})}resize(e,t){let i=this.gl;if(!e||!t||e<=0||t<=0){l.warn(`Invalid resize dimensions: ${e}x${t}, skipping resize`);return}this.canvas.width=e,this.canvas.height=t,this.renderWidth=Math.max(1,Math.floor(e*this.supersampleFactor)),this.renderHeight=Math.max(1,Math.floor(t*this.supersampleFactor)),l.info("Resizing renderer",{canvasSize:`${e}x${t}`,renderSize:`${this.renderWidth}x${this.renderHeight}`,supersampleFactor:this.supersampleFactor}),this.framebufferManager.resize(this.renderWidth,this.renderHeight),this.bilateralManager.resize(this.renderWidth,this.renderHeight),this.smaaManager.resize(this.renderWidth,this.renderHeight),i.bindTexture(i.TEXTURE_2D,this.ldrTexture),i.texImage2D(i.TEXTURE_2D,0,i.RGBA,this.renderWidth,this.renderHeight,0,i.RGBA,i.UNSIGNED_BYTE,null),i.bindTexture(i.TEXTURE_2D,null),i.bindTexture(i.TEXTURE_2D,this.finalTexture),i.texImage2D(i.TEXTURE_2D,0,i.RGBA,this.renderWidth,this.renderHeight,0,i.RGBA,i.UNSIGNED_BYTE,null),i.bindTexture(i.TEXTURE_2D,null),this.gl.viewport(0,0,e,t),this.clearScreen()}updateConfig(e){l.info("Updating configuration",e);let t=!1;if(e.dimensions!==void 0&&e.dimensions!==this.dimensions&&(l.info(`Changing dimensions: ${this.dimensions} \u2192 ${e.dimensions}`),this.dimensions=e.dimensions,t=!0,this.coordinateSystem=j(this.dimensions),l.info(`Updated coordinate system to ${this.dimensions}D Cartesian`),this.particleSystem.setDimensions(this.dimensions),this.textureManager.dispose(),this.textureManager=new De(this.gl,this.dimensions,this.particleSystem.getResolution(),this.strategy),this.particleRenderMode==="lines"&&this.textureManager.setLineMode(!0),this.textureManager.initializeData(this.particleSystem.getAllData())),e.coordinateSystem!==void 0&&(l.info(`Updating coordinate system: ${this.coordinateSystem.name} \u2192 ${e.coordinateSystem.name}`),this.coordinateSystem=e.coordinateSystem,t=!0,l.info("  needsRecompile flag set to true (coordinate system change)")),e.expressions!==void 0){l.info("Updating vector field expressions",e.expressions),this.expressions=e.expressions;try{let i;this.coordinateSystem&&typeof this.coordinateSystem.getVariableNames=="function"?i=this.coordinateSystem.getVariableNames():(l.warn("coordinateSystem missing getVariableNames(), using Cartesian fallback"),i=["x","y","z","w","u","v"].slice(0,this.dimensions)),this.velocityEvaluators=Kt(this.expressions,i),l.verbose("Velocity evaluators created successfully",{coordinateSystem:this.coordinateSystem?this.coordinateSystem.name:"unknown",variables:i})}catch(i){l.warn("Failed to create velocity evaluators",i),this.velocityEvaluators=null}t=!0,l.info("  needsRecompile flag set to true (expressions change)")}if(e.velocityGLSL!==void 0&&(l.info("Updating pre-generated velocity GLSL",e.velocityGLSL),this.velocityGLSL=e.velocityGLSL,t=!0,l.info("  needsRecompile flag set to true (velocityGLSL change)")),e.integratorType!==void 0&&e.integratorType!==this.integratorType&&(l.info(`Changing integrator: ${this.integratorType} \u2192 ${e.integratorType}`),this.integratorType=e.integratorType,t=!0),e.integratorParams!==void 0&&(l.verbose("Updating integrator parameters",e.integratorParams),this.integratorParams={...this.integratorParams,...e.integratorParams},t=!0),e.solutionMethod!==void 0&&e.solutionMethod!==this.solutionMethod&&(l.info(`Changing solution method: ${this.solutionMethod} \u2192 ${e.solutionMethod}`),this.solutionMethod=e.solutionMethod,t=!0,l.info("  needsRecompile flag set to true (solutionMethod change)")),l.info("Solution method update complete, continuing..."),e.transformType!==void 0&&e.transformType!==this.transformType&&(l.info(`Changing transform: ${this.transformType} \u2192 ${e.transformType}`),this.transformType=e.transformType,t=!0),e.transformParams!==void 0&&(l.verbose("Updating transform parameters",e.transformParams),this.transformParams=e.transformParams,t=!0),e.mapperType!==void 0&&e.mapperType!==this.mapperType&&(l.info(`Changing mapper: ${this.mapperType} \u2192 ${e.mapperType}`),this.mapperType=e.mapperType,t=!0),e.mapperParams!==void 0&&(l.verbose("Updating mapper parameters",e.mapperParams),this.mapperParams=e.mapperParams,t=!0),e.colorMode!==void 0&&e.colorMode!==this.colorMode&&(l.info(`Changing color mode: ${this.colorMode} \u2192 ${e.colorMode}`),this.colorMode=e.colorMode,t=!0),e.colorExpression!==void 0&&e.colorExpression!==this.colorExpression&&(l.info(`Changing color expression: ${this.colorExpression} \u2192 ${e.colorExpression}`),this.colorExpression=e.colorExpression,t=!0),e.colorGradient!==void 0&&JSON.stringify(e.colorGradient)!==JSON.stringify(this.colorGradient)&&(l.info("Changing color gradient"),this.colorGradient=e.colorGradient,t=!0),e.velocityScaleMode!==void 0&&e.velocityScaleMode!==this.velocityScaleMode&&(l.info(`Changing velocity scale mode: ${this.velocityScaleMode} \u2192 ${e.velocityScaleMode}`),this.velocityScaleMode=e.velocityScaleMode),e.velocityLogScale!==void 0&&e.velocityLogScale!==this.velocityLogScale&&(l.info(`Changing velocity log scale: ${this.velocityLogScale} \u2192 ${e.velocityLogScale}`),this.velocityLogScale=e.velocityLogScale,t=!0),e.timestep!==void 0&&(l.verbose(`Timestep: ${this.timestep} \u2192 ${e.timestep}`),this.timestep=e.timestep),e.fadeOpacity!==void 0&&(l.verbose(`Fade opacity: ${this.fadeOpacity} \u2192 ${e.fadeOpacity}`),this.fadeOpacity=e.fadeOpacity),e.dropProbability!==void 0&&(l.verbose(`Drop probability: ${this.dropProbability} \u2192 ${e.dropProbability}`),this.dropProbability=e.dropProbability),e.respawnMargin!==void 0&&(l.verbose(`Respawn margin: ${this.respawnMargin} \u2192 ${e.respawnMargin}`),this.respawnMargin=e.respawnMargin),e.lockShaderRecompilation!==void 0&&(l.info(`Shader recompilation lock: ${this.lockShaderRecompilation} \u2192 ${e.lockShaderRecompilation}`),this.lockShaderRecompilation=e.lockShaderRecompilation),e.useHDR!==void 0&&e.useHDR!==this.useHDR){l.info(`Changing HDR rendering: ${this.useHDR} \u2192 ${e.useHDR}`),this.useHDR=e.useHDR,this.framebufferManager.dispose(),this.framebufferManager=new Be(this.gl,this.canvas.width,this.canvas.height,{useHDR:this.useHDR,usePingPong:!0});let i=this.framebufferManager.getHDRSupport();l.info("HDR rendering system reconfigured",{hdrEnabled:i.enabled,hdrSupported:i.supported,textureFormat:i.format}),this.useHDR&&!i.supported&&l.warn("HDR requested but not supported. Falling back to LDR."),this.clearScreen()}if(e.exposure!==void 0&&(l.verbose(`Exposure: ${this.exposure} \u2192 ${e.exposure}`),this.exposure=e.exposure,t=!0),e.gamma!==void 0&&(l.verbose(`Gamma: ${this.gamma} \u2192 ${e.gamma}`),this.gamma=e.gamma,t=!0),e.luminanceGamma!==void 0&&(l.verbose(`Luminance Gamma: ${this.luminanceGamma} \u2192 ${e.luminanceGamma}`),this.luminanceGamma=e.luminanceGamma,t=!0),e.highlightCompression!==void 0&&(l.verbose(`Highlight Compression: ${this.highlightCompression} \u2192 ${e.highlightCompression}`),this.highlightCompression=e.highlightCompression),e.compressionThreshold!==void 0&&(l.verbose(`Compression Threshold: ${this.compressionThreshold} \u2192 ${e.compressionThreshold}`),this.compressionThreshold=e.compressionThreshold),e.whitePoint!==void 0&&(l.verbose(`White point: ${this.whitePoint} \u2192 ${e.whitePoint}`),this.whitePoint=e.whitePoint,t=!0),e.tonemapOperator!==void 0&&e.tonemapOperator!==this.tonemapOperator&&(l.info(`Tone mapping operator: ${this.tonemapOperator} \u2192 ${e.tonemapOperator}`),this.tonemapOperator=e.tonemapOperator,t=!0),e.particleIntensity!==void 0&&(l.verbose(`Particle intensity: ${this.particleIntensity} \u2192 ${e.particleIntensity}`),this.particleIntensity=e.particleIntensity),e.particleSize!==void 0&&(l.verbose(`Particle size: ${this.particleSize} \u2192 ${e.particleSize}`),this.particleSize=e.particleSize),e.particleRenderMode!==void 0&&e.particleRenderMode!==this.particleRenderMode){l.info(`Particle render mode: ${this.particleRenderMode} \u2192 ${e.particleRenderMode}`),this.particleRenderMode=e.particleRenderMode;let i=this.particleRenderMode==="lines";this.textureManager.setLineMode(i),t=!0}if(e.colorSaturation!==void 0&&(l.verbose(`Color saturation: ${this.colorSaturation} \u2192 ${e.colorSaturation}`),this.colorSaturation=e.colorSaturation),e.brightnessDesaturation!==void 0&&(l.verbose(`Brightness desaturation: ${this.brightnessDesaturation} \u2192 ${e.brightnessDesaturation}`),this.brightnessDesaturation=e.brightnessDesaturation),e.brightnessSaturation!==void 0&&(l.verbose(`Brightness saturation: ${this.brightnessSaturation} \u2192 ${e.brightnessSaturation}`),this.brightnessSaturation=e.brightnessSaturation),e.useDepthTest!==void 0&&(l.verbose(`Depth testing: ${this.useDepthTest} \u2192 ${e.useDepthTest}`),this.useDepthTest=e.useDepthTest),e.smaaEnabled!==void 0&&(l.verbose(`SMAA enabled: ${this.smaaEnabled} \u2192 ${e.smaaEnabled}`),this.smaaEnabled=e.smaaEnabled,this.smaaManager.updateConfig({enabled:e.smaaEnabled})),e.smaaIntensity!==void 0&&(l.verbose(`SMAA intensity: ${this.smaaIntensity} \u2192 ${e.smaaIntensity}`),this.smaaIntensity=e.smaaIntensity,this.smaaManager.updateConfig({intensity:e.smaaIntensity})),e.smaaThreshold!==void 0&&(l.verbose(`SMAA threshold: ${this.smaaThreshold} \u2192 ${e.smaaThreshold}`),this.smaaThreshold=e.smaaThreshold,this.smaaManager.updateConfig({threshold:e.smaaThreshold})),e.bilateralEnabled!==void 0&&(l.verbose(`Bilateral filter enabled: ${this.bilateralEnabled} \u2192 ${e.bilateralEnabled}`),this.bilateralEnabled=e.bilateralEnabled,this.bilateralManager.updateConfig({enabled:e.bilateralEnabled})),e.bilateralSpatialSigma!==void 0&&(l.verbose(`Bilateral spatial sigma: ${this.bilateralSpatialSigma} \u2192 ${e.bilateralSpatialSigma}`),this.bilateralSpatialSigma=e.bilateralSpatialSigma,this.bilateralManager.updateConfig({spatialSigma:e.bilateralSpatialSigma})),e.bilateralIntensitySigma!==void 0&&(l.verbose(`Bilateral intensity sigma: ${this.bilateralIntensitySigma} \u2192 ${e.bilateralIntensitySigma}`),this.bilateralIntensitySigma=e.bilateralIntensitySigma,this.bilateralManager.updateConfig({intensitySigma:e.bilateralIntensitySigma})),e.supersampleFactor!==void 0&&e.supersampleFactor!==this.supersampleFactor){l.info(`Render scale factor: ${this.supersampleFactor} \u2192 ${e.supersampleFactor}`);let i=Math.max(.5,Math.min(4,e.supersampleFactor));i!==e.supersampleFactor&&l.warn(`Invalid render scale factor ${e.supersampleFactor}, clamping to ${i}`),this.supersampleFactor=i,l.info(`Applying render scale factor: ${this.supersampleFactor}`),this.resize(this.canvas.width,this.canvas.height)}if(e.particleCount!==void 0&&e.particleCount!==this.particleSystem.particleCount&&(this.particleSystem.setParticleCount(e.particleCount),this.textureManager.resize(this.particleSystem.getResolution()),this.textureManager.initializeData(this.particleSystem.getAllData()),this.gl.deleteBuffer(this.indexBufferPoints),this.gl.deleteBuffer(this.indexBufferLines),this.gl.deleteBuffer(this.vertexIdBufferLines),this.indexBufferPoints=this.createBuffer(this.particleSystem.getIndices()),this.indexBufferLines=this.createLineIndexBuffer(this.particleSystem.getActualParticleCount()),this.vertexIdBufferLines=this.createLineVertexIdBuffer(this.particleSystem.getActualParticleCount())),e.bbox!==void 0&&(this.bbox=e.bbox,this.particleSystem.setBBox(this.bbox),e.reinitializeParticles&&(this.particleSystem.initializeParticles(),this.textureManager.initializeData(this.particleSystem.getAllData()),l.info("Particles reinitialized to new viewport bounds")),this.clearScreen()),l.info(`Checking needsRecompile flag: ${t}`),t)if(this.lockShaderRecompilation)l.info("Shader recompilation needed but LOCKED - skipping recompilation");else{l.info("Recompiling shaders...");try{let i=this.compileShaders();if(i){l.error("Recompilation failed",i),l.warn("Shader compilation failed - visualization will not render until error is resolved");return}this.frame=0,l.info("Clearing screen after recompilation..."),this.clearScreen(),l.info("Configuration updated successfully, shaders recompiled")}catch(i){l.error("EXCEPTION during shader recompilation:",i),l.error("Stack trace:",i.stack),l.warn("Shader compilation exception - visualization will not render until error is resolved")}}else l.info("Configuration updated (no recompile needed)");l.info("Current settings applied",e)}testEncodeDecodeSymmetry(){let{encodeFloatRGBA:e,decodeFloatRGBA:t}=(pi(),hn(fr)),i=[0,.1,.25,.5,.75,.9,1];for(let d=0;d<100;d++)i.push(Math.random());let r=0,o=0,s=[],a=[];l.info("Testing encode/decode symmetry...");for(let d of i){let h=new Uint8Array(4);e(d,h,0);let p=t(h[0],h[1],h[2],h[3]),m=Math.abs(d-p);s.push(m),o+=m,r=Math.max(r,m),m>1e-6&&a.push({original:d,encoded:`[${h[0]}, ${h[1]}, ${h[2]}, ${h[3]}]`,decoded:p,error:m})}o/=i.length,l.info("Encode/Decode symmetry test results:",{testCount:i.length,maxError:r.toExponential(3),avgError:o.toExponential(3),problematicCount:a.length,problematicSamples:a.slice(0,5)}),l.info("Testing world coordinate round-trip [-5, 5]...");let c=[-5,-4,-2.5,0,2.5,4,5],u=[];for(let d of c){let h=(d-this.bbox.min[0])/(this.bbox.max[0]-this.bbox.min[0]),p=new Uint8Array(4);e(h,p,0);let m=t(p[0],p[1],p[2],p[3]),f=this.bbox.min[0]+m*(this.bbox.max[0]-this.bbox.min[0]),b=Math.abs(d-f);u.push({world:d.toFixed(3),normalized:h.toFixed(6),decoded:f.toFixed(6),error:b.toExponential(3)})}return l.info("World coordinate round-trip results:",{tests:u}),{maxError:r,avgError:o,problematic:a.length}}logBufferStats(){let e=this.gl;if(!this.framebufferManager){l.warn("No framebuffer manager available");return}let t=this.framebufferManager.currentIndex;e.bindFramebuffer(e.FRAMEBUFFER,this.framebufferManager.framebuffers[t]);let i=this.canvas.width,r=this.canvas.height,o=i*r,s=this.framebufferManager.isHDR(),a=s?new Float32Array(o*4):new Uint8Array(o*4);e.readPixels(0,0,i,r,e.RGBA,s?e.FLOAT:e.UNSIGNED_BYTE,a);let c=1/0,u=1/0,d=1/0,h=-1/0,p=-1/0,m=-1/0,f=0,b=0,x=0,g=-1/0,y=0;for(let E=0;E<o;E++){let L=E*4,D=a[L],A=a[L+1],B=a[L+2];s||(D/=255,A/=255,B/=255),c=Math.min(c,D),u=Math.min(u,A),d=Math.min(d,B),h=Math.max(h,D),p=Math.max(p,A),m=Math.max(m,B),f+=D,b+=A,x+=B;let N=Math.max(D,A,B);g=Math.max(g,N),N>1&&y++}let _=f/o,S=b/o,F=x/o;l.info("=== HDR Buffer Statistics ==="),l.info(`Framebuffer: ${i}x${r} (${o.toLocaleString()} pixels)`),l.info(`Format: ${s?"HDR Float":"LDR RGBA8"}`),l.info(`Red   - Min: ${c.toFixed(6)}, Max: ${h.toFixed(6)}, Avg: ${_.toFixed(6)}`),l.info(`Green - Min: ${u.toFixed(6)}, Max: ${p.toFixed(6)}, Avg: ${S.toFixed(6)}`),l.info(`Blue  - Min: ${d.toFixed(6)}, Max: ${m.toFixed(6)}, Avg: ${F.toFixed(6)}`),l.info(`Max Brightness (max of R/G/B): ${g.toFixed(6)}`),l.info(`Pixels above 1.0: ${y.toLocaleString()} (${(y/o*100).toFixed(2)}%)`),e.bindFramebuffer(e.FRAMEBUFFER,null)}logShaders(){if(!this.shaderSource){console.warn("No shader source available");return}console.log("=== UPDATE VERTEX SHADER ==="),console.log(this.shaderSource.updateVertex),console.log(`=== END SHADER ===
...
`),console.log("=== DRAW FRAGMENT SHADER ==="),console.log(this.shaderSource.drawFragment),console.log("=== END SHADER ==="),l.info("Shaders logged to console")}logUpdateShader(){if(!this.shaderSource){console.warn("No shader source available");return}console.log("=== UPDATE FRAGMENT SHADER (Integrator + Jacobian) ==="),console.log(this.shaderSource.updateFragment),console.log("=== END SHADER ==="),l.info("Update shader logged to browser console")}logDrawShader(){if(!this.shaderSource){console.warn("No shader source available");return}console.log("=== DRAW VERTEX SHADER ==="),console.log(this.shaderSource.drawVertex),console.log("=== END VERTEX SHADER ==="),console.log(""),console.log("=== DRAW FRAGMENT SHADER (Color Mode) ==="),console.log(this.shaderSource.drawFragment),console.log("=== END FRAGMENT SHADER ==="),l.info("Draw shaders (vertex + fragment) logged to browser console")}logScreenShader(){if(!this.shaderSource||!this.shaderSource.screenFragment){console.warn("No screen shader source available");return}console.log("=== SCREEN FRAGMENT SHADER (Tone Mapping) ==="),console.log(this.shaderSource.screenFragment),console.log("=== END SHADER ==="),l.info("Screen shader logged to browser console")}logStatsShaders(){if(l.info("=== DIAGNOSTIC STATS SHADERS ==="),this.velocityStatsManager&&this.velocityStatsManager.getShaderSource){let e=this.velocityStatsManager.getShaderSource();e?(console.log("=== VELOCITY STATS FRAGMENT SHADER ==="),console.log(e),console.log("=== END SHADER ===")):console.warn("Velocity stats shader not available")}else console.warn("VelocityStatsManager not initialized or no getShaderSource method");if(this.bufferStatsManager&&this.bufferStatsManager.getShaderSource){let e=this.bufferStatsManager.getShaderSource();e?(console.log("=== BRIGHTNESS STATS FRAGMENT SHADER ==="),console.log(e),console.log("=== END SHADER ===")):console.warn("Buffer stats shader not available")}else console.warn("BufferStatsManager not initialized or no getShaderSource method");l.info("Stats shaders logged to browser console")}readPixelAt(e,t){let i=this.gl,r=Math.floor(e*(this.renderWidth/this.canvas.width)),o=Math.floor(t*(this.renderHeight/this.canvas.height)),s=this.renderHeight-o-1;if(r<0||r>=this.renderWidth||s<0||s>=this.renderHeight)return null;let a={hdr:null,ldr:null};if(this.useHDR&&this.framebufferManager){let c=this.framebufferManager.getCurrentTexture(),u=this.framebufferManager.framebuffers[this.framebufferManager.currentIndex];if(i.bindFramebuffer(i.FRAMEBUFFER,u),this.framebufferManager.hdrType===i.FLOAT){let d=new Float32Array(4);i.readPixels(r,s,1,1,i.RGBA,i.FLOAT,d),a.hdr=[d[0],d[1],d[2]]}else{let d=new Uint8Array(4);i.readPixels(r,s,1,1,i.RGBA,i.UNSIGNED_BYTE,d),a.hdr=[d[0]/255,d[1]/255,d[2]/255]}}if(this.ldrFramebuffer&&this.ldrTexture){i.bindFramebuffer(i.FRAMEBUFFER,this.ldrFramebuffer);let c=new Uint8Array(4);i.readPixels(r,s,1,1,i.RGBA,i.UNSIGNED_BYTE,c),a.ldr=[c[0]/255,c[1]/255,c[2]/255]}return i.bindFramebuffer(i.FRAMEBUFFER,null),a}};ae();var ie=class{constructor(e,t,i={}){this.id=e,this.defaultValue=t,this.settingsKey=i.settingsKey||e,this.onChange=i.onChange||null,this.element=null}getValue(){throw new Error("getValue() must be implemented by subclass")}setValue(e){throw new Error("setValue() must be implemented by subclass")}reset(){this.setValue(this.defaultValue)}attachListeners(e){throw new Error("attachListeners() must be implemented by subclass")}getMousePositionInElement(e,t){let r=$(t).offset();return{x:e.pageX-r.left,y:e.pageY-r.top}}handleButtonAction(e){return!1}saveToSettings(e){e[this.settingsKey]=this.getValue()}restoreFromSettings(e){e&&e[this.settingsKey]!=null&&this.setValue(e[this.settingsKey])}};var xt=class extends ie{constructor(e,t,i={}){super(e,t,i),this.trim=i.trim!==void 0?i.trim:!0}getValue(){let t=$(`#${this.id}`).val();return this.trim&&(t=t.trim()),t}setValue(e){$(`#${this.id}`).val(e)}attachListeners(e){let t=$(`#${this.id}`);this.element=t,this.setValue(this.defaultValue),t.on("input",()=>{let i=this.getValue();this.onChange&&this.onChange(i),e&&e()})}};var yt=class{constructor(e={}){this.controls=new Map,this.storageKey=e.storageKey||"settings",this.onApply=e.onApply||null,this.debounceTime=e.debounceTime||300,this.applyTimeout=null,this.renderer=e.renderer||null}register(e){return this.controls.set(e.id,e),e}registerAll(e){e.forEach(t=>this.register(t))}initializeControls(){let e=()=>this.debouncedApply();for(let t of this.controls.values())t.attachListeners(e)}attachAllListeners(){this.initializeControls()}applySettings(e){this.setSettings(e)}get(e){return this.controls.get(e)}getSettings(){let e={};for(let t of this.controls.values())t.saveToSettings(e);return e}setSettings(e){if(e.coordinateSystem&&e.dimensions)try{let t=e.coordinateSystem,i=O.fromJSON(t);if(i.dimensions===e.dimensions){this.renderer&&(this.renderer.coordinateSystem=i);let r=this.get("dimension-inputs");r&&r.setCoordinateSystem&&r.setCoordinateSystem(i,!1)}}catch(t){console.error("Failed to restore coordinate system from settings:",t)}for(let t of this.controls.values())t.restoreFromSettings(e);for(let t of this.controls.values())t.onChange&&e&&e[t.settingsKey]!==void 0&&t.onChange(t.getValue())}resetAll(){for(let e of this.controls.values())e.reset()}saveToStorage(){let e=this.getSettings();return localStorage.setItem(this.storageKey,JSON.stringify(e)),e}loadFromStorage(){try{let e=localStorage.getItem(this.storageKey);if(e){let t=JSON.parse(e);return this.setSettings(t),t}}catch(e){console.warn("Failed to load settings from localStorage:",e)}return null}clearStorage(){localStorage.removeItem(this.storageKey)}debouncedApply(){this.applyTimeout&&clearTimeout(this.applyTimeout),this.applyTimeout=setTimeout(()=>{if(this.onApply){let e=this.getSettings();this.onApply(e)}},this.debounceTime)}apply(){if(this.onApply){let e=this.getSettings();this.onApply(e)}}};function Gn(n=2,e=10){return t=>{let i=Math.abs(t);return i>=e||i<1/e&&i!==0?t.toExponential(n):t.toFixed(n+1)}}function Hn(n=2){return e=>e.toFixed(n)}function jn(){return n=>n.toFixed(0)}function gr(n,e,t){return n>0&&e/n>1e3?"adaptive":"linear"}function br(n,e,t=!1){let i=Math.abs(n),r=i<e?e:Math.pow(10,Math.floor(Math.log10(i))-1);return r=Math.max(e,r),t&&(r*=10),r}function xr(n=!1){return n?10:1}function yr(n,e=!1){return e?n*10:n}function vr(n){if(typeof n.displayFormat=="function")return n.displayFormat;let e=n.displayPrecision??2;switch(n.displayFormat){case"scientific":return Gn(e,n.scientificThreshold??10);case"integer":return jn();case"decimal":default:return Hn(e)}}function me(n,e=0){setTimeout(()=>{let t;if(typeof n=="string"?t=$(n):t=n,!t.length){console.warn("resizeAccordion: Element not found",n);return}let i=t.closest(".accordion-section");i.length&&!i.hasClass("collapsed")&&i.css("max-height",i[0].scrollHeight+"px")},e)}var fi=class extends ie{constructor(e,t,i,r={}){super(e,i,r),this.parameterDef=t,this.scale=t.scale||gr(t.min,t.max,t.step),this.formatter=vr(t),this.displayElement=null}calculateIncrement(e,t=!1){switch(this.scale){case"adaptive":return br(e,this.parameterDef.step,t);case"log":return xr(t);case"linear":default:return yr(this.parameterDef.step,t)}}handleButtonAction(e){let t=$(`#${this.id}`);if(!t.length)return!1;let i=this.getValue(),r=this.parameterDef.min,o=this.parameterDef.max,s=i;if(e==="reset")s=this.defaultValue;else{let a=this.calculateIncrement(i,e.includes("large"));if(e.startsWith("increase"))s=Math.min(o,i+a);else if(e.startsWith("decrease"))s=Math.max(r,i-a);else return!1}return s!==i?(this.setValue(s),t.trigger("input"),!0):!1}render(){let e=`${this.id}-display`,t=this.defaultValue;return`
        ...
        `}attachListeners(e){super.attachListeners(e),setTimeout(()=>{let t=$(`#${this.id}`);if(!t.length)return;let i=$(`#${this.id}-anim-btn`);if(!i.length)return;i.on("click",s=>{s.stopPropagation(),this.setAnimationEnabled(!this.animationEnabled)});let r=`
            ...
            `,o=t.closest(".control-group");o.length?(o.after(r),this.attachThumbDragHandlers()):console.warn(`AnimatableParameterControl: Could not find .control-group parent for #${this.id}`)},0)}attachThumbDragHandlers(){let e=$(`#${this.id}-min-handle`),t=$(`#${this.id}-max-handle`),i=e.parent();[{handle:e,bound:"min"},{handle:t,bound:"max"}].forEach(({handle:o,bound:s})=>{let a=!1,c=0,u=0;o.on("mousedown",d=>{a=!0,c=d.pageX,u=parseFloat(o.css("left"))||0,o.addClass("dragging"),e.css("z-index","10"),t.css("z-index","10"),o.css("z-index","11"),$("body").css("user-select","none"),d.preventDefault()}),$(document).on("mousemove",d=>{if(!a)return;let h=i.offset(),p=i.width(),f=(d.pageX-h.left)/p*100;f=Math.max(0,Math.min(100,f));let b=this.parameterDef.min,x=this.parameterDef.max,g=b+f/100*(x-b);s==="min"?g<=this.animationMax&&(this.animationMin=g):g>=this.animationMin&&(this.animationMax=g),this.updateBoundsDisplay()}),$(document).on("mouseup",()=>{a&&(a=!1,o.removeClass("dragging"),$("body").css("user-select",""))})})}saveToSettings(e){super.saveToSettings(e),this.animationEnabled&&(e.animations||(e.animations={}),e.animations[this.settingsKey]={enabled:!0,min:this.animationMin,max:this.animationMax})}restoreFromSettings(e){if(super.restoreFromSettings(e),e.animations&&e.animations[this.settingsKey]){let t=e.animations[this.settingsKey];this.animationEnabled=t.enabled||!1,this.animationMin=t.min,this.animationMax=t.max,this.updateAnimationUI()}}};M();var vt=class extends ie{constructor(e,t={}){super("dimension-inputs",e,t),this.dimensionsControl=null,this.varNames=["x","y","z","w","u","v"],this.coordinateSystem=null}setDimensionsControl(e){this.dimensionsControl=e}setCoordinateSystem(e,t=!0){this.coordinateSystem=e,e&&e.getDisplayLabels?this.varNames=e.getDisplayLabels():this.varNames=["x","y","z","w","u","v"],t&&this.updateInputs()}getDimensions(){if(this.dimensionsControl)return this.dimensionsControl.getValue();let e=document.getElementById("dimensions");return e&&e.getValue?e.getValue():2}getValue(){let e=this.getDimensions(),t=[];for(let i=0;i<e;i++){let r=$(`#expr-${i}`).val(),o=r?r.trim():"0";o&&window.UnicodeAutocomplete&&window.UnicodeAutocomplete.unicodeToAscii&&(o=window.UnicodeAutocomplete.unicodeToAscii(o)),t.push(o||"0")}return t}setValue(e){Array.isArray(e)&&this.updateInputs(e.length,e)}updateInputs(e=null,t=null){e===null&&(e=this.getDimensions()),l.verbose("updateInputs called with dimensions:",e);let i=$("#dimension-inputs");if(l.verbose("Container found:",i.length),i.length===0){l.error("Could not find #dimension-inputs element");return}let r;if(t&&Array.isArray(t))r=t,l.verbose("Using provided newValues");else{let o=$("#expr-0");l.verbose("firstElement exists:",o.length>0);try{if(o.length>0){let s=[],a=0;for(;$(`#expr-${a}`).length>0;){let c=$(`#expr-${a}`).val();s.push(c?c.trim():"0"),a++}r=s,l.verbose("Got current values from existing inputs:",r)}else r=this.defaultValue,l.verbose("Using defaultValue")}catch(s){l.error("Error getting values:",s),r=this.defaultValue}}if(r.length<e){let o=[...r];for(let s=r.length;s<e;s++)o[s]="0";r=o,l.verbose("Padded values to match dimensions:",r)}l.verbose("Values to use:",r),i.empty(),l.verbose("Container emptied, now creating",e,"inputs");for(let o=0;o<e;o++){let s=["x","y","z","w","u","v"],a=this.varNames[o]||s[o]||`v${o}`,c=r[o]||"0",u=$('<div class="dimension-input"></div>');u.append(`<label>d${a}/dt =</label>`),u.append(`<input type="text" id="expr-${o}" value="${c}">`),i.append(u),l.verbose("Created input",o,"with value:",c)}l.verbose("All inputs created, container now has",i.children().length,"children"),this.attachInputListeners(),window.unicodeAutocomplete&&window.unicodeAutocomplete.attachToAll('[id^="expr-"]'),this.updateAccordionHeight()}updateAccordionHeight(){me("#dimension-inputs",0)}attachInputListeners(){let e=this.onChangeCallback;$(document).off("input",'[id^="expr-"]'),$(document).on("input",'[id^="expr-"]',()=>{this.onChange&&this.onChange(this.getValue()),e&&e()})}attachListeners(e){this.onChangeCallback=e,this.updateInputs(),this.attachInputListeners()}reset(){this.setValue(this.defaultValue)}},wt=class extends ie{constructor(e,t={}){super("mapper-params",e,t),this.dimensionsControl=null,this.mapperControl=null,this.varNames=["x","y","z","w","u","v"]}setRelatedControls(e,t){this.dimensionsControl=e,this.mapperControl=t}getContext(){let e=this.dimensionsControl?this.dimensionsControl.getValue():2,t=this.mapperControl?this.mapperControl.getValue():"select";return{dimensions:e,mapper:t}}getValue(){let{mapper:e}=this.getContext();if(e==="custom"){let r=$("#mapper-horizontal-expr"),o=$("#mapper-vertical-expr"),s=$("#mapper-depth-expr");return r.length&&o.length?{horizontalExpr:r.val()||"x",verticalExpr:o.val()||"y",depthExpr:s.val()||""}:this.currentParams||{horizontalExpr:"x",verticalExpr:"y",depthExpr:""}}let t=$("#mapper-dim1"),i=$("#mapper-dim2");if(t.length&&i.length){let r=parseInt(t.val()),o=parseInt(i.val());return isNaN(r)||isNaN(o)?(console.warn("Invalid mapper dimensions, using defaults"),this.defaultValue):{dim1:r,dim2:o}}return this.defaultValue}setValue(e){this.currentParams=e,this.updateControls()}updateControls(){let{dimensions:e,mapper:t}=this.getContext(),i=$("#mapper-controls");if(i.length!==0){if(i.empty(),t==="select"){let r=$('<div class="control-row"></div>'),o=$('<div class="control-group" style="flex: 1;"></div>');o.append("<label>Horizontal</label>");let s=$('<select id="mapper-dim1"></select>'),a=this.currentParams?this.currentParams.dim1:this.defaultValue.dim1;for(let h=0;h<e;h++){let p=h===a?"selected":"";s.append(`<option value="${h}" ${p}>${this.varNames[h]}</option>`)}o.append(s);let c=$('<div class="control-group" style="flex: 1;"></div>');c.append("<label>Vertical</label>");let u=$('<select id="mapper-dim2"></select>'),d=this.currentParams?this.currentParams.dim2:this.defaultValue.dim2;for(let h=0;h<e;h++){let p=h===d?"selected":"";u.append(`<option value="${h}" ${p}>${this.varNames[h]}</option>`)}c.append(u),r.append(o),r.append(c),i.append(r),this.attachSelectListeners()}else if(t==="project"){let r=$('<div class="info">Linear projection uses default 2D projection</div>');i.append(r)}else if(t==="custom"){let r=this.varNames.slice(0,e).join(", "),o=$('<div class="control-group"></div>');o.append("<label>Horizontal (X):</label>");let s=this.currentParams?.horizontalExpr||"x",a=$(`<input type="text" id="mapper-horizontal-expr" value="${s}" placeholder="e.g., sin(x) + sin(y)" style="font-family: monospace; width: 100%;" />`);o.append(a),i.append(o);let c=$('<div class="control-group"></div>');c.append("<label>Vertical (Y):</label>");let u=this.currentParams?.verticalExpr||"y",d=$(`<input type="text" id="mapper-vertical-expr" value="${u}" placeholder="e.g., -cos(x) - cos(y)" style="font-family: monospace; width: 100%;" />`);c.append(d),i.append(c);let h=$('<div class="control-group"></div>');h.append("<label>Depth (Z, optional):</label>");let p=this.currentParams?.depthExpr||"",m=$(`<input type="text" id="mapper-depth-expr" value="${p}" placeholder="optional, e.g., z" style="font-family: monospace; width: 100%;" />`);h.append(m),i.append(h);let f=$('<div class="info">Define math expressions using variables: '+r+". Functions: sin, cos, tan, exp, log, sqrt, abs, etc.</div>");i.append(f),this.attachCustomListener()}}}attachSelectListeners(){let e=this.onChangeCallback;$(document).off("change","#mapper-dim1, #mapper-dim2"),$("#mapper-dim1, #mapper-dim2").on("change",()=>{this.currentParams=this.getValue(),this.onChange&&this.onChange(this.currentParams),e&&e()})}attachCustomListener(){let e=this.onChangeCallback;$(document).off("input","#mapper-horizontal-expr, #mapper-vertical-expr, #mapper-depth-expr"),$("#mapper-horizontal-expr, #mapper-vertical-expr, #mapper-depth-expr").on("input",()=>{this.currentParams=this.getValue(),this.onChange&&this.onChange(this.currentParams),e&&e()})}attachListeners(e){this.onChangeCallback=e,this.currentParams=this.defaultValue,this.updateControls()}reset(){this.currentParams=this.defaultValue,this.updateControls()}},_t=class extends ie{constructor(e,t={}){super("color-gradient",e,t),this.gradientEditor=null}setGradientEditor(e){this.gradientEditor=e}getValue(){return this.gradientEditor&&this.gradientEditor.getGradient?this.gradientEditor.getGradient():this.currentGradient||this.defaultValue}setValue(e){this.currentGradient=e,this.gradientEditor&&this.gradientEditor.setGradient&&this.gradientEditor.setGradient(e)}attachListeners(e){this.onChangeCallback=e}notifyChange(e){this.currentGradient=e,this.onChange&&this.onChange(e),this.onChangeCallback&&this.onChangeCallback()}reset(){this.setValue(this.defaultValue)}saveToSettings(e){e[this.settingsKey]=this.getValue()}restoreFromSettings(e){e&&e[this.settingsKey]!==void 0&&this.setValue(e[this.settingsKey])}},Et=class extends ie{constructor(e,t={}){super("transform-params",e,t),this.transformControl=null,this.currentParams=e||{},this.parameterControls=new Map}setTransformControl(e){this.transformControl=e}getTransformType(){return this.transformControl?this.transformControl.getValue():"identity"}getValue(){let e={};for(let[t,i]of this.parameterControls.entries()){let r=i.settingsKey;e[r]=i.getValue()}return e}setValue(e){this.currentParams=e||{},this.updateControls()}updateControls(){let e=this.getTransformType(),t=$("#transform-controls");if(t.length===0)return;t.empty(),this.parameterControls.clear();let i=ft(e);if(!i){l.warn(`Transform not found: ${e}`),this.updateAccordionHeight();return}let r=i.getParameters();if(r.length===0){this.updateAccordionHeight();return}r.forEach((o,s)=>{let a=`transform-param-${s}`,c=this.currentParams[o.name]??o.default,u=new _e(a,o,c,{settingsKey:o.name,onChange:d=>{this.currentParams[o.name]=d,this.onChange&&this.onChange(this.currentParams)}});t.append(u.render()),u.attachListeners(this.onChangeCallback),this.parameterControls.set(a,u)}),this.updateAccordionHeight()}updateAccordionHeight(){me("#transform-controls",0)}handleTransformParamButton(e,t){let i=this.parameterControls.get(e);return i?i.handleButtonAction(t):!1}attachListeners(e){this.onChangeCallback=e,this.updateControls()}reset(){this.currentParams=this.defaultValue,this.updateControls()}saveToSettings(e){e[this.settingsKey]=this.getValue()}restoreFromSettings(e){e&&e[this.settingsKey]!==void 0&&this.setValue(e[this.settingsKey])}};function wr(n,e,t){let i=$(`#${n}`);if(!i.length)return console.error(`Gradient editor container #${n} not found`),null;let r=e.map(u=>({...u}));function o(){let u=[...r].sort((h,p)=>h.position-p.position),d='<div class="gradient-stops">';u.forEach((h,p)=>{let m=r.indexOf(h),f=ui(h.color);d+=`
...
2. Enable "Use iterative Newton solver" checkbox`),m.text(f),m.prop("disabled",!1)),$("#coord-preset").val("custom")}catch(b){console.error("Error solving inverse:",b),alert("Error solving inverse: "+b.message),m.text(f),m.prop("disabled",!1)}}),$("#coord-use-iterative").off("change").on("change",function(){let m=$(this).is(":checked");o.useIterativeSolver=m,$(".coord-inverse-transform").prop("disabled",m),m?$(".coord-inverse-transform").css("opacity","0.5"):$(".coord-inverse-transform").css("opacity","1.0"),$("#coord-preset").val("custom")}),$("#coord-apply").off("click").on("click",function(){let m=$(this);m.prop("disabled",!0);let f=m.text();m.text("Applying...");try{if(d()){let x=o.forwardTransforms.map(_=>window.UnicodeAutocomplete&&window.UnicodeAutocomplete.unicodeToAscii?window.UnicodeAutocomplete.unicodeToAscii(_):_),g=o.inverseTransforms?o.inverseTransforms.map(_=>window.UnicodeAutocomplete&&window.UnicodeAutocomplete.unicodeToAscii?window.UnicodeAutocomplete.unicodeToAscii(_):_):null,y=new O(o.name,o.dimensions,o.variables,x,g,o.useIterativeSolver||!1);setTimeout(()=>{i(y),m.prop("disabled",!1),m.text(f)},100)}else alert("Invalid coordinate system. Please check your expressions."),m.prop("disabled",!1),m.text(f)}catch(b){console.error("Error in apply handler:",b),m.prop("disabled",!1),m.text(f)}}),$("#coord-cancel").off("click").on("click",function(){i(null)})}function d(){for(let m=0;m<s;m++)if(!o.variables[m]||!o.variables[m].label||!o.forwardTransforms[m]||o.forwardTransforms[m].trim()==="")return!1;return!0}function h(m){o=m,c()}function p(m){m!==s&&(s=m,o=j(m),c())}return c(),a}function $r(n,e,t){let i=$("#coordinate-editor-panel");i.length||($("body").append(`
        ...
        `),i=$("#coordinate-editor-panel")),i.show();let r=Jn("coordinate-editor-container",n,e,c=>{c===null||t(c),i.hide()});$(".floating-panel-close",i).off("click").on("click",function(){i.hide()});let o=!1,s=0,a=0;return $(".panel-header",i).off("mousedown").on("mousedown",function(c){o=!0,s=c.clientX-i.offset().left,a=c.clientY-i.offset().top,$(document).on("mousemove.panelDrag",function(u){o&&i.css({left:u.clientX-s,top:u.clientY-a})})}),$(document).on("mouseup.panelDrag",function(){o=!1,$(document).off("mousemove.panelDrag")}),r}ae();M();M();var $t=class{constructor(e){if(!e)throw new Error("GLSLWorkflow requires a generator instance");this.generator=e}getGenerationContext(e,t){throw new Error("GLSLWorkflow subclass must implement getGenerationContext()")}applyToRenderer(e,t,i){throw new Error("GLSLWorkflow subclass must implement applyToRenderer()")}async executeInteractive(e,t,i,r){let o=e.length,s=[],a=e.map(u=>u.getMath());for(let u=0;u<a.length;u++){let d=this.getGenerationContext(u,o),h=this.generator.validate(a[u],d);h.valid||s.push({index:u,error:h.error})}if(s.length>0)return this._displayErrors(r,s),{success:!1,errors:s};let c=[];for(let u=0;u<a.length;u++)try{let d=this.getGenerationContext(u,o),h=this.generator.generate(a[u],t,d);c.push(h)}catch(d){s.push({index:u,error:d.message})}if(s.length>0)return this._displayErrors(r,s),{success:!1,errors:s};try{this.applyToRenderer(c,i,a),l.info(`Applied ${c.length} expressions to renderer`)}catch(u){return this._displayErrors(r,[{index:-1,error:`Failed to apply to renderer: ${u.message}`}]),{success:!1,errors:[{index:-1,error:u.message}]}}return r.hide(),{success:!0,glslArray:c,expressions:a}}executeAutomated(e,t,i){let r=e.length,o=[];for(let a=0;a<e.length;a++){let c=this.getGenerationContext(a,r),u=this.generator.validate(e[a],c);u.valid||o.push({index:a,error:u.error})}if(o.length>0){let a=o.map(c=>`Index ${c.index}: ${c.error}`).join("; ");throw new Error(`Validation failed: ${a}`)}let s=[];for(let a=0;a<e.length;a++)try{let c=this.getGenerationContext(a,r),u=this.generator.generate(e[a],t,c);s.push(u)}catch(c){throw new Error(`Failed to generate GLSL at index ${a}: ${c.message}`)}try{this.applyToRenderer(s,i,e),l.info(`Applied ${s.length} expressions to renderer (automated)`)}catch(a){throw new Error(`Failed to apply to renderer: ${a.message}`)}return{success:!0,glslArray:s,expressions:e}}_displayErrors(e,t){let i=t.map(r=>`<div style="color: #f44336; margin: 4px 0;">
                <strong>${r.index>=0?`Index ${r.index}`:"Global"}:</strong> ${r.error}
            </div>`).join("");e.showError(i)}};M();var St=class{generate(e,t,i={}){throw new Error("IGLSLGenerator.generate() must be implemented by subclass")}validate(e,t={}){throw new Error("IGLSLGenerator.validate() must be implemented by subclass")}getDescription(){return"GLSL Generator"}getMathPlaceholder(){return"Enter math expression..."}getGLSLPlaceholder(){return"Generated GLSL code..."}};se();M();var Ee=class extends St{constructor(e={}){super(),this.variableNames=e.variableNames||null}validate(e,t={}){return!e||!e.trim()?{valid:!1,error:"Expression is empty"}:t.dimension===void 0?{valid:!1,error:"Context must include dimension index"}:t.totalDims===void 0?{valid:!1,error:"Context must include totalDims"}:{valid:!0}}generate(e,t,i={}){let r=this.validate(e,i);if(!r.valid)throw new Error(`Validation failed: ${r.error}`);let{dimension:o,totalDims:s}=i,a=e;if(t)try{a=t.expandFunctions(e),l.verbose(`Field equation ${o}: "${e}" \u2192 "${a}"`)}catch(u){l.warn(`Failed to expand functions for dimension ${o}:`,u.message)}let c=this.variableNames||this._getDefaultVariableNames(s);try{let u=W(a,s,c,"pos");return l.verbose(`Field equation ${o} GLSL: ${u}`),u}catch(u){throw new Error(`Failed to parse expression for dimension ${o}: ${u.message}`)}}_getDefaultVariableNames(e){return["x","y","z","w","u","v"].slice(0,e)}getDescription(){return"Vector field equation (dx/dt = ...)"}getMathPlaceholder(){return"Enter field equation (e.g., -y, sin(x), etc.)"}getGLSLPlaceholder(){return"Generated GLSL expression..."}};M();var le=class extends $t{constructor(e=null){super(e||new Ee)}getGenerationContext(e,t){return{dimension:e,totalDims:t}}applyToRenderer(e,t,i){let r=i.map((o,s)=>{if(window.notebook)try{return window.notebook.expandFunctions(o)}catch(a){return l.warn(`Failed to expand "${o}" for JS evaluator:`,a.message),o}return o});t.updateConfig({dimensions:i.length,expressions:r,velocityGLSL:e})}_displayErrors(e,t){let i=t.map(r=>`<div style="color: #f44336; margin: 4px 0;">
                <strong>${r.index>=0?`Dimension ${r.index}`:"Global"}:</strong> ${r.error}
            </div>`).join("");e.showError(i)}};var Tt=class{constructor(e){this.manager=e,this.registrationPromises=[]}register(e,t){let i=customElements.whenDefined(e).then(()=>new Promise(r=>{requestAnimationFrame(()=>{let o=document.getElementById(t);if(o&&o._initialized){this.manager.register(o);let s=()=>this.manager.debouncedApply();o.attachListeners(s),r(o)}else console.warn(`Web Component ${e}#${t} not found or not initialized`),r(null)})}));return this.registrationPromises.push(i),i}whenAllReady(){return Promise.all(this.registrationPromises)}async restoreSettings(e){let t=await this.whenAllReady();for(let i of t)i&&e[i.settingsKey]!==void 0&&i.setValue(e[i.settingsKey])}async applyWhenReady(e=null){await this.whenAllReady(),e&&(this.manager.applySettings(e),await this.restoreSettings(e)),this.manager.apply()}};M();M();se();async function Sr(){if(window.MathJax&&window.MathJax.startup&&window.MathJax.startup.promise){await window.MathJax.startup.promise,l.info("MathJax fully initialized via startup.promise");return}return window.MathJax&&window.MathJax.typesetPromise?(l.info("MathJax already available"),Promise.resolve()):(l.info("Waiting for MathJax to load..."),new Promise(n=>{let e=setInterval(()=>{window.MathJax&&window.MathJax.typesetPromise&&(clearInterval(e),l.info("MathJax loaded and ready"),n())},100);setTimeout(()=>{clearInterval(e),window.MathJax||l.error("MathJax failed to load after 10 seconds"),n()},1e4)}))}async function Tr(n,e){if(await Sr(),!window.MathJax){l.error("MathJax not available after wait, falling back to plain text"),n.textContent=e;return}try{n.innerHTML="",l.info("Rendering LaTeX:",e);let t=window.MathJax.tex2svg(e,{display:!0});n.appendChild(t),l.info("Rendered LaTeX successfully")}catch(t){l.error("Failed to render LaTeX:",t),l.error("Error stack:",t.stack),l.error("LaTeX content:",e),n.textContent=e}}async function Cr(n,e,t=null){if(await Sr(),!window.MathJax){l.error("MathJax not available after wait, falling back to plain text"),n.textContent=e;return}try{n.innerHTML="";let i=window.MathJax.tex2svg(e,{display:!0}),r=document.createElement("div"),o=document.createElement("div");o.style.cssText=`
            transform-origin: right center;
            display: inline-block;
            position: absolute;
            right: 0;
        `,o.appendChild(i);let s=document.createElement("div");s.style.cssText="visibility: hidden",r.appendChild(o),r.appendChild(s),n.appendChild(r);let a=r.getBoundingClientRect(),c=o.offsetWidth,u=t||n.clientWidth;l.info(`Rendered TeX size: ${c}px (measured), target: ${u}px`);let d=1;c>u&&u>0?(d=u/c,l.info(`Scaling TeX: ${c}px -> ${u}px (scale: ${d.toFixed(3)}x)`)):l.info(`No scaling needed: rendered ${c}px, target ${u}px`);let h=o.offsetHeight;r.style.display="block",r.style.position="relative",r.style.minHeight=`${h}px`,r.style.maxWidth=`${u}px`,o.style.transform=`scale(${d})`,s.style.height=`height: ${h*d}px`,o.style.visibility="visible",r.style.visibility="visible",l.info(`Actual rendered: wrapper=${r.offsetWidth}px, scaler=${o.offsetWidth}px, spacer=${s.offsetHeight}px`),l.info(`Rendered LaTeX successfully with scale ${d.toFixed(3)}`)}catch(i){l.error("Failed to render LaTeX:",i),n.textContent=e}}function Yn(n,e,t){l.verbose(`Converting to derivative notation: d${e}/dt = ${n}`);let i=Ni(n,t);return l.verbose(`Generated LaTeX: ${i}`),`\\frac{d${e}}{dt} = ${i}`}function Ar(n,e){if(l.verbose(`createSystemLatex called with ${n.length} expressions:`,n),l.verbose("Variables:",e),!n||n.length===0)return"";let t=n.length;return`\\begin{array}{l}
${n.map((r,o)=>{let s=e[o]||`x_{${o}}`;return`\\displaystyle ${Yn(r,s,t)}`}).join(` \\\\
`)}
\\end{array}`}var gi=class{constructor(){this.overlayElement=null,this.visible=!1,this.currentExpressions=[],this.currentVariables=[],this.visibleOpacity="60%"}initialize(e="equation-overlay"){if(this.overlayElement=document.getElementById(e),!this.overlayElement){l.error(`Equation overlay element not found: #${e}`);return}this.contentElement=document.createElement("div"),this.contentElement.className="equation-content",this.overlayElement.appendChild(this.contentElement),window.addEventListener("resize",()=>{this.visible&&this.scaleToFit()}),l.info("Equation overlay initialized")}show(){this.overlayElement&&(this.visible=!0,this.overlayElement.classList.add("visible"),this.overlayElement.style.opacity=this.visibleOpacity||"1",l.verbose("Equation overlay shown"))}hide(){this.overlayElement&&(this.visible=!1,this.overlayElement.classList.remove("visible"),this.overlayElement.style.opacity="0",l.verbose("Equation overlay hidden"))}setVisible(e){e?this.show():this.hide()}async updateEquations(e,t){if(!(!this.overlayElement||!this.contentElement)&&(this.currentExpressions=e,this.currentVariables=t,!!this.visible))try{let i=Ar(e,t);if(!i){this.contentElement.innerHTML="<em>No equations</em>";return}await Tr(this.contentElement,i),this.scaleToFit(),l.verbose(`Updated equation overlay with ${e.length} equations`)}catch(i){l.error("Failed to update equation overlay:",i),this.contentElement.innerHTML="<em>Error rendering equations</em>"}}scaleToFit(){if(!this.overlayElement||!this.contentElement)return;let e=this.contentElement.querySelector("mjx-container");e&&(this.overlayElement.style.opacity="0",e.style.transform="",e.style.transformOrigin="top left",e.style.fontSize="",setTimeout(()=>{e.offsetWidth;let t=window.innerWidth-810,i=t-40,r=e.querySelector("svg"),o;r?o=r.getBoundingClientRect().width:o=e.scrollWidth,l.verbose(`Equation sizing: SVG content=${o.toFixed(1)}px, available=${i}px (max-width=${t}px - 40px padding)`),l.verbose(`  mjx-container: getBBox=${e.getBoundingClientRect().width.toFixed(1)}px, scrollWidth=${e.scrollWidth}px`);let s=()=>{setTimeout(()=>{let a=e.querySelector("svg"),c=a?a.getBoundingClientRect():e.getBoundingClientRect(),u=Math.ceil(c.width+40),d=Math.ceil(c.height+32);this.overlayElement.style.width=`${u}px`,this.overlayElement.style.height=`${d}px`,this.overlayElement.style.opacity=this.visibleOpacity||"1",l.verbose(`Final panel size: ${u}px \xD7 ${d}px (SVG: ${c.width.toFixed(1)}px \xD7 ${c.height.toFixed(1)}px)`)},100)};if(o>i){let a=o/i;if(a<1.5){let c=Math.max(.7,1/a);e.style.fontSize=`${(c*100).toFixed(0)}%`,l.verbose(`Reduced equation font size to ${(c*100).toFixed(0)}% to fit (${o.toFixed(1)}px \u2192 ${i}px)`)}else{e.style.fontSize="70%";let c=Math.max(.4,i/(o*.7));e.style.transform=`scale(${c})`,l.verbose(`Reduced font to 70% and scaled by ${(c*100).toFixed(0)}% to fit (${o.toFixed(1)}px \u2192 ${i}px)`)}s()}else l.verbose("Equations fit naturally (no scaling needed)"),s()},250))}async refresh(){this.currentExpressions.length>0&&await this.updateEquations(this.currentExpressions,this.currentVariables)}isVisible(){return this.visible}},Y=new gi;M();var Ct=class{constructor(e,t){this.renderer=e,this.controlManager=t,this.isRunning=!1,this.animationFrameId=null,this.direction=1,this.alpha=0,this.stepsPerIncrement=10,this.stepCounter=0,this.options={clearParticles:!1,clearScreen:!1,smoothTiming:!1,lockShaders:!1},this.ALPHA_STEP_TIME_TARGET_PERCENTILE=95,this.ALPHA_STEP_TIME_DECAY=.002,this.ALPHA_STEP_TIME_UPWARD=this.ALPHA_STEP_TIME_TARGET_PERCENTILE/(100-this.ALPHA_STEP_TIME_TARGET_PERCENTILE)*this.ALPHA_STEP_TIME_DECAY,this.ALPHA_STEP_TIME_WARMUP_CYCLES=3,this.ALPHA_STEP_TIME_DISPLAY_INTERVAL=10,this.alphaStepTimeEMA=null,this.alphaStepStartTime=null,this.alphaStepWarmupCounter=0,this.alphaStepDisplayCounter=0,this.cachedAnimatableControls=null,this.cachedAnimatableParams=null,this.lastAppliedSettings=null,this.shaderLockWasEnabled=!1,this.savedLoggerVerbosity=null,this.frameCaptureMode=null,this.frameCaptureTotal=0,this.frameCaptureCount=0,this.frameCaptureAlphaIncrement=.01,this.capturedFrames=[],this.frameCaptureOnProgress=null,this.frameCaptureOnComplete=null,this._alphaChangedListeners=[]}setAlpha(e,t=!0){this.alpha=Math.max(0,Math.min(1,e)),this.renderer&&this.renderer.setAnimationAlpha(this.alpha),t&&(this._updateAnimatableControls(this.alpha),this._applyChangedSettings(),this.options.clearParticles&&this.renderer&&this.renderer.resetParticles(),this.options.clearScreen&&this.renderer&&this.renderer.clearRenderBuffer()),this._emitAlphaChanged(this.alpha)}getAlpha(){return this.alpha}setSpeed(e){this.stepsPerIncrement=Math.max(1,Math.round(e))}setOptions(e){Object.assign(this.options,e)}start(e={}){if(this.isRunning)return;let{captureFrames:t=!1,totalFrames:i=0,loops:r=1,halfLoops:o=!1,onProgress:s=null,onComplete:a=null}=e;if(t){this.frameCaptureMode="fixed",this.frameCaptureTotal=i,this.frameCaptureCount=0,this.capturedFrames=[],this.frameCaptureOnProgress=s,this.frameCaptureOnComplete=a;let c=o?1:2;this.frameCaptureAlphaIncrement=r*c/i}else this.frameCaptureMode="continuous",this.frameCaptureAlphaIncrement=.01;this.isRunning=!0,this.renderer&&this.renderer.stop(),this._resetTimingState(),this.options.lockShaders&&this.renderer?(this.renderer.lockShaderRecompilation=!0,this.shaderLockWasEnabled=!0,l.info("Shader recompilation lock ENABLED (animation started)",null,!1)):this.shaderLockWasEnabled=!1,this.savedLoggerVerbosity=l.verbosity,l.setVerbosity("silent"),this._cacheAnimatableControls(),this._renderLoop()}stop(){this.isRunning=!1,this.animationFrameId!==null&&(cancelAnimationFrame(this.animationFrameId),this.animationFrameId=null),this.renderer&&!this.renderer.isRunning&&this.renderer.start();let e=document.getElementById("step-time-counter");e&&(e.style.display="none"),this.cachedAnimatableControls=null,this.cachedAnimatableParams=null,this.lastAppliedSettings=null,this.savedLoggerVerbosity!==null&&(l.setVerbosity(this.savedLoggerVerbosity),this.savedLoggerVerbosity=null),this.shaderLockWasEnabled&&this.renderer&&(this.renderer.lockShaderRecompilation=!1,this.shaderLockWasEnabled=!1,l.info("Shader recompilation lock DISABLED (animation stopped)")),this.frameCaptureMode=null}getFrames(){return this.capturedFrames}onAlphaChanged(e){this._alphaChangedListeners.push(e)}offAlphaChanged(e){let t=this._alphaChangedListeners.indexOf(e);t!==-1&&this._alphaChangedListeners.splice(t,1)}_resetTimingState(){this.stepCounter=0,this.alphaStepTimeEMA=null,this.alphaStepStartTime=null,this.alphaStepWarmupCounter=0,this.alphaStepDisplayCounter=0,this.lastAppliedSettings=null}_cacheAnimatableControls(){this.cachedAnimatableControls=[],this.controlManager.controls.forEach(t=>{t&&typeof t.animationEnabled<"u"&&t.updateFromAlpha&&this.cachedAnimatableControls.push(t)}),this.cachedAnimatableParams=[];let e=this.controlManager.controls.get("transform-params");e&&e.parameterControls&&e.parameterControls.forEach(t=>{t instanceof _e&&this.cachedAnimatableParams.push(t)})}_updateAnimatableControls(e){this.cachedAnimatableControls||this._cacheAnimatableControls();for(let t=0;t<this.cachedAnimatableControls.length;t++){let i=this.cachedAnimatableControls[t];i.animationEnabled&&i.updateFromAlpha(e)}for(let t=0;t<this.cachedAnimatableParams.length;t++){let i=this.cachedAnimatableParams[t];i.animationEnabled&&i.updateFromAlpha(e)}}_applyChangedSettings(){if(!this.renderer||!this.cachedAnimatableControls)return;let e={};for(let t=0;t<this.cachedAnimatableControls.length;t++){let i=this.cachedAnimatableControls[t],r=i.getValue();(this.lastAppliedSettings===null||r!==this.lastAppliedSettings[i.settingsKey])&&(e[i.settingsKey]=r)}for(let t=0;t<this.cachedAnimatableParams.length;t++){let i=this.cachedAnimatableParams[t],r=i.getValue(),o=i.parameterName;e.transformParams||(e.transformParams=this.lastAppliedSettings?.transformParams?{...this.lastAppliedSettings.transformParams}:{}),(this.lastAppliedSettings===null||r!==this.lastAppliedSettings.transformParams?.[o])&&(e.transformParams[o]=r)}if(Object.keys(e).length>0)try{this.renderer.updateConfig(e),this.lastAppliedSettings===null&&(this.lastAppliedSettings={}),Object.assign(this.lastAppliedSettings,e)}catch(t){l.error("Failed to apply animation settings:",t)}else this.lastAppliedSettings===null&&(this.lastAppliedSettings={})}_renderLoop(){if(this.isRunning){if(this.stepCounter++,this.stepCounter===1&&(this.renderer&&this.renderer.shadersJustRecompiled&&(this.renderer.shadersJustRecompiled=!1),this.alphaStepStartTime=performance.now()),this.stepCounter>=this.stepsPerIncrement){this.stepCounter=0,this.options.clearScreen&&this.renderer&&this.renderer.render(!0);let t=performance.now()-this.alphaStepStartTime;if(this._updateTimingEMA(t),this.alpha+=this.direction*this.frameCaptureAlphaIncrement,this.alpha>=1?(this.alpha=1,this.direction=-1):this.alpha<=0&&(this.alpha=0,this.direction=1),this._updateAnimatableControls(this.alpha),this.renderer&&this.renderer.setAnimationAlpha(this.alpha),this._applyChangedSettings(),this._emitAlphaChanged(this.alpha),this.options.clearParticles&&this.renderer&&this.renderer.resetParticles(),this.renderer&&(this.options.clearScreen||this.renderer.render(!0),this.options.clearScreen&&this.renderer.clearRenderBuffer(!0)),this.frameCaptureMode==="fixed"&&this._captureFrame(),this.options.smoothTiming&&this.alphaStepTimeEMA!==null&&t<this.alphaStepTimeEMA){let i=this.alphaStepTimeEMA-t;setTimeout(()=>{this.isRunning&&(this.animationFrameId=requestAnimationFrame(()=>this._renderLoop()))},i);return}}else this.renderer&&(this.options.clearScreen?this.renderer.render(!1):this.renderer.render(!0));this.isRunning&&(this.animationFrameId=requestAnimationFrame(()=>this._renderLoop()))}}_updateTimingEMA(e){if(this.alphaStepWarmupCounter<this.ALPHA_STEP_TIME_WARMUP_CYCLES?this.alphaStepWarmupCounter++:this.alphaStepTimeEMA===null?this.alphaStepTimeEMA=e:e>this.alphaStepTimeEMA?this.alphaStepTimeEMA=this.ALPHA_STEP_TIME_UPWARD*e+(1-this.ALPHA_STEP_TIME_UPWARD)*this.alphaStepTimeEMA:this.alphaStepTimeEMA=this.ALPHA_STEP_TIME_DECAY*e+(1-this.ALPHA_STEP_TIME_DECAY)*this.alphaStepTimeEMA,this.options.smoothTiming){if(this.alphaStepDisplayCounter++,this.alphaStepDisplayCounter>=this.ALPHA_STEP_TIME_DISPLAY_INTERVAL){this.alphaStepDisplayCounter=0;let t=document.getElementById("step-time-counter");t&&(this.alphaStepTimeEMA===null?t.textContent=`Step: ${e.toFixed(1)}ms (warming up...)`:t.textContent=`Step: ${e.toFixed(1)}ms (EMA: ${this.alphaStepTimeEMA.toFixed(1)}ms)`,t.style.display="block")}}else{let t=document.getElementById("step-time-counter");t&&(t.style.display="none")}}async _captureFrame(){if(this.renderer)try{let e=await this.renderer.captureRenderBuffer();this.capturedFrames.push(e),this.frameCaptureCount++,this.frameCaptureOnProgress&&this.frameCaptureOnProgress(this.frameCaptureCount,this.frameCaptureTotal,this.alpha),this.frameCaptureCount>=this.frameCaptureTotal&&(this.stop(),this.frameCaptureOnComplete&&this.frameCaptureOnComplete(this.capturedFrames))}catch(e){console.error("Failed to capture animation frame:",e),this.stop(),alert("Failed to capture animation frame. Check console for details.")}}_emitAlphaChanged(e){for(let t of this._alphaChangedListeners)t(e)}};var X={GRID:1,COUNTERS:10,CONTROLS_PANEL:200,COORDINATE_EDITOR:300,GRADIENT_PANEL:325,RENDERING_PANEL:350,MOBILE_VIEW_CONTROLS:9998,MOBILE_MENU_BAR:1e4,MENU_BAR:10001,MENU_BAR_LOWERED:1,MOBILE_PANEL:2e4,CLOSE_BUTTON:99999};function Fr(){$("#menu-bar").css("z-index",X.MENU_BAR_LOWERED)}function Rr(){$("#menu-bar").css("z-index",X.MENU_BAR)}function $e(){return window.innerWidth<=768}var Se=class{constructor(e,t={}){this.panel=$(e),this.panelSelector=e,this.closeButton=this.panel.find(".floating-panel-close, .mobile-overlay-close"),this.onShow=t.onShow,this.onHide=t.onHide,this.zIndex=t.zIndex||X.MOBILE_PANEL,this.manageMobileOnly=t.manageMobileOnly!==!1,this.panel.length||console.warn(`PanelManager: Panel not found: ${e}`)}show(){this.panel.length&&(this.panel.show(),(!this.manageMobileOnly||$e())&&(this.panel.css("z-index",this.zIndex),this.closeButton.css("z-index",X.CLOSE_BUTTON),Fr()),this.onShow&&this.onShow())}hide(){this.panel.length&&(this.panel.hide(),(!this.manageMobileOnly||$e())&&Rr(),this.onHide&&this.onHide())}toggle(){this.isVisible()?this.hide():this.show()}isVisible(){return this.panel.is(":visible")}setZIndex(e){this.zIndex=e,this.isVisible()&&(!this.manageMobileOnly||$e())&&this.panel.css("z-index",e)}getPanel(){return this.panel}wireCloseButton(){this.closeButton.off("click.panelManager").on("click.panelManager",()=>{this.hide()})}enableClickOutside(e=[]){this.clickOutsideExclusions=[this.panelSelector,...e],$(document).off("click.panelManager"+this.panelSelector),$(document).on("click.panelManager"+this.panelSelector,t=>{for(let i of this.clickOutsideExclusions)if($(t.target).is(i)||$(t.target).closest(i).length>0)return;this.isVisible()&&this.hide()}),this.panel.off("click.panelManagerStop").on("click.panelManagerStop",t=>{t.stopPropagation()})}disableClickOutside(){$(document).off("click.panelManager"+this.panelSelector),this.panel.off("click.panelManagerStop"),this.clickOutsideExclusions=null}destroy(){this.closeButton.off("click.panelManager"),this.disableClickOutside()}};function qe(n){let t=["reinhard_extended","uncharted2","hable","luminance_extended"].includes(n);$("#white-point-control").toggle(t)}function bi(n){n==="expression"?$("#expression-controls").show():$("#expression-controls").hide()}function xi(n){n==="expression"||n==="velocity_magnitude"||n==="velocity_angle"||n==="velocity_combined"?$("#gradient-button-container").show():$("#gradient-button-container").hide(),n==="expression"?$("#gradient-preset-toggle").hide():$("#gradient-preset-toggle").show()}function yi(n){n==="velocity_magnitude"||n==="velocity_combined"?($("#velocity-scaling-container").show(),$("#velocity-log-container").show()):($("#velocity-scaling-container").hide(),$("#velocity-log-container").hide())}ae();M();function Kn(n){try{let e=n.replace(/-/g,"+").replace(/_/g,"/");for(;e.length%4;)e+="=";let t=atob(e);return JSON.parse(t)}catch(e){return console.error("Failed to decode settings:",e),null}}function Zn(n){try{let e=JSON.stringify(n);return btoa(e).replace(/\+/g,"-").replace(/\//g,"_").replace(/=+$/,"")}catch(e){return console.error("Failed to encode settings:",e),null}}function Lr(){try{let n=new URLSearchParams(window.location.search),e=n.get("s"),t=null;if(e){if(t=Kn(e),t){console.log("Loaded settings from URL parameter"),localStorage.setItem("vectorFieldSettings",JSON.stringify(t)),n.delete("s");let i=n.toString(),r=window.location.pathname+(i?"?"+i:"");window.history.replaceState({},"",r),console.log("URL cleaned, settings now in localStorage")}}else{let i=localStorage.getItem("vectorFieldSettings");i&&(t=JSON.parse(i),console.log("Loaded settings from localStorage"))}return t}catch(n){return console.warn("Failed to load settings:",n),null}}function Mr(n,e){let t=n.getSettings();return e&&e.bbox&&(t.bbox={min:[...e.bbox.min],max:[...e.bbox.max]}),e&&e.coordinateSystem&&(t.coordinateSystem=e.coordinateSystem.toJSON()),localStorage.setItem("vectorFieldSettings",JSON.stringify(t)),t}function Pr(n){let e=Zn(n);if(!e){alert("Failed to encode settings");return}let t=new URL(window.location.href);t.search="";let i=new URLSearchParams(window.location.search).get("storage");i&&t.searchParams.set("storage",i),t.searchParams.set("s",e);let r=t.toString();if(navigator.clipboard&&navigator.clipboard.writeText)navigator.clipboard.writeText(r).then(()=>{let o=$("#share-url"),s=o.text();o.text("URL Copied!"),setTimeout(()=>{o.text(s)},2e3)}).catch(o=>{console.error("Failed to copy URL to clipboard:",o),alert(`Share this URL:

`+r)});else{let o=document.createElement("input");o.value=r,document.body.appendChild(o),o.select(),o.setSelectionRange(0,99999);try{let s=document.execCommand("copy");if(document.body.removeChild(o),s){let a=$("#share-url"),c=a.text();a.text("URL Copied!"),setTimeout(()=>{a.text(c)},2e3)}else throw new Error("execCommand failed")}catch{document.body.removeChild(o),alert(`Copy this URL to share:

`+r)}}}function Qn(n,e,t){if(!n||!n.coordinateSystem||!n.dimensions)return!1;try{let i=n.coordinateSystem;i.forwardTransforms&&window.UnicodeAutocomplete&&(i.forwardTransforms=i.forwardTransforms.map(o=>typeof o=="string"&&window.UnicodeAutocomplete.unicodeToAscii?window.UnicodeAutocomplete.unicodeToAscii(o):o));let r=O.fromJSON(i);if(r.dimensions===n.dimensions)return e.coordinateSystem=r,t.setCoordinateSystem(r,!1),console.log("Restored coordinate system:",r.name),!0;{console.warn(`Coordinate system dimension mismatch: system has ${r.dimensions}D but settings use ${n.dimensions}D. Resetting to Cartesian.`);let o=j(n.dimensions);return e.coordinateSystem=o,t.setCoordinateSystem(o,!1),!1}}catch(i){if(console.warn("Failed to restore coordinate system:",i),n.dimensions){let r=j(n.dimensions);e.coordinateSystem=r,t.setCoordinateSystem(r,!1)}return!1}}function eo(n){n.expressions&&Array.isArray(n.expressions)&&window.UnicodeAutocomplete&&(n.expressions=n.expressions.map(e=>typeof e=="string"&&window.UnicodeAutocomplete.unicodeToAscii?window.UnicodeAutocomplete.unicodeToAscii(e):e))}function to(n){if(n.mapperParams){let e=n.mapperParams;e.dim1!==void 0&&e.dim2!==void 0&&e.dim1===e.dim2&&(console.warn("Invalid mapper params: dim1 and dim2 are the same, fixing to default"),n.mapperParams={dim1:0,dim2:1})}}function io(n,e){if(!e)return n;let t=e.width/e.height,i=n.max[0]-n.min[0],r=n.max[1]-n.min[1],o=i/r,s=(n.min[0]+n.max[0])/2,a=(n.min[1]+n.max[1])/2,c=i,u=r;return t>o?c=r*t:u=i/t,{min:[s-c/2,a-u/2],max:[s+c/2,a+u/2]}}function Ir(n,e,t,i){if(!n){i&&i();return}let r=e.get("dimension-inputs");r&&Qn(n,t,r),eo(n),to(n),e.webComponentRegistry?e.webComponentRegistry.applyWhenReady(n).then(()=>{i&&i()}):(e.applySettings(n),e.apply(),i&&i())}function kr(n,e){if(n&&n.bbox&&e){let t=e.gl?.canvas,i=io(n.bbox,t);e.updateConfig({bbox:i,reinitializeParticles:!1}),l.verbose("Restored bbox expanded for aspect ratio")}}ae();M();var wi="customPresets";function Dr(){let n={colorMode:"velocity_angle",integratorType:"implicit-euler",solutionMethod:"fixed-point",supersampleFactor:2,smaaEnabled:!0,"smaa-intensity":1,"smaa-threshold":0,bilateralEnabled:!0,bilateralSpatialSigma:5,bilateralIntensitySigma:.5,tonemapOperator:"luminance_extended",exposure:.005,gamma:.7803178779033633,luminanceGamma:1.2188608780748513,highlightCompression:9.999999999999993,compressionThreshold:100000.00000000001,whitePoint:3.443499307633384,particleIntensity:47.31512589614813,particleSize:.10000000000000002,particleRenderMode:"points",colorSaturation:.429,brightnessDesaturation:.685,brightnessSaturation:.13,frameLimit:300,frameLimitEnabled:!0};window.presets={"2d_rotation":Object.assign({dimensions:2,expressions:["-y","x"],timestep:.001,implicitIterations:3,particleCount:5e5,fadeOpacity:.9999,dropProbability:.03,name:"Simple Rotation"},n),"2d_vortex":Object.assign({dimensions:2,expressions:["-y + x*(1 - x*x - y*y)","x + y*(1 - x*x - y*y)"],timestep:.001,implicitIterations:3,particleCount:5e5,fadeOpacity:.9999,dropProbability:.03,bbox:{min:[-3,-3],max:[3,3]},name:"Vortex"},n),"2d_vanderpol":Object.assign({dimensions:2,expressions:["y","(1 - x*x)*y - x"],timestep:.001,implicitIterations:3,particleCount:5e5,fadeOpacity:.9999,dropProbability:.03,name:"Van der Pol Oscillator"},n),"3d_lorenz":Object.assign({dimensions:3,expressions:["10*(y - x)","x*(28 - z) - y","x*y - 2.67*z"],timestep:.001,implicitIterations:3,particleCount:5e5,fadeOpacity:.9999,dropProbability:.03,name:"Lorenz Attractor"},n),"3d_rossler":Object.assign({dimensions:3,expressions:["-y - z","x + 0.2*y","0.2 + z*(x - 5.7)"],timestep:.001,implicitIterations:3,particleCount:5e5,fadeOpacity:.9999,dropProbability:.03,name:"R\xF6ssler Attractor"},n),"4d_hypersphere":Object.assign({dimensions:4,expressions:["-y","x","-w","z"],timestep:.001,implicitIterations:3,particleCount:5e5,fadeOpacity:.9999,dropProbability:.03,name:"4D Hypersphere Rotation"},n),"2d_fluid_stirring":Object.assign({dimensions:2,expressions:["-y + sin(x)*cos(y)*3.0 - 0.1*x","x + cos(x)*sin(y)*3.0 - 0.1*y"],timestep:.001,implicitIterations:3,particleCount:5e5,fadeOpacity:.9999,dropProbability:.03,name:"Simplified Fluid with Stirring"},n),"4d_double_pendulum":Object.assign({dimensions:4,expressions:["z","w","(-10*(2*sin(x) + sin(x - 2*y)) - 2*sin(x - y)*(w*w + z*z*cos(x - y))) / (3 - cos(2*x - 2*y))","(2*sin(x - y)*(2*z*z + 20*cos(x) + w*w*cos(x - y))) / (3 - cos(2*x - 2*y))"],timestep:.005,implicitIterations:3,particleCount:5e5,fadeOpacity:.9999,dropProbability:.03,mapperType:"custom",mapperParams:{horizontalExpr:"sin(x) + sin(y)",verticalExpr:"-cos(x) - cos(y)",depthExpr:""},name:"Double Pendulum (Chaotic)"},n),"2d_strange_attractor":Object.assign({dimensions:2,expressions:["-y + x*(1 - x*x*x*x - y*y*y*y)","x + y*(1 - x*x*x*x - y*y*y*y)"],bbox:{min:[-3.5719104227544065,-1.7259582784950003],max:[3.5719104227544065,1.7259582784950003]},timestep:.472,implicitIterations:3,particleCount:880100,fadeOpacity:.9999,dropProbability:.03,name:"Strange Attractor (Chaotic Limit Cycle)"},n)}}function ro(n,e){if(!e)return n;let t=e.width/e.height,i=n.max[0]-n.min[0],r=n.max[1]-n.min[1],o=i/r,s=(n.min[0]+n.max[0])/2,a=(n.min[1]+n.max[1])/2,c=i,u=r;return t>o?c=r*t:u=i/t,{min:[s-c/2,a-u/2],max:[s+c/2,a+u/2]}}function At(n,e){let t=window.presets[n];if(t||(t=We()[n]),!t){console.error("Preset not found:",n),l.error(`Preset not found: ${n}`);return}if(l.info(`Loading preset: ${t.name||n} (dimensions=${t.dimensions}, expressions=${t.expressions.length})`),e.resetAll(),t.dimensions){let c;t.coordinateSystem?c=O.fromJSON(t.coordinateSystem):c=j(t.dimensions),window.renderer&&(window.renderer.coordinateSystem=c);let u=e.get("dimension-inputs");u&&u.setCoordinateSystem(c),l.info(`Preset loaded with coordinate system: ${c.name}`)}e.webComponentRegistry?e.webComponentRegistry.applyWhenReady(t):(e.applySettings(t),e.apply());let i=e.get("dimension-inputs");i&&t.dimensions&&i.updateInputs(t.dimensions);let r=e.get("mapper-params");r&&r.updateControls();let o=e.get("transform-params");o&&o.updateControls();let s=e.getSettings();if(t.bbox){let c=window.renderer?.gl?.canvas,u=ro(t.bbox,c);s.bbox=u;let d=t.bbox.max[0]-t.bbox.min[0],h=t.bbox.max[1]-t.bbox.min[1],p=u.max[0]-u.min[0],m=u.max[1]-u.min[1];c?l.info(`Preset bbox expanded from [${d.toFixed(2)} x ${h.toFixed(2)}] to [${p.toFixed(2)} x ${m.toFixed(2)}] for aspect ratio ${(c.width/c.height).toFixed(2)}`):l.info(`Preset bbox: min=${t.bbox.min}, max=${t.bbox.max}`)}s.implicitIterations!==void 0&&(s.integratorParams={iterations:s.implicitIterations});let a=s.expressions;if(delete s.expressions,window.renderer&&window.renderer.updateConfig(s),a&&a.length>0&&window.renderer&&window.notebook)try{new le().executeAutomated(a,window.notebook,window.renderer),l.info(`Applied ${a.length} field equations via workflow`)}catch(c){l.error("Failed to apply field equations from preset:",c.message)}l.info(`Preset ${t.name||n} loaded successfully`)}function We(){try{let n=localStorage.getItem(wi);return n?JSON.parse(n):{}}catch(n){return console.error("Failed to load custom presets:",n),{}}}function no(n,e){let t=We();t[n]=e;try{localStorage.setItem(wi,JSON.stringify(t))}catch(i){console.error("Failed to save custom preset:",i),alert("Failed to save preset: "+i.message)}}function oo(n){let e=We();delete e[n];try{localStorage.setItem(wi,JSON.stringify(e))}catch(t){console.error("Failed to delete custom preset:",t)}}function vi(){let n=We(),e=$("#custom-presets-group");e.empty();let t=Object.keys(n).sort();t.length>0?(e.show(),t.forEach(i=>{let o=n[i].name||i;e.append(`<option value="${i}">${o}</option>`)})):e.hide()}function Br(n){$("#preset-selector").on("change",function(){let e=$(this).val();e&&(At(e,n),$(this).val(""),We()[e]?($("#preset-name-input").val(e),$("#delete-preset-btn").show()):($("#preset-name-input").val(""),$("#delete-preset-btn").hide()))}),$("#save-preset-btn").on("click",function(){let e=$("#preset-name-input").val().trim();if(!e){alert("Please enter a preset name");return}let i={...n.getSettings(),name:e};no(e,i),vi(),$("#delete-preset-btn").show(),l.info("Preset saved: "+e)}),$("#delete-preset-btn").on("click",function(){let e=$("#preset-name-input").val().trim();e&&confirm(`Delete preset "${e}"?`)&&(oo(e),vi(),$("#preset-name-input").val(""),$("#delete-preset-btn").hide(),l.info("Preset deleted: "+e))}),vi()}function so(n){n==="expression"||n==="velocity_magnitude"||n==="velocity_angle"||n==="velocity_combined"?$("#gradient-button-container").show():$("#gradient-button-container").hide(),n==="expression"?$("#gradient-preset-toggle").hide():$("#gradient-preset-toggle").show()}function Vr(n,e){let t=null,i=new Se("#gradient-panel",{zIndex:X.GRADIENT_PANEL,onShow:()=>{so(n.get("color-mode").getValue()),t?t.setGradient(e.getValue()):(t=wr("main-gradient-editor",e.getValue(),s=>{e.notifyChange(s)}),e.setGradientEditor(t))}});function r(){i.show()}function o(){i.hide()}return $("#open-gradient-editor").on("click",function(){r()}),$("#reset-gradient").on("click",function(){let s=ve();t&&(t.setGradient(s),e.notifyChange(s))}),i.enableClickOutside(["#open-gradient-editor"]),{showGradientPanel:r,hideGradientPanel:o,gradientEditor:t}}function Nr(n){let e=new Se("#rendering-panel",{zIndex:X.RENDERING_PANEL,onShow:()=>{qe(n.get("tonemap-operator").getValue())}});function t(){e.show()}function i(){e.hide()}return $("#open-rendering-settings").on("click",function(){t()}),$("#rendering-panel .floating-panel-close").on("click",function(){window.mobilePanelManager&&window.mobilePanelManager.getCurrentPanel()==="rendering"?window.mobilePanelManager.hidePanel("rendering"):i()}),e.enableClickOutside(["#open-rendering-settings","#mobile-menu-rendering"]),{showRenderingPanel:t,hideRenderingPanel:i,renderingPanelManager:e}}M();function zr(n,e,t){let i=new Ct(e,n);window.animationController=i;let r=document.getElementById("animation-alpha"),o=document.getElementById("animation-speed");r&&(r.setController(i),t.register("animation-alpha","animation-alpha"),l.info("Animation alpha web component registered")),o&&(o.setController(i),l.info("Animation speed web component registered"));let s=document.getElementById("frame-limit"),a=document.getElementById("frame-limit-enabled"),c=document.getElementById("sync-steps-to-frame-limit");function u(){try{if(console.log("syncStepsToFrameLimit called"),typeof c?.getValue!="function"){console.log("bail: syncCheckbox no getValue");return}if(!c.getValue()){console.log("bail: syncCheckbox not checked");return}if(typeof o?.getValue!="function"){console.log("bail: animationSpeed no getValue");return}if(typeof s?.setValue!="function"){console.log("bail: frameLimit no setValue");return}let y=o.getValue();console.log("syncing steps:",y),s.setValue(y),a&&!a.getValue()&&a.setValue(!0),window.renderer&&(window.renderer.frameLimit=y,window.renderer.frameLimitEnabled=!0,window.renderer.totalFrames=0,window.renderer.clearRenderBuffer(),window.renderer.resetParticles(),window.renderer.isRunning||window.renderer.start())}catch(y){console.warn("syncStepsToFrameLimit error:",y)}}c?.addEventListener("change",function(){try{typeof c.getValue=="function"&&c.getValue()&&u()}catch(y){console.warn("sync checkbox change error:",y)}}),o&&(o.onChange=u),c&&t.register("check-box","sync-steps-to-frame-limit");let d=document.getElementById("animation-clear-particles"),h=document.getElementById("animation-clear-screen"),p=document.getElementById("animation-smooth-timing"),m=document.getElementById("animation-lock-shaders");d&&d.addEventListener("change",function(){i.setOptions({clearParticles:d.getValue()})}),h&&h.addEventListener("change",function(){i.setOptions({clearScreen:h.getValue()})}),p&&p.addEventListener("change",function(){i.setOptions({smoothTiming:p.getValue()})}),m&&m.addEventListener("change",function(){i.setOptions({lockShaders:m.getValue()})});let f=document.getElementById("animation-half-loops");f&&f.addEventListener("change",function(){let _=f.getValue()?"Number of half cycles (1 = 0.0 \u2192 1.0, 2 = 0.0 \u2192 1.0 \u2192 0.0)":"Number of complete alpha cycles (0.0 \u2192 1.0 \u2192 0.0)";$("#animation-loops-info").text(_)});let b=document.getElementById("animation-create-btn");b&&b.addEventListener("action",function(){if(i.isRunning){i.stop(),b.setLabel("Create Animation"),b.setIcon("\u25B6"),b.setStyle("#4CAF50");let A=document.getElementById("animation-frames"),B=document.getElementById("animation-loops"),N=document.getElementById("animation-half-loops"),v=document.getElementById("animation-download-btn");A&&(A.disabled=!1),B&&(B.disabled=!1),N&&(N.disabled=!1),v&&v.setEnabled(i.getFrames().length>0),$("#progress-text").text(`Stopped: ${i.getFrames().length} frames captured`),l.info(`Animation stopped early: ${i.getFrames().length} frames captured`);return}let y=document.getElementById("animation-frames"),_=document.getElementById("animation-loops"),S=document.getElementById("animation-half-loops"),F=document.getElementById("animation-download-btn"),E=y?y.getValue():100,L=_?_.getValue():1,D=S?.getValue?S.getValue():!1;b.setLabel("Stop Animation"),b.setIcon("\u23F9"),b.setStyle("#f44336"),y&&(y.disabled=!0),_&&(_.disabled=!0),S&&(S.disabled=!0),F&&F.setEnabled(!1),$("#animation-progress").show(),$("#progress-bar").css("width","0%"),$("#progress-text").text(`Frame 0 / ${E}`),$("#progress-alpha").text("\u03B1: 0.00"),i.start({captureFrames:!0,totalFrames:E,loops:L,halfLoops:D,onProgress:(A,B,N)=>{let v=A/B*100;$("#progress-bar").css("width",`${v}%`),$("#progress-text").text(`Frame ${A} / ${B}`),$("#progress-alpha").text(`\u03B1: ${N.toFixed(2)}`)},onComplete:A=>{b.setLabel("Create Animation"),b.setIcon("\u25B6"),b.setStyle("#4CAF50"),y&&(y.disabled=!1),_&&(_.disabled=!1),S&&(S.disabled=!1),F&&F.setEnabled(!0),$("#progress-bar").css("width","100%"),$("#progress-text").text(`Complete: ${A.length} frames`),l.info(`Animation creation complete: ${A.length} frames captured`)}})});let x=document.getElementById("animation-download-btn");x&&x.addEventListener("action",async function(){let y=i.getFrames();if(y.length===0){alert("No frames to download. Create an animation first.");return}x.setLoading(!0,"Creating ZIP...");try{let S=y.length,F=Math.ceil(S/300),E=new Date().toISOString().replace(/[:.]/g,"-").slice(0,-5);l.info(`Splitting ${S} frames into ${F} ZIP file(s)`);for(let L=0;L<F;L++){let D=L*300,A=Math.min((L+1)*300,S);x.setLoading(!0,`Creating ZIP ${L+1}/${F}...`);let B=new JSZip,N=B.folder("frames");for(let R=D;R<A;R++){let P=String(R).padStart(5,"0");N.file(`frame_${P}.png`,y[R])}let v=await B.generateAsync({type:"blob"}),C=URL.createObjectURL(v),w=document.createElement("a"),T=F>1?`.${L+1}`:"";w.download=`animation-${E}${T}.zip`,w.href=C,document.body.appendChild(w),w.click(),document.body.removeChild(w),URL.revokeObjectURL(C),l.info(`Part ${L+1}/${F} downloaded: frames ${D}-${A-1}`),L<F-1&&await new Promise(R=>setTimeout(R,500))}l.info(`Animation download complete: ${S} frames in ${F} ZIP file(s)`),F>1&&alert(`Animation split into ${F} ZIP files. Use create-video.sh with the base filename (without .1, .2, etc) to reconstruct.`)}catch(_){l.error("Failed to create ZIP:",_),alert("Failed to create ZIP file: "+_.message)}finally{x.setLoading(!1)}});let g=document.getElementById("export-animation-json");return g&&g.addEventListener("action",function(){let y=n.getSettings();e&&e.bbox&&(y.bbox={min:[e.bbox.min[0],e.bbox.min[1]],max:[e.bbox.max[0],e.bbox.max[1]]}),delete y.animationAlpha;let _={name:"Untitled Animation",description:"Animation created from current settings",fps:30,baseSettings:y,timeline:[{time:0,settings:{},easing:"linear"},{time:10,settings:{},easing:"linear"}],frameConfig:{burnInSteps:5e3,clearAfterBurnIn:!0,accumulationSteps:2e3}},S=new Blob([JSON.stringify(_,null,2)],{type:"application/json"}),F=URL.createObjectURL(S),E=document.createElement("a");E.href=F,E.download="animation-template.json",document.body.appendChild(E),E.click(),document.body.removeChild(E),URL.revokeObjectURL(F),l.info("Exported animation template JSON")}),i}M();function Ur(n){let e={controls:$("#controls"),rendering:$("#rendering-panel")},t=null;function i(s){t&&r(t);let a=e[s];if(!a){l.warn(`Mobile panel not found: ${s}`);return}s==="rendering"?n.show():(a.addClass("mobile-overlay active"),a.find(".mobile-overlay-close").show(),a.css("z-index",X.MOBILE_PANEL),a.find(".mobile-overlay-close").css("z-index",X.CLOSE_BUTTON),$("#menu-bar").css("z-index",X.MENU_BAR_LOWERED)),$(`#mobile-menu-${s}`).addClass("active"),t=s}function r(s){let a=e[s];a&&(s==="rendering"?n.hide():(a.removeClass("mobile-overlay active"),a.find(".mobile-overlay-close").hide(),$("#menu-bar").css("z-index",X.MENU_BAR)),$(`#mobile-menu-${s}`).removeClass("active"),t===s&&(t=null))}function o(){Object.keys(e).forEach(r)}return $("#mobile-menu-controls").on("click",function(){t==="controls"?r("controls"):i("controls")}),$("#mobile-menu-rendering").on("click",function(){t==="rendering"?r("rendering"):i("rendering")}),$(".mobile-overlay-close").on("click",function(){let a=$(this).closest("[id]").attr("id"),u={controls:"controls"}[a];u&&r(u)}),{showPanel:i,hidePanel:r,hideAllPanels:o,getCurrentPanel:()=>t}}function Or(n,e){let t=document.getElementById("mobile-controls");if(!t){l.info("Mobile controls component not found, skipping sync setup");return}function i(){let o=n.get("timestep");o&&t.setTimestep(o.getValue())}$("#timestep").on("input change",i),t.onTimestepChange=o=>{let s=n.get("timestep");s&&(s.setValue(o),n.debouncedApply())},t.onFrameLimitEnabledChange=o=>{let s=n.get("frame-limit-enabled");s&&(s.setValue(o),n.debouncedApply())},t.onFrameLimitChange=o=>{let s=n.get("frame-limit");s&&(s.setValue(o),n.debouncedApply())};let r=n.webComponentRegistry;r?r.whenAllReady().then(()=>{setTimeout(()=>{i();let o=n.get("frame-limit-enabled"),s=n.get("frame-limit");o&&t.setFrameLimitEnabled(o.getValue()),s&&t.setFrameLimit(s.getValue())},100)}):l.warn("Web component registry not found on manager, mobile controls sync may not work properly")}function Yr(n,e){let t=null,i=new yt({renderer:n,storageKey:"vectorFieldSettings",debounceTime:300,onApply:v=>{v.implicitIterations!==void 0&&(v.integratorParams={iterations:v.implicitIterations});let C=v.dimensions;if(C!==t){let P=i.get("dimension-inputs");P&&(P.updateInputs(C),v.expressions=P.getValue());let V=i.get("mapper-params");V&&V.updateControls(),t=C}let w=v.expressions?[...v.expressions]:null,T=v.expressions;if(delete v.expressions,n.updateConfig(v),T&&T.length>0&&window.notebook)try{new le().executeAutomated(T,window.notebook,n),l.info(`Applied ${T.length} field equations via workflow`)}catch(P){l.error("Failed to apply field equations:",P.message),ao(`Failed to apply field equations: ${P.message}`);return}if(n.coordinateSystem){let P=i.get("dimension-inputs");P&&P.setCoordinateSystem(n.coordinateSystem,!1)}if(Y.isVisible()&&w&&n.coordinateSystem){let P=n.coordinateSystem.getVariableNames(),V=w.map(z=>{if(window.notebook)try{return window.notebook.expandFunctions(z)}catch(I){return l.warn(`Failed to expand "${z}" for LaTeX:`,I.message),z}return z});Y.updateEquations(V,P)}let R=i.getSettings();n&&n.bbox&&(R.bbox={min:[...n.bbox.min],max:[...n.bbox.max]}),n&&n.coordinateSystem&&(R.coordinateSystem=n.coordinateSystem.toJSON()),localStorage.setItem("vectorFieldSettings",JSON.stringify(R)),lo()}}),r=new Tt(i);i.webComponentRegistry=r,r.register("select-control","integrator");let o=document.getElementById("integrator");o&&o.addEventListener("change",()=>{let v=o.getValue(),C=v.startsWith("implicit-")||v==="trapezoidal";$("#implicit-iterations-group").toggle(C),$("#solution-method-group").toggle(C),me("#integrator",0)}),r.register("select-control","solution-method"),r.register("animatable-timestep","timestep"),r.register("animatable-slider","fade").then(v=>{v&&Promise.resolve().then(()=>(Ye(),Jr)).then(C=>{let{createFadeTransform:w}=C,{transform:T,inverseTransform:R}=w();v.setCustomTransform(T,R),v.updateBoundsDisplay&&v.updateBoundsDisplay()})}),r.register("select-control","transform");let s=document.getElementById("transform");s&&s.addEventListener("change",()=>{let v=i.get("transform-params");v&&v.updateControls()}),i.register(new Et({},{settingsKey:"transformParams"})).setTransformControl(s),r.register("select-control","mapper");let c=document.getElementById("mapper");c&&c.addEventListener("change",()=>{let v=i.get("mapper-params");v&&v.updateControls()});let u=i.register(new wt({dim1:0,dim2:1},{settingsKey:"mapperParams"})),d=document.getElementById("dimensions");u.setRelatedControls(d,c),r.register("select-control","color-mode");let h=document.getElementById("color-mode");h&&h.addEventListener("change",()=>{let v=h.getValue();bi(v),xi(v),yi(v)});let p=i.register(new xt("color-expression","x * y",{settingsKey:"colorExpression"})),m=i.register(new _t(ve(),{settingsKey:"colorGradient"}));r.register("select-control","velocity-scale-mode");let f=i.register(new vt(["-y","x"],{settingsKey:"expressions"}));r.register("select-control","tonemap-operator");let b=document.getElementById("tonemap-operator");b&&b.addEventListener("change",()=>{let v=b.getValue();qe(v)}),r.register("select-control","particle-render-mode"),r.register("linear-slider","dimensions").then(v=>{v&&(v.onChange=C=>{l.verbose("Dimensions changed to:",C);let w=i.get("dimension-inputs");w&&(l.verbose("Updating dimension inputs with dimensions:",C),w.updateInputs(C))})}),r.register("linear-slider","implicit-iterations"),r.register("linear-slider","particles"),r.register("linear-slider","drop"),r.register("linear-slider","supersample-factor"),r.register("linear-slider","bilateral-spatial"),r.register("linear-slider","bilateral-intensity"),r.register("log-slider","exposure"),r.register("log-slider","gamma"),r.register("log-slider","luminance-gamma"),r.register("log-slider","highlight-compression"),r.register("log-slider","compression-threshold"),r.register("log-slider","white-point"),r.register("log-slider","particle-intensity"),r.register("log-slider","particle-size"),r.register("log-slider","frame-limit").then(v=>{v&&(v.onChange=C=>{window.renderer&&(window.renderer.frameLimit=C)})}),r.register("percent-slider","smaa-intensity"),r.register("percent-slider","smaa-threshold"),r.register("percent-slider","color-saturation"),r.register("percent-slider","brightness-desat"),r.register("percent-slider","saturation-buildup"),r.register("percent-slider","respawn-margin"),r.register("check-box","velocity-log-scale"),r.register("check-box","show-grid"),r.register("check-box","show-equations"),r.register("check-box","frame-limit-enabled"),r.register("check-box","use-hdr"),r.register("check-box","smaa-enabled"),r.register("check-box","bilateral-enabled"),r.register("check-box","animation-clear-particles"),r.register("check-box","animation-clear-screen"),r.register("check-box","animation-smooth-timing"),r.register("check-box","animation-lock-shaders"),r.register("check-box","animation-half-loops");let x=document.getElementById("frame-limit-enabled");x&&x.addEventListener("change",()=>{window.renderer&&(window.renderer.frameLimitEnabled=x.getValue())});let g=document.getElementById("show-equations");if(g){let v=async()=>{let C=g.getValue();Y.setVisible(C),C&&await Y.refresh()};g.addEventListener("change",v)}r.register("select-control","theme-selector");let y=document.getElementById("theme-selector");y&&y.addEventListener("change",()=>{y.getValue()==="light"?$("body").addClass("light-theme"):$("body").removeClass("light-theme"),i.saveToStorage()}),$("#default-settings").on("click",function(){i.resetAll(),i.clearStorage(),i.apply()}),$("#share-url").on("click",function(){let v=i.getSettings();Pr(v)}),$("#reset").on("click",function(){let v=n.gl.canvas,T=10*(v.width/v.height);n.updateConfig({bbox:{min:[-T/2,-5],max:[T/2,5]},reinitializeParticles:!0})}),$("#reset-particles").on("click",function(){n.clearScreen()}),$("#save-image").on("click",async function(){try{let v=await n.captureRenderBuffer(),C=URL.createObjectURL(v),w=document.createElement("a"),T=new Date().toISOString().replace(/[:.]/g,"-").slice(0,-5),R=`${n.renderWidth}x${n.renderHeight}`;w.download=`vector-field-${T}-${R}.png`,w.href=C,document.body.appendChild(w),w.click(),document.body.removeChild(w),URL.revokeObjectURL(C),l.info("Image saved: "+w.download)}catch(v){l.error("Failed to save image:",v),alert("Failed to save image. Check console for details.")}});let S=new URLSearchParams(window.location.search).get("storage")||"float";$("#storage-strategy").val(S),$("#storage-strategy").on("change",function(){let v=$(this).val(),C=i.getSettings();n&&n.bbox&&(C.bbox={min:[...n.bbox.min],max:[...n.bbox.max]}),n&&n.coordinateSystem&&(C.coordinateSystem=n.coordinateSystem.toJSON()),localStorage.setItem("vectorFieldSettings",JSON.stringify(C));let w=new URL(window.location);w.searchParams.set("storage",v),window.location.href=w.toString()}),$(document).on("click",".slider-btn",function(){let v=$(this).data("slider"),C=$(this).data("action");if(!v){console.warn("Slider button missing data-slider attribute");return}let w=i.get(v);if(w&&w.handleButtonAction){w.handleButtonAction(C);return}if(v.startsWith("transform-param-")){let T=i.get("transform-params");T&&T.handleTransformParamButton&&T.handleTransformParamButton(v,C);return}console.warn(`No button handler found for slider: ${v}`)}),$("#configure-coordinates").on("click",function(){let v=n.dimensions,C=n.coordinateSystem;(!C||C.dimensions!==v)&&(console.warn("Coordinate system dimension mismatch, resetting to Cartesian"),C=j(v),n.coordinateSystem=C),$r(v,C,w=>{n.updateConfig({coordinateSystem:w}),f.setCoordinateSystem(w),i.apply()})});let F=Vr(i,m),E=Nr(i),L=zr(i,n,r);Br(i),Dr();let D=Ur(E.renderingPanelManager);window.mobilePanelManager=D,Or(i,r),$(document).on("click","#menu-settings",function(){window.appModal&&window.appModal.show()}),$(document).on("click","#menu-docs",function(){window.appModal&&window.appModal.show("docs")}),$(document).on("click","#menu-notebook",function(){window.notebookEditor&&window.notebookEditor.open()}),$(document).on("keydown",function(v){(v.ctrlKey||v.metaKey)&&v.key===","&&(v.preventDefault(),window.appModal&&window.appModal.show())}),$("#animation-section-toggle").on("click",function(){let v=$("#animation-section-content"),C=$("#animation-section-arrow"),w=$("#histogram-panel"),T=$("#cursor-position");v.is(":visible")?(v.slideUp(200),C.text("\u25BC"),w.removeClass("shifted"),T.removeClass("shifted")):(v.slideDown(200),C.text("\u25B2"),w.addClass("shifted"),T.addClass("shifted"))}),i.initializeControls(),Y.initialize("equation-overlay");let A=Lr();if($e()&&(!A||A.frameLimitEnabled===void 0&&A.frameLimit===void 0)){x&&x.setValue(!0);let v=document.getElementById("frame-limit");v&&v.setValue(200),window.renderer&&(window.renderer.frameLimitEnabled=!0,window.renderer.frameLimit=200),l.info("Mobile detected: frame limit enabled at 200 frames")}r.whenAllReady().then(()=>{if(A&&(Ir(A,i,n),A.expressions&&Array.isArray(A.expressions)&&window.UnicodeAutocomplete&&(A.expressions=A.expressions.map(v=>typeof v=="string"&&window.UnicodeAutocomplete.unicodeToAscii?window.UnicodeAutocomplete.unicodeToAscii(v):v)),A.mapperParams)){let v=A.mapperParams;v.dim1!==void 0&&v.dim2!==void 0&&v.dim1===v.dim2&&(console.warn("Invalid mapper params: dim1 and dim2 are the same, fixing to default"),A.mapperParams={dim1:0,dim2:1})}r.applyWhenReady(A).then(()=>{let v=document.getElementById("animation-clear-particles"),C=document.getElementById("animation-clear-screen"),w=document.getElementById("animation-smooth-timing"),T=document.getElementById("animation-lock-shaders");if(L.setOptions({clearParticles:v?.getValue?v.getValue():!1,clearScreen:C?.getValue?C.getValue():!1,smoothTiming:w?.getValue?w.getValue():!1,lockShaders:T?.getValue?T.getValue():!1}),n){n.frameLimitEnabled=x?.getValue?x.getValue():!1;let I=document.getElementById("frame-limit");I&&(n.frameLimit=I.getValue())}qe(i.get("tonemap-operator").getValue());let R=i.get("color-mode").getValue();bi(R),xi(R),yi(R);let P=i.get("integrator").getValue()||"rk2",V=P.startsWith("implicit-")||P==="trapezoidal";if($("#implicit-iterations-group").toggle(V),$("#solution-method-group").toggle(V),g?.getValue&&g.getValue()&&(Y.setVisible(!0),n&&n.coordinateSystem)){let I=n.expressions;if(I){let U=n.coordinateSystem.getVariableNames();Y.updateEquations(I,U)}}i.get("theme-selector").getValue()==="light"&&$("body").addClass("light-theme")})}),window.unicodeAutocomplete&&(window.unicodeAutocomplete.setEnabled(!0),window.unicodeAutocomplete.attachToAll('[id^="expr-"]'),window.unicodeAutocomplete.attachToAll("#mapper-expression"),window.unicodeAutocomplete.attachToAll("#color-expression"),window.unicodeAutocomplete.attachToAll("#custom-color-code"),window.unicodeAutocomplete.attachToAll("#custom-integrator-code"),console.log("Unicode autocomplete enabled for all math inputs"));function B(){if(!n)return;l.info("Syncing UI from renderer state");let v=document.getElementById("dimensions");if(v&&v.setValue&&n.dimensions!==void 0){let w=v.getValue();w!==n.dimensions&&(l.info(`Syncing dimensions: ${w} \u2192 ${n.dimensions}`),v.setValue(n.dimensions))}let C=i.get("dimension-inputs");if(C&&n.expressions&&(l.info("Syncing expressions from renderer:",n.expressions),C.setValue(n.expressions)),Y&&Y.isVisible&&Y.isVisible()&&n.expressions&&n.coordinateSystem){let w=n.coordinateSystem.getVariableNames();Y.updateEquations(n.expressions,w),l.info("Updated equation overlay with new expressions")}l.info("UI sync complete")}window.syncUIFromRenderer=B,window.manager=i,window.equationOverlay=Y;function N(){return Mr(i,n)}return e&&e({manager:i,state:i.getSettings(),saveSettings:N}),A&&A.bbox&&kr(A,n),i}function ao(n){$("#error-message").text(n).show()}function lo(){$("#error-message").hide()}M();M();var Kr={linear:n=>n,easeIn:n=>n*n,easeOut:n=>1-Math.pow(1-n,2),easeInOut:n=>n<.5?2*n*n:1-Math.pow(-2*n+2,2)/2,easeInCubic:n=>n*n*n,easeOutCubic:n=>1-Math.pow(1-n,3),easeInOutCubic:n=>n<.5?4*n*n*n:1-Math.pow(-2*n+2,3)/2,elastic:n=>{let e=2*Math.PI/3;return n===0?0:n===1?1:-Math.pow(2,10*n-10)*Math.sin((n*10-10.75)*e)},bounce:n=>n<1/2.75?7.5625*n*n:n<2/2.75?7.5625*(n-=1.5/2.75)*n+.75:n<2.5/2.75?7.5625*(n-=2.25/2.75)*n+.9375:7.5625*(n-=2.625/2.75)*n+.984375},Lt=class{constructor(e,t){this.renderer=e,this.manager=t,this.frames=[],this.isRunning=!1,this.isPaused=!1,this.script=null}loadScript(e){if(!e.baseSettings)throw new Error("Animation script missing baseSettings");if(!e.timeline||!Array.isArray(e.timeline))throw new Error("Animation script missing timeline array");if(e.timeline.length<2)throw new Error("Timeline must have at least 2 keyframes");for(let r=1;r<e.timeline.length;r++)if(e.timeline[r].time<=e.timeline[r-1].time)throw new Error("Timeline keyframes must be sorted by time");this.script={name:e.name||"Untitled Animation",description:e.description||"",fps:e.fps||30,baseSettings:e.baseSettings,timeline:e.timeline.map(r=>({time:r.time,settings:r.settings||{},easing:r.easing||"linear",convergenceSteps:r.convergenceSteps||0})),frameConfig:{burnInSteps:e.frameConfig?.burnInSteps||5e3,clearAfterBurnIn:e.frameConfig?.clearAfterBurnIn!==void 0?e.frameConfig.clearAfterBurnIn:!0,accumulationSteps:e.frameConfig?.accumulationSteps||2e3}};let t=this.script.timeline[this.script.timeline.length-1].time,i=Math.ceil(t*this.script.fps);return l.info(`Loaded animation: ${this.script.name}`),l.info(`  Duration: ${t}s @ ${this.script.fps} fps = ${i} frames`),l.info(`  Keyframes: ${this.script.timeline.length}`),this.script}getKeyframeIndices(e){let t=this.script.timeline;if(e<=t[0].time)return{prev:0,next:0,t:0};if(e>=t[t.length-1].time)return{prev:t.length-1,next:t.length-1,t:1};for(let i=1;i<t.length;i++)if(e<=t[i].time){let r=i-1,o=i,s=(e-t[r].time)/(t[o].time-t[r].time);return{prev:r,next:o,t:s}}return{prev:t.length-1,next:t.length-1,t:1}}interpolateValue(e,t,i,r="linear"){let s=(Kr[r]||Kr.linear)(i);if(typeof e=="number"&&typeof t=="number")return e+(t-e)*s;if(Array.isArray(e)&&Array.isArray(t))return e.map((a,c)=>{let u=t[c]!==void 0?t[c]:a;return typeof a=="number"?a+(u-a)*s:s<.5?a:u});if(typeof e=="object"&&e!==null&&typeof t=="object"&&t!==null){let a={...e};for(let c in t)e[c]!==void 0?a[c]=this.interpolateValue(e[c],t[c],s,r):a[c]=t[c];return a}return s<.5?e:t}getInterpolatedSettings(e){let{prev:t,next:i,t:r}=this.getKeyframeIndices(e),o=this.script.timeline;if(t===i)return{...this.script.baseSettings,...o[t].settings};let s={...this.script.baseSettings,...o[t].settings},a={...this.script.baseSettings,...o[i].settings},c=o[i].easing,u={...s};for(let d in a)s[d]!==void 0?u[d]=this.interpolateValue(s[d],a[d],r,c):u[d]=a[d];return u}getAnimationAlpha(e){let t=this.script.timeline[this.script.timeline.length-1].time;return Math.max(0,Math.min(1,e/t))}waitFrame(){return new Promise(e=>requestAnimationFrame(e))}async captureFrame(){return this.renderer.captureRenderBuffer()}async renderAnimationFrame(e,t,i){let{frameConfig:r}=this.script,o=this.getInterpolatedSettings(e),s=this.getAnimationAlpha(e);if(l.info(`Frame ${t+1}/${i}: t=${e.toFixed(3)}s, alpha=${s.toFixed(3)}`),this.manager.setSettings(o),this.renderer.setAnimationAlpha(s),await this.waitFrame(),this.renderer.clearRenderBuffer(),this.renderer.resetParticles(),r.burnInSteps>0&&(l.verbose(`  Burn-in: ${r.burnInSteps} steps`),this.renderer.step(r.burnInSteps)),r.clearAfterBurnIn&&this.renderer.clearRenderBuffer(),r.accumulationSteps>0){l.verbose(`  Accumulation: ${r.accumulationSteps} steps`);for(let c=0;c<r.accumulationSteps;c++)this.renderer.updatePositions(),this.renderer.drawParticles(),this.renderer.fadeScreen()}return this.renderer.render(),await this.waitFrame(),await this.captureFrame()}async run(e){if(this.isRunning)throw new Error("Animation already running");if(!this.script)throw new Error("No animation script loaded");this.isRunning=!0,this.isPaused=!1,this.frames=[];let t=this.script.timeline[this.script.timeline.length-1].time,i=Math.ceil(t*this.script.fps);try{l.info("Applying base settings...");let r={...this.script.baseSettings};if(delete r.coordinateSystem,this.manager.setSettings(r),this.script.baseSettings.coordinateSystem){let{CoordinateSystem:o}=await Promise.resolve().then(()=>(ae(),pr)),s=o.fromJSON(this.script.baseSettings.coordinateSystem);this.renderer.coordinateSystem=s,l.info(`Set coordinate system: ${s.name}`),this.renderer.compileShaders(),l.info("Shaders recompiled with coordinate system")}await this.waitFrame();for(let o=0;o<i;o++){for(;this.isPaused&&this.isRunning;)await new Promise(c=>setTimeout(c,100));if(!this.isRunning){l.info("Animation stopped");break}let s=o/this.script.fps,a=await this.renderAnimationFrame(s,o,i);this.frames.push({frameNum:o,time:s,blob:a,timestamp:Date.now()}),e&&e(o+1,i,s)}return l.info(`Animation complete: ${this.frames.length} frames captured`),this.frames}catch(r){throw l.error("Animation failed:",r),r}finally{this.isRunning=!1}}pause(){this.isPaused=!0}resume(){this.isPaused=!1}stop(){this.isRunning=!1,this.isPaused=!1}async downloadFrames(){if(this.frames.length===0)throw new Error("No frames to download");return window.JSZip?await this.downloadAsZip():await this.downloadIndividually()}async downloadAsZip(){let e=window.JSZip,t=new e;for(let{frameNum:s,blob:a}of this.frames){let c=`frame_${String(s).padStart(6,"0")}.png`;t.file(c,a)}let i={script:this.script,captureDate:new Date().toISOString(),frameCount:this.frames.length};t.file("metadata.json",JSON.stringify(i,null,2)),l.info("Generating ZIP archive...");let r=await t.generateAsync({type:"blob"}),o=`${this.script.name.replace(/\s+/g,"_")}.zip`;this.downloadBlob(r,o),l.info(`Downloaded ${o}`)}async downloadIndividually(){for(let{frameNum:e,blob:t}of this.frames){let i=`frame_${String(e).padStart(6,"0")}.png`;this.downloadBlob(t,i),await new Promise(r=>setTimeout(r,100))}l.info(`Downloaded ${this.frames.length} frames`)}downloadBlob(e,t){let i=URL.createObjectURL(e),r=document.createElement("a");r.href=i,r.download=t,document.body.appendChild(r),r.click(),document.body.removeChild(r),URL.revokeObjectURL(i)}};se();var oe=class{constructor(e,t){this.id=e,this.title=t,this._isRendered=!1,this._isActive=!1,this._contentElement=null}get isRendered(){return this._isRendered}get isActive(){return this._isActive}render(e){throw new Error("Tab subclass must implement render() method")}onActivate(){}onDeactivate(){}_activate(e){this._isRendered||(this._contentElement=this.render(e),this._isRendered=!0),this._contentElement&&(this._contentElement.style.display="block"),this._isActive=!0,this.onActivate()}_deactivate(){this._contentElement&&(this._contentElement.style.display="none"),this._isActive=!1,this.onDeactivate()}destroy(){this._contentElement&&this._contentElement.parentNode&&this._contentElement.parentNode.removeChild(this._contentElement),this._contentElement=null,this._isRendered=!1,this._isActive=!1}},Mt=class{constructor(){this._tabs=new Map,this._activeTab=null,this._tabBarElement=null,this._contentElement=null}register(e){if(!(e instanceof oe))throw new Error("Tab must be an instance of Tab class");if(this._tabs.has(e.id))throw new Error(`Tab with id '${e.id}' already registered`);this._tabs.set(e.id,e),this._tabBarElement&&this._addTabButton(e)}unregister(e){let t=this._tabs.get(e);if(t&&(t===this._activeTab&&(this._activeTab=null),t.destroy(),this._tabs.delete(e),this._tabBarElement)){let i=this._tabBarElement.querySelector(`[data-tab-id="${e}"]`);i&&i.remove()}}activate(e){let t=this._tabs.get(e);return t?(this._activeTab&&this._activeTab!==t&&(this._activeTab._deactivate(),this._updateTabButton(this._activeTab.id,!1)),this._activeTab=t,t._activate(this._contentElement),this._updateTabButton(e,!0),!0):(console.warn(`Tab '${e}' not found`),!1)}getActiveTab(){return this._activeTab}getAllTabs(){return Array.from(this._tabs.values())}initializeUI(e,t){this._tabBarElement=e,this._contentElement=t;for(let i of this._tabs.values())this._addTabButton(i)}_addTabButton(e){if(!this._tabBarElement)return;let t=document.createElement("button");t.className="modal-tab-button",t.textContent=e.title,t.dataset.tabId=e.id,t.addEventListener("click",()=>{this.activate(e.id)}),this._tabBarElement.appendChild(t)}_updateTabButton(e,t){if(!this._tabBarElement)return;let i=this._tabBarElement.querySelector(`[data-tab-id="${e}"]`);i&&(t?i.classList.add("active"):i.classList.remove("active"))}destroy(){for(let e of this._tabs.values())e.destroy();this._tabs.clear(),this._activeTab=null,this._tabBarElement=null,this._contentElement=null}};var Ri=class{constructor(){this.tabManager=new Mt,this._modalElement=null,this._isVisible=!1,this._onCloseCallback=null,this._wasPaused=!1}initialize(){if(this._modalElement=document.getElementById("modal-overlay"),!this._modalElement){console.error("Modal overlay element not found");return}let e=document.getElementById("modal-tab-bar"),t=document.getElementById("modal-content");if(!e||!t){console.error("Modal tab bar or content element not found");return}this.tabManager.initializeUI(e,t);let i=document.getElementById("modal-close");i&&i.addEventListener("click",()=>this.hide()),this._modalElement.addEventListener("click",r=>{r.target===this._modalElement&&this.hide()}),document.addEventListener("keydown",r=>{r.key==="Escape"&&this._isVisible&&this.hide()})}registerTab(e){this.tabManager.register(e)}show(e=null,t=null){if(!this._modalElement){console.error("Modal not initialized");return}if(!e){let i=this.tabManager.getAllTabs();i.length>0&&(e=i[0].id)}if(e&&(this.tabManager.activate(e),t)){let i=this.tabManager.getAllTabs().find(r=>r.id===e);i&&typeof i.openToSection=="function"&&(i.isRendered?i.openToSection(t):typeof i.setPendingSection=="function"&&i.setPendingSection(t))}this._modalElement.style.display="flex",this._isVisible=!0,document.body.style.overflow="hidden",window.renderer&&window.renderer.isRunning?(window.renderer.stop(),this._wasPaused=!1):this._wasPaused=!0}hide(){this._modalElement&&(this._modalElement.style.display="none",this._isVisible=!1,document.body.style.overflow="",window.renderer&&!this._wasPaused&&window.renderer.start(),this._onCloseCallback&&this._onCloseCallback())}isVisible(){return this._isVisible}onClose(e){this._onCloseCallback=e}getActiveTab(){return this.tabManager.getActiveTab()}},Fi=null;function co(){return Fi||(Fi=new Ri),Fi}function Zr(){let n=co();return n.initialize(),n}var Pt=class extends oe{constructor(){super("notebook-debug","Notebook Debug"),this.contextTextarea=null,this.refreshButton=null}render(e){let t=document.createElement("div");return t.id="notebook-debug-tab-content",t.className="modal-tab-content",t.style.display="none",t.innerHTML=`
...
`:i+=`  (Not available)
`,this.contextTextarea.value=i}catch(e){this.contextTextarea.value=`Error refreshing context: ${e.message}`,console.error("Error refreshing notebook debug context:",e)}}};var It=class extends oe{constructor(e){super("debug","Debug Console"),this.logger=e,this.debugToggle=null,this.debugVerbosity=null,this.debugOutput=null,this.bufferSizeSpan=null,this.bufferPercentSpan=null,this.bufferStatusDiv=null}render(e){let t=document.createElement("div");t.id="debug-tab-content",t.className="modal-tab-content",t.style.display="none",t.innerHTML=`
...
`}navigator.clipboard.writeText(i).then(()=>{this.logger.info("Log copied to clipboard ("+t.length+" entries)")}).catch(r=>{this.logger.error("Failed to copy log to clipboard",r)})}),e.querySelector("#debug-clear").addEventListener("click",()=>{this.logger.clear()}),e.querySelector("#debug-flush-buffer").addEventListener("click",()=>{this.logger.flush(),this._updateBufferStatus()}),e.querySelector("#debug-clear-buffer").addEventListener("click",()=>{this.logger.clearSilencedBuffer(),this._updateBufferStatus()}),e.querySelector("#debug-show-line-numbers").addEventListener("change",t=>{this.logger.setShowLineNumbers(t.target.checked),this.logger.info("Line numbers "+(t.target.checked?"enabled":"disabled")),this.logger.updateUI()}),e.querySelector("#debug-log-update-shader").addEventListener("click",()=>{window.renderer&&typeof window.renderer.logUpdateShader=="function"?window.renderer.logUpdateShader():this.logger.warn("Renderer not initialized - cannot log update shader")}),e.querySelector("#debug-log-draw-shader").addEventListener("click",()=>{window.renderer&&typeof window.renderer.logDrawShader=="function"?window.renderer.logDrawShader():this.logger.warn("Renderer not initialized - cannot log draw shader")}),e.querySelector("#debug-log-screen-shader").addEventListener("click",()=>{window.renderer&&typeof window.renderer.logScreenShader=="function"?window.renderer.logScreenShader():this.logger.warn("Renderer not initialized - cannot log screen shader")}),e.querySelector("#debug-log-stats-shaders").addEventListener("click",()=>{window.renderer&&typeof window.renderer.logStatsShaders=="function"?window.renderer.logStatsShaders():this.logger.warn("Renderer not initialized - cannot log stats shaders")}),e.querySelector("#debug-buffer-stats").addEventListener("click",()=>{window.renderer&&typeof window.renderer.logBufferStats=="function"?window.renderer.logBufferStats():this.logger.warn("Renderer not initialized - cannot log buffer stats")}),e.querySelector("#debug-enable-stats").addEventListener("change",t=>{if(window.renderer){let i=t.target.checked;window.renderer.enableDebugStats=i,this.logger.info(`GPU debug stats ${i?"ENABLED":"DISABLED"}`)}})}_updateBufferStatus(){let e=this.logger.getBufferStats();this.bufferSizeSpan.textContent=e.bufferSize,this.bufferPercentSpan.textContent=e.bufferUsagePercent,e.bufferUsagePercent>80?this.bufferSizeSpan.style.color="#EF5350":e.bufferUsagePercent>50?this.bufferSizeSpan.style.color="#FFA726":this.bufferSizeSpan.style.color="#4CAF50"}onActivate(){this._bufferUpdateInterval||(this._bufferUpdateInterval=setInterval(()=>{this.logger.isSilenced()&&this._isActive&&this._updateBufferStatus()},1e3))}onDeactivate(){this._bufferUpdateInterval&&(clearInterval(this._bufferUpdateInterval),this._bufferUpdateInterval=null)}destroy(){this._bufferUpdateInterval&&(clearInterval(this._bufferUpdateInterval),this._bufferUpdateInterval=null),super.destroy()}};var kt=class extends oe{constructor(){super("docs","Documentation"),this.sections=[],this.sectionDefinitions=[{id:"display-options",title:"Display Options & UI Controls",file:"docs/display-options.md"},{id:"vector-fields",title:"Vector Fields",file:"docs/vector-fields.md"},{id:"integrators",title:"Integration Methods",file:"docs/integrators.md"},{id:"color-modes",title:"Color Modes",file:"docs/color-modes.md"},{id:"projection",title:"2D Projection",file:"docs/projection.md"},{id:"coordinate-systems",title:"Coordinate Systems",file:"docs/coordinate-systems.md"},{id:"domain-transforms",title:"Domain Transforms",file:"docs/domain-transforms.md"},{id:"hdr-rendering",title:"HDR & Tone Mapping",file:"docs/hdr-rendering.md"},{id:"rendering-effects",title:"Rendering Effects",file:"docs/rendering-effects.md"},{id:"particle-settings",title:"Particle Settings",file:"docs/particle-settings.md"},{id:"animation",title:"Animation System",file:"docs/animation.md"},{id:"preset-management",title:"Preset Management",file:"docs/preset-management.md"},{id:"storage-strategy",title:"Storage Strategy",file:"docs/storage-strategy.md"},{id:"keyboard-shortcuts",title:"Keyboard Shortcuts",file:"docs/keyboard-shortcuts.md"}]}async _loadMarkdown(e){try{let t=await fetch(e);if(!t.ok)throw new Error(`Failed to load ${e}: ${t.statusText}`);let i=await t.text();return typeof marked>"u"?(console.error("marked.js not loaded"),"<p>Error: Markdown parser not available</p>"):marked.parse(i)}catch(t){return console.error("Error loading markdown:",t),`<p>Error loading documentation: ${t.message}</p>`}}render(e){let t=document.createElement("div");return t.id="docs-tab-content",t.className="modal-tab-content",t.style.display="none",t.innerHTML=`
        ...
        `)}attachInternalListeners(){let e=this.querySelector("#timestep-slider"),t=this.querySelector("#timestep-value"),i=this.querySelector("#timestep-decrease"),r=this.querySelector("#timestep-increase");e.addEventListener("input",()=>{let c=parseFloat(e.value);this._timestepLog=this.sliderToTimestepLog(c),this._timestep=Math.pow(10,this._timestepLog),t.textContent=this._timestep.toFixed(4),this.triggerChange(),this.onTimestepChange&&this.onTimestepChange(this._timestep)}),i.addEventListener("click",()=>{let c=Math.pow(10,-3);this._timestep=Math.max(c,this._timestep*.98),this._timestepLog=Math.log10(this._timestep),e.value=this.timestepLogToSlider(this._timestepLog),t.textContent=this._timestep.toFixed(4),this.triggerChange(),this.onTimestepChange&&this.onTimestepChange(this._timestep)}),r.addEventListener("click",()=>{let c=Math.pow(10,.4);this._timestep=Math.min(c,this._timestep*1.02),this._timestepLog=Math.log10(this._timestep),e.value=this.timestepLogToSlider(this._timestepLog),t.textContent=this._timestep.toFixed(4),this.triggerChange(),this.onTimestepChange&&this.onTimestepChange(this._timestep)});let o=this.querySelector("#frame-limit-checkbox"),s=this.querySelector("#frame-limit-slider"),a=this.querySelector("#frame-limit-value");o.addEventListener("change",()=>{this._frameLimitEnabled=o.checked,this.triggerChange(),this.onFrameLimitEnabledChange&&this.onFrameLimitEnabledChange(this._frameLimitEnabled)}),s.addEventListener("input",()=>{let c=parseFloat(s.value);this._frameLimit=this.sliderToFrame(c),a.textContent=Math.round(this._frameLimit),this.triggerChange(),this.onFrameLimitChange&&this.onFrameLimitChange(this._frameLimit)})}frameToSlider(e){return Math.log10(Math.max(1,e))/Math.log10(1e5)*100}sliderToFrame(e){return Math.round(Math.pow(10,e/100*Math.log10(1e5)))}sliderToTimestepLog(e){let o=(e- -3)/3.4;return-3+Math.pow(o,this._timestepCurve)*3.4}timestepLogToSlider(e){let o=(e- -3)/3.4;return-3+Math.pow(o,1/this._timestepCurve)*3.4}setFPS(e){if(this._fps=e,!this.shadowRoot)return;let t=this.shadowRoot.querySelector(".fps-display");t&&(t.textContent=`FPS: ${e}`)}setTimestep(e){if(this._timestep=e,this._timestepLog=Math.log10(e),!this.shadowRoot)return;let t=this.shadowRoot.querySelector("#timestep-slider"),i=this.shadowRoot.querySelector("#timestep-value");t&&(t.value=this.timestepLogToSlider(this._timestepLog)),i&&(i.textContent=e.toFixed(4))}setFrameLimitEnabled(e){if(this._frameLimitEnabled=e,!this.shadowRoot)return;let t=this.shadowRoot.querySelector("#frame-limit-checkbox");t&&(t.checked=e)}setFrameLimit(e){if(this._frameLimit=e,!this.shadowRoot)return;let t=this.shadowRoot.querySelector("#frame-limit-slider"),i=this.shadowRoot.querySelector("#frame-limit-value");t&&(t.value=this.frameToSlider(e)),i&&(i.textContent=Math.round(e))}getTimestep(){return this._timestep}getFrameLimitEnabled(){return this._frameLimitEnabled}getFrameLimit(){return this._frameLimit}};customElements.define("mobile-controls",Dt);var Qe=new Map,Bt=new Map,Li=class n extends HTMLElement{async connectedCallback(){let e=this.getAttribute("src");if(!e){console.error("HtmlInclude: src attribute is required");return}if(this.innerHTML.trim().length>0){console.debug(`Section already inline: ${e}`);return}let t=await n.loadSection(e);t&&(this.innerHTML=t)}static async loadSection(e){if(Qe.has(e))return Qe.get(e);if(Bt.has(e))return await Bt.get(e),Qe.get(e);let t=(async()=>{try{let r=await fetch(`src/ui/${e}`);if(r.ok){let o=await r.text();return Qe.set(e,o),o}}catch(r){console.error(`Failed to load section: ${e}`,r)}return Qe.set(e,null),null})();Bt.set(e,t);let i=await t;return Bt.delete(e),i}};customElements.define("html-include",Li);Ce();Ye();function Qr(){customElements.get("linear-slider")||customElements.define("linear-slider",Q),customElements.get("log-slider")||customElements.define("log-slider",Ke),customElements.get("percent-slider")||customElements.define("percent-slider",pe),customElements.get("animatable-slider")||customElements.define("animatable-slider",ue),customElements.get("animatable-timestep")||customElements.define("animatable-timestep",Ze),customElements.get("animation-alpha")||customElements.define("animation-alpha",Ae),customElements.get("animation-speed")||customElements.define("animation-speed",Fe),customElements.get("number-input")||customElements.define("number-input",Re),customElements.get("action-button")||customElements.define("action-button",Le),customElements.get("check-box")||customElements.define("check-box",Me),customElements.get("select-control")||customElements.define("select-control",Pe)}M();M();M();M();var Vt=class{constructor(e,t={}){this.cellData=e,this.callbacks=t,this.element=null,this.inputField=null,this.outputDiv=null,this.errorDiv=null,this._createUI()}_createUI(){this.element=document.createElement("div"),this.element.className="notebook-cell",this.element.dataset.cellId=this.cellData.id,this.element.style.cssText=`
            margin: 8px 0;
            display: flex;
            flex-direction: column;
            gap: 4px;
        ...
        `,this.inputField=document.createElement("input"),this.inputField.type="text",this.inputField.value=this.cellData.input||"",this.inputField.placeholder="Enter expression or function definition (e.g., f(x) = sin(x))",this.inputField.style.cssText=`
            ...
            border-radius: 4px;
            padding: 6px 8px;
        `,this.inputField.addEventListener("input",()=>{this.callbacks.onInputChange&&this.callbacks.onInputChange(this.cellData.id,this.inputField.value)}),this.inputField.addEventListener("keydown",c=>{c.key==="Enter"&&!c.shiftKey&&(c.preventDefault(),this._evaluate())});let t=this._createIconButton("\u2192","Evaluate (Enter)",()=>this._evaluate());t.style.color="#4CAF50",t.style.fontSize="18px";let i=document.createElement("div");i.style.flex="0 0 8px";let r=this._createIconButton("\u2191","Move up",()=>{this.callbacks.onMoveUp&&this.callbacks.onMoveUp(this.cellData.id)}),o=this._createIconButton("\u2193","Move down",()=>{this.callbacks.onMoveDown&&this.callbacks.onMoveDown(this.cellData.id)}),s=this._createIconButton("\u2715","Delete cell",()=>{this.callbacks.onDelete&&this.callbacks.onDelete(this.cellData.id)});s.style.color="#f44336";let a=this._createIconButton("+","Insert cell below",()=>{this.callbacks.onInsertBelow&&this.callbacks.onInsertBelow(this.cellData.id)});a.style.color="#4CAF50",e.appendChild(this.inputField),e.appendChild(t),e.appendChild(i),e.appendChild(r),e.appendChild(o),e.appendChild(s),e.appendChild(a),this.outputDiv=document.createElement("div"),this.outputDiv.className="notebook-cell-output",this.outputDiv.style.cssText=`
            display: none;
            text-align: right;
            color: #e0e0e0;
            font-size: 14px;
        ...
        `,r.addEventListener("click",i),r.addEventListener("mouseenter",()=>{r.style.background="rgba(76, 175, 80, 0.2)",r.style.borderColor="#4CAF50"}),r.addEventListener("mouseleave",()=>{r.style.background="rgba(76, 175, 80, 0.1)",r.style.borderColor="#444"}),r}_evaluate(){this.callbacks.onEvaluate&&this.callbacks.onEvaluate(this.cellData.id)}async updateOutput(e){if(this.cellData.output=e.output,this.cellData.tex=e.tex,this.cellData.error=e.error,this.outputDiv.style.display="none",this.errorDiv.style.display="none",e.error)this.errorDiv.textContent=e.error,this.errorDiv.style.display="block";else if(e.tex||e.output)if(this.outputDiv.style.display="block",e.tex)try{let t=this.element.clientWidth;l.info(`Cell width: ${t}px, using for TeX scaling`),await Cr(this.outputDiv,e.tex,t)}catch(t){l.warn("Failed to render TeX, falling back to plain text:",t),this.outputDiv.textContent=e.output||e.tex}else this.outputDiv.textContent=e.output}getElement(){return this.element}getInput(){return this.inputField.value}setInput(e){this.inputField.value=e}getId(){return this.cellData.id}focus(){this.inputField&&this.inputField.focus()}destroy(){this.element&&this.element.parentNode&&this.element.parentNode.removeChild(this.element),this.element=null,this.inputField=null,this.outputDiv=null,this.errorDiv=null}};var Nt=class{constructor(e={}){if(this.notebook=e.notebook||window.notebook,this.showExport=e.showExport!==!1,!this.notebook)throw new Error("No notebook instance provided");this.element=null,this.cellsContainer=null,this.cellComponents=new Map,this._createUI(),this._renderCells()}_createUI(){this.element=document.createElement("div"),this.element.className="notebook-repl",this.element.style.cssText=`
            display: flex;
            flex-direction: column;
            gap: 8px;
        `,this.cellsContainer=document.createElement("div"),this.cellsContainer.className="notebook-cells",this.cellsContainer.style.cssText=`
            ...
            gap: 8px;
            min-height: 50px;
        `,this.element.appendChild(this.cellsContainer)}_renderCells(){this.cellComponents.forEach(t=>t.destroy()),this.cellComponents.clear(),this.cellsContainer.innerHTML="";let e=this.notebook.cells||[];if(e.length===0){this.addCell();return}e.forEach(t=>{this._renderCell(t)})}_renderCell(e){let t=new Vt(e,{onEvaluate:i=>this.evaluateCell(i),onMoveUp:i=>this.moveCellUp(i),onMoveDown:i=>this.moveCellDown(i),onDelete:i=>this.deleteCell(i),onInsertBelow:i=>this.insertCellBelow(i),onInputChange:(i,r)=>this.updateCellInput(i,r)});this.cellComponents.set(e.id,t),this.cellsContainer.appendChild(t.getElement())}addCell(){let e=this.notebook.addCell("code","");l.info("Added new cell:",e);let t=this.notebook.cells.find(i=>i.id===e);return t&&this._renderCell(t),e}insertCellBelow(e){let t=this.notebook.cells||[],i=t.findIndex(s=>s.id===e);if(i===-1)return l.warn("Cell not found:",e),this.addCell();let r=this.notebook.addCell("code",""),o=t.findIndex(s=>s.id===r);if(o!==-1){let s=i+1;for(let a=o;a>s;a--)this.notebook.moveCellUp(r)}return l.info("Inserted new cell below:",e),this._renderCells(),setTimeout(()=>{let s=this.cellComponents.get(r);s&&s.focus()},50),r}deleteCell(e){this.notebook.deleteCell(e),l.info("Deleted cell:",e),this._renderCells()}moveCellUp(e){this.notebook.moveCellUp(e),l.verbose("Moved cell up:",e),this._renderCells()}moveCellDown(e){this.notebook.moveCellDown(e),l.verbose("Moved cell down:",e),this._renderCells()}updateCellInput(e,t){this.notebook.updateCell(e,t),l.verbose("Updated cell input:",e)}async evaluateCell(e){l.info("Evaluating cell:",e);let t=this.notebook.cells||[],r=t.findIndex(o=>o.id===e)===t.length-1;try{let o=await this.notebook.evaluateCell(e),s=this.cellComponents.get(e);if(s&&await s.updateOutput(o),l.info("Cell evaluated successfully:",e),r){let a=this.addCell();setTimeout(()=>{let c=this.cellComponents.get(a);c&&c.focus()},50)}}catch(o){l.error("Error evaluating cell:",o);let s=this.cellComponents.get(e);s&&await s.updateOutput({output:null,tex:null,error:o.message})}}async evaluateAll(){l.info("Evaluating all cells");try{await this.notebook.evaluateAll();for(let[e,t]of this.cellComponents.entries()){let i=this.notebook.cells.find(r=>r.id===e);i&&await t.updateOutput(i)}l.info("All cells evaluated successfully")}catch(e){l.error("Error evaluating all cells:",e)}}exportNotebook(){try{this.notebook.exportJSON("notebook.json"),l.info("Exported notebook")}catch(e){l.error("Failed to export notebook:",e),alert(`Export failed: ${e.message}`)}}importNotebook(){let e=document.createElement("input");e.type="file",e.accept=".json",e.addEventListener("change",async t=>{let i=t.target.files[0];if(i)try{await this.notebook.importJSON(i),l.info("Imported notebook"),this._renderCells()}catch(r){l.error("Failed to import notebook:",r),alert(`Import failed: ${r.message}`)}}),e.click()}refresh(){this._renderCells()}cleanupEmptyCells(){let t=(this.notebook.cells||[]).filter(i=>!i.input||!i.input.trim());t.length>0&&(l.info(`Cleaning up ${t.length} empty cells`),t.forEach(i=>{this.notebook.deleteCell(i.id)}),this._renderCells())}getElement(){return this.element}destroy(){this.cellComponents.forEach(e=>e.destroy()),this.cellComponents.clear(),this.element&&this.element.parentNode&&this.element.parentNode.removeChild(this.element),this.element=null,this.cellsContainer=null}};var Ie=class{constructor(e){this.config={title:e.title||"Notebook Editor",customContent:e.customContent||null,onApply:e.onApply||(()=>({success:!0})),onCancel:e.onCancel||null,applyButtonText:e.applyButtonText||"Apply",showNotebook:e.showNotebook!==!1,showExport:e.showExport!==!1},this.overlay=null,this.modalContainer=null,this.notebookRepl=null,this.customContentContainer=null,this.errorContainer=null,this.isVisible=!1,this._createUI()}_createUI(){this.overlay=document.createElement("div"),this.overlay.className="notebook-modal-overlay",this.overlay.addEventListener("click",u=>{u.target===this.overlay&&this.hide()}),this.modalContainer=document.createElement("div"),this.modalContainer.className="notebook-modal-container";let e=document.createElement("div");e.className="notebook-modal-header";let t=document.createElement("h2");t.className="notebook-modal-title",t.textContent=this.config.title;let i=document.createElement("div");if(i.style.cssText="display: flex; gap: 8px; align-items: center;",this.config.showExport){let u=document.createElement("button");u.className="notebook-modal-header-button",u.textContent="Export",u.title="Export notebook as JSON",u.addEventListener("click",()=>{this.notebookRepl&&this.notebookRepl.exportNotebook()});let d=document.createElement("button");d.className="notebook-modal-header-button",d.textContent="Import",d.title="Import notebook from JSON",d.addEventListener("click",()=>{this.notebookRepl&&this.notebookRepl.importNotebook()}),i.appendChild(u),i.appendChild(d)}let r=document.createElement("button");r.className="notebook-modal-close",r.textContent="\xD7",r.title="Close",r.addEventListener("click",()=>this.hide()),i.appendChild(r),e.appendChild(t),e.appendChild(i);let o=document.createElement("div");if(o.className="notebook-modal-content",this.customContentContainer=document.createElement("div"),this.customContentContainer.className="notebook-modal-custom-content",this.config.customContent&&(typeof this.config.customContent=="string"?this.customContentContainer.innerHTML=this.config.customContent:this.config.customContent instanceof HTMLElement&&this.customContentContainer.appendChild(this.config.customContent),o.appendChild(this.customContentContainer)),this.config.showNotebook){let u=document.createElement("div");u.className="notebook-modal-notebook-section",this.notebookRepl=new Nt({showExport:!1}),u.appendChild(this.notebookRepl.getElement()),o.appendChild(u)}this.errorContainer=document.createElement("div"),this.errorContainer.className="notebook-modal-error";let s=document.createElement("div");s.className="notebook-modal-footer";let a=document.createElement("button");a.className="notebook-modal-button-cancel",a.textContent="Cancel",a.addEventListener("click",()=>this.hide());let c=document.createElement("button");c.className="notebook-modal-button-apply",c.textContent=this.config.applyButtonText,c.addEventListener("click",()=>this._handleApply()),s.appendChild(a),s.appendChild(c),this.modalContainer.appendChild(e),this.modalContainer.appendChild(o),this.modalContainer.appendChild(this.errorContainer),this.modalContainer.appendChild(s),this.overlay.appendChild(this.modalContainer),document.body.appendChild(this.overlay),this._escapeHandler=u=>{u.key==="Escape"&&this.isVisible&&this.hide()},document.addEventListener("keydown",this._escapeHandler)}async _handleApply(){try{this.notebookRepl&&this.notebookRepl.cleanupEmptyCells();let e=this._gatherCustomFields(),t=await this.config.onApply(window.notebook,e);t.success?(l.info("Modal apply successful"),this.hide()):(alert(`Error: ${t.error||"Unknown error"}`),l.error("Modal apply failed:",t.error))}catch(e){alert(`Error: ${e.message}`),l.error("Modal apply error:",e)}}_gatherCustomFields(){let e={};return this.customContentContainer&&this.customContentContainer.querySelectorAll("input, textarea, select").forEach(i=>{i.id&&(i.type==="checkbox"?e[i.id]=i.checked:e[i.id]=i.value)}),e}show(){this.overlay.style.display="block",this.isVisible=!0,this.hideError(),this.notebookRepl&&this.notebookRepl.refresh(),l.info("Opened notebook modal:",this.config.title)}hide(){this.notebookRepl&&this.notebookRepl.cleanupEmptyCells(),this.overlay.style.display="none",this.isVisible=!1,this.config.onCancel&&this.config.onCancel(),l.info("Closed notebook modal")}isOpen(){return this.isVisible}getCustomContentContainer(){return this.customContentContainer}getNotebookREPL(){return this.notebookRepl}showError(e){this.errorContainer.innerHTML=e,this.errorContainer.style.display="block"}hideError(){this.errorContainer.style.display="none",this.errorContainer.innerHTML=""}destroy(){document.removeEventListener("keydown",this._escapeHandler),this.notebookRepl&&this.notebookRepl.destroy(),this.overlay&&this.overlay.parentNode&&this.overlay.parentNode.removeChild(this.overlay),this.overlay=null,this.modalContainer=null,this.notebookRepl=null,this.customContentContainer=null}};var Mi=class{constructor(){this.modal=null,this._initialized=!1}_initialize(){this._initialized||(this.modal=new Ie({title:"Notebook",customContent:null,applyButtonText:"Close",showNotebook:!0,showExport:!0,onApply:()=>({success:!0})}),this._initialized=!0,l.info("Notebook editor initialized"))}_createInfoSection(){let e=document.createElement("div");return e.style.cssText="margin-bottom: 16px; font-size: 13px; color: #e0e0e0;",e.innerHTML=`
        ...
        `,e}open(){this._initialize(),this.modal.show()}isOpen(){return this.modal&&this.modal.isOpen()}getModal(){return this.modal}},en=new Mi;var zt=class{constructor(){this._ready=!1}async initialize(){throw new Error("initialize() must be implemented by subclass")}isReady(){return this._ready}parse(e){throw new Error("parse() must be implemented by subclass")}evaluate(e){throw new Error("evaluate() must be implemented by subclass")}differentiate(e,t){throw new Error("differentiate() must be implemented by subclass")}simplify(e){throw new Error("simplify() must be implemented by subclass")}solve(e,t){throw new Error("solve() must be implemented by subclass")}invertMatrix(e){throw new Error("invertMatrix() must be implemented by subclass")}matrixElement(e,t,i){throw new Error("matrixElement() must be implemented by subclass")}setVariable(e,t){throw new Error("setVariable() must be implemented by subclass")}toGLSL(e){throw new Error("toGLSL() must be implemented by subclass")}toTeX(e){throw new Error("toTeX() must be implemented by subclass")}clearCache(){throw new Error("clearCache() must be implemented by subclass")}toString(e){throw new Error("toString() must be implemented by subclass")}getName(){throw new Error("getName() must be implemented by subclass")}getCapabilities(){throw new Error("getCapabilities() must be implemented by subclass")}getVersion(){return"unknown"}};M();function tn(n){return n=n.replace(/([a-z]+)\^(\d+)/g,(e,t,i)=>{let r=parseInt(i);return r===0?"1":r===1?t:Array(r).fill(t).join("*")}),n=n.replace(/pow\(([a-z]+),\s*(\d+)\)/g,(e,t,i)=>{let r=parseInt(i);return r===0?"1":r===1?t:r<=4?Array(r).fill(t).join("*"):e}),n}var Ut=class extends zt{constructor(){super()}async initialize(){if(!window.nerdamer)throw new Error("Nerdamer not loaded - ensure nerdamer CDN script is included in index.html");if(typeof window.nerdamer!="function")throw new Error("Nerdamer is not a function - invalid state");if(!window.nerdamer.diff)throw new Error("Nerdamer.diff not available - incomplete initialization");this._ready=!0,l.info("NerdamerEngine initialized successfully")}parse(e){if(!this._ready)throw new Error("NerdamerEngine not initialized");return window.nerdamer(e)}evaluate(e){if(!this._ready)throw new Error("NerdamerEngine not initialized");try{let i=window.nerdamer(e).toString(),r=window.nerdamer.convertToLaTeX(i);return r=this._cleanupTeX(r),{result:i,tex:r}}catch(t){return{result:"",tex:"",error:t.message}}}differentiate(e,t){if(!this._ready)throw new Error("NerdamerEngine not initialized");try{let i=window.nerdamer(e),o=window.nerdamer.diff(i,t).toString();return tn(o)}catch(i){return l.warn(`Failed to differentiate "${e}" w.r.t. ${t}:`,i.message),"0"}}simplify(e){if(!this._ready)throw new Error("NerdamerEngine not initialized");try{return window.nerdamer(`simplify(${e})`).toString()}catch(t){return l.warn(`Failed to simplify "${e}":`,t.message),e}}solve(e,t){if(!this._ready)throw new Error("NerdamerEngine not initialized");try{let i=window.nerdamer.solve(e,t);return i?i.toString():null}catch(i){return l.warn(`Failed to solve "${e}" for ${t}:`,i.message),null}}invertMatrix(e){if(!this._ready)throw new Error("NerdamerEngine not initialized");let t=e.length;if(t<2||t>4)return l.error(`Matrix inversion only supported for 2x2, 3x3, and 4x4 matrices (got ${t}x${t})`),null;try{let i=e.map(o=>`[${o.join(",")}]`).join(",");l.verbose("Inverting matrix:",`matrix(${i})`),window.nerdamer.setVar("J_temp",`matrix(${i})`),window.nerdamer.setVar("J_inv","invert(J_temp)");let r=[];for(let o=0;o<t;o++){let s=[];for(let a=0;a<t;a++){let c=window.nerdamer(`matget(J_inv, ${o}, ${a})`).toString(),u=tn(c);s.push(u)}r.push(s)}return l.verbose("Inverted matrix successfully"),r}catch(i){return l.error("Failed to invert matrix:",i.message),null}}matrixElement(e,t,i){if(!this._ready)throw new Error("NerdamerEngine not initialized");try{return window.nerdamer(`matget(${e}, ${t}, ${i})`).toString()}catch(r){return l.error(`Failed to get matrix element [${t},${i}]:`,r.message),"0"}}setVariable(e,t){if(!this._ready)throw new Error("NerdamerEngine not initialized");window.nerdamer.setVar(e,t)}toGLSL(e){let t=e;return t=t.replace(/([a-zA-Z0-9_]+)\^([a-zA-Z0-9_]+)/g,"pow($1, $2)"),t=t.replace(/\bln\(/g,"log("),t}toTeX(e){let t=window.nerdamer.convertToLaTeX(e);return this._cleanupTeX(t)}_cleanupTeX(e){return e=e.replace(/\^\{1\}/g,""),e}_basicTexConversion(e){let t=e;return t=t.replace(/([a-zA-Z0-9_]+)\^([0-9]+)/g,"$1^{$2}"),t=t.replace(/([a-zA-Z0-9_]+)\^([a-zA-Z]+)/g,"$1^{$2}"),t=t.replace(/sqrt\(([^)]+)\)/g,"\\sqrt{$1}"),t=t.replace(/([a-zA-Z0-9_]+)\/([a-zA-Z0-9_]+)/g,"\\frac{$1}{$2}"),t}clearCache(){this._ready&&window.nerdamer&&window.nerdamer.clear&&(l.verbose("Clearing Nerdamer cache (all definitions cleared)"),window.nerdamer.clear("all"))}toString(e){return typeof e=="string"?e:e&&typeof e.toString=="function"?e.toString():String(e)}getName(){return"Nerdamer"}getCapabilities(){return{differentiate:!0,integrate:!1,solve:!0,matrices:!0,simplify:!0,persistentNotebookContext:!1}}getVersion(){return window.nerdamer?"loaded":"not loaded"}};M();var et={NERDAMER:"nerdamer",MAXIMA:"maxima"};async function rn(n=et.NERDAMER){l.info(`Creating CAS engine: ${n}`);let e;switch(n){case et.NERDAMER:e=new Ut;break;case et.MAXIMA:throw new Error("Maxima engine not yet implemented (Phase 10-11)");default:throw new Error(`Unknown CAS engine type: ${n}`)}try{return await e.initialize(),l.info(`CAS engine ${n} initialized successfully`),e}catch(t){throw l.error(`Failed to initialize CAS engine ${n}:`,t.message),t}}function nn(){let n=localStorage.getItem("casEngine");return n&&Object.values(et).includes(n)?n:et.NERDAMER}M();var on="de-render-notebook";function uo(){return`cell-${Date.now()}-${Math.random().toString(36).substr(2,9)}`}var Ot=class{constructor(e){this.casEngine=e,this.cells=[],this.contextDirty=!1,l.info("Notebook initialized with CAS engine:",e.getName())}addCell(e,t=""){let i=uo(),r={id:i,type:e,input:t,output:null,tex:null,error:null};return this.cells.push(r),this.save(),l.info(`Added ${e} cell:`,i),i}updateCell(e,t){let i=this.cells.find(r=>r.id===e);if(!i){l.warn(`Cell not found: ${e}`);return}i.input=t,i.output=null,i.tex=null,i.error=null,this.save(),this.markContextDirty()}deleteCell(e){let t=this.cells.findIndex(i=>i.id===e);if(t===-1){l.warn(`Cell not found: ${e}`);return}this.cells.splice(t,1),this.save(),this.markContextDirty(),l.info(`Deleted cell: ${e}`)}moveCellUp(e){let t=this.cells.findIndex(i=>i.id===e);t<=0||([this.cells[t-1],this.cells[t]]=[this.cells[t],this.cells[t-1]],this.save(),this.markContextDirty())}moveCellDown(e){let t=this.cells.findIndex(i=>i.id===e);t===-1||t>=this.cells.length-1||([this.cells[t],this.cells[t+1]]=[this.cells[t+1],this.cells[t]],this.save(),this.markContextDirty())}async evaluateCell(e){let t=this.cells.find(i=>i.id===e);if(!t)return l.warn(`Cell not found: ${e}`),{result:"",tex:"",error:"Cell not found"};if(t.type!=="code")return{result:"",tex:"",error:"Not a code cell"};try{await this.evaluateUpTo(e),l.verbose(`Evaluating cell ${e}: "${t.input}"`);let i=this.evaluate(t.input);return t.output=i.result,t.tex=i.tex,t.error=i.error||null,this.save(),l.info(`Cell ${e} evaluated successfully:`,i.result),i}catch(i){return l.error(`Error evaluating cell ${e}:`,i.message),t.error=i.message,t.output=null,t.tex=null,this.save(),{result:"",tex:"",error:i.message}}}async evaluateUpTo(e){let t=this.cells.findIndex(i=>i.id===e);if(t!==-1)for(let i=0;i<t;i++){let r=this.cells[i];r.type==="code"&&!r.output&&!r.error&&await this.evaluateCell(r.id)}}async evaluateAll(){l.info("Evaluating all notebook cells");for(let e of this.cells)e.type==="code"&&await this.evaluateCell(e.id);l.info("All cells evaluated")}markContextDirty(){this.contextDirty=!0}ensureContext(){if(!this.contextDirty)return;l.verbose("Re-applying notebook context to CAS engine");let e=this.contextDirty;this.contextDirty=!1;for(let t of this.cells)if(t.type==="code"&&t.output&&!t.error)try{this.evaluate(t.input)}catch(i){l.warn(`Failed to re-apply cell ${t.id}:`,i.message)}l.verbose("Notebook context applied")}expandFunctions(e){this.ensureContext();try{let t=[];for(let o of this.cells)if(o.type==="code"&&o.output&&!o.error){let s=o.input.match(/^([a-zA-Z_][a-zA-Z0-9_]*)\s*\(([^)]*)\)\s*=\s*(.+)$/);if(s){let[,a]=s;t.push(a)}}let i=!1;for(let o of t)if(new RegExp(`\\b${o}\\s*\\(`).test(e)){i=!0;break}if(!i)return l.verbose(`No custom functions in "${e}", skipping expansion`),e;let r=this.casEngine.evaluate(e);if(r.result&&r.result!==e){let o=t.filter(s=>new RegExp(`\\b${s}\\s*\\(`).test(e));return o.length>0&&l.verbose(`Expanded with: ${o.join(", ")}`),l.verbose(`  "${e}" -> "${r.result}"`),r.result}return e}catch(t){return l.warn(`Failed to expand functions in "${e}":`,t.message),e}}differentiate(e,t){return this.ensureContext(),this.casEngine.differentiate(e,t)}solve(e,t){return this.ensureContext(),this.casEngine.solve(e,t)}invertMatrix(e){return this.ensureContext(),this.casEngine.invertMatrix(e)}evaluate(e){this.ensureContext(),l.verbose(`Notebook.evaluate() called with: "${e}"`);let t=e.match(/^([a-zA-Z_][a-zA-Z0-9_]*)\s*\(([^)]*)\)\s*=\s*(.+)$/);if(l.verbose("Function definition regex match:",t?"YES":"NO"),t){let[,i,r,o]=t,s=r.split(",").map(a=>a.trim()).filter(a=>a);if(l.verbose(`Parsed function: name="${i}", args=[${s.join(", ")}], body="${o}"`),this.casEngine.getName()==="Nerdamer"&&window.nerdamer&&window.nerdamer.setFunction)try{l.verbose(`Calling nerdamer.setFunction("${i}", [${s.join(", ")}], "${o}")`),window.nerdamer.setFunction(i,s,o),l.info(`Defined function: ${i}(${s.join(", ")}) = ${o}`);let a=`${i}(${s.join(", ")})`,c={result:`${a} = ${o}`,tex:`${a} = ${o}`};return l.verbose("Returning result:",c),c}catch(a){return l.error(`Error defining function: ${a.message}`),{result:"",tex:"",error:`Failed to define function ${i}: ${a.message}`}}else{l.warn("CAS engine does not support setFunction, function definition ignored");let a=`${i}(${s.join(", ")})`;return{result:`${a} = ${o} (not supported by engine)`,tex:`${a} = ${o}`,error:"Function definitions not supported by current CAS engine"}}}return l.verbose("Not a function definition, evaluating normally"),this.casEngine.evaluate(e)}parse(e){return this.ensureContext(),this.casEngine.parse(e)}save(){try{let e=this.toJSON();localStorage.setItem(on,JSON.stringify(e)),l.verbose("Notebook saved to localStorage")}catch(e){l.error("Failed to save notebook:",e.message)}}load(){try{let e=localStorage.getItem(on);if(!e){l.info("No saved notebook found");return}let t=JSON.parse(e);this.fromJSON(t),l.info(`Loaded notebook with ${this.cells.length} cells`),this.evaluateAll()}catch(e){l.error("Failed to load notebook:",e.message)}}toJSON(){return{version:1,cells:this.cells.map(e=>({id:e.id,type:e.type,input:e.input,output:e.output,tex:e.tex,error:e.error}))}}fromJSON(e){if(e.version!==1)throw new Error(`Unsupported notebook version: ${e.version}`);this.cells=e.cells||[],this.markContextDirty()}exportJSON(e="notebook.json"){let t=this.toJSON(),i=new Blob([JSON.stringify(t,null,2)],{type:"application/json"}),r=URL.createObjectURL(i),o=document.createElement("a");o.href=r,o.download=e,o.click(),URL.revokeObjectURL(r),l.info(`Exported notebook to ${e}`)}async importJSON(e){return new Promise((t,i)=>{let r=new FileReader;r.onload=async o=>{try{let s=JSON.parse(o.target.result);this.fromJSON(s),this.save(),await this.evaluateAll(),l.info("Imported notebook successfully"),t()}catch(s){l.error("Failed to import notebook:",s.message),i(s)}},r.onerror=()=>i(new Error("Failed to read file")),r.readAsText(e)})}clear(){this.cells=[],this.save(),this.markContextDirty(),l.info("Notebook cleared")}};ht();M();var Gt=class{constructor(e={}){if(this.config={generator:e.generator||null,onGenerate:e.onGenerate||null,context:e.context||{},mathPlaceholder:e.mathPlaceholder||(e.generator?e.generator.getMathPlaceholder():"Enter math expression..."),glslPlaceholder:e.glslPlaceholder||(e.generator?e.generator.getGLSLPlaceholder():"Generated GLSL code..."),initialMath:e.initialMath||"",initialGLSL:e.initialGLSL||"",readonly:e.readonly||!1},!this.config.generator&&!this.config.onGenerate)throw new Error("MathToGLSLControl requires either generator or onGenerate");this.element=null,this.mathInput=null,this.glslOutput=null,this.generateButton=null,this.errorDiv=null,this._createUI()}_createUI(){this.element=document.createElement("div"),this.element.className="math-to-glsl-control",this.element.style.cssText=`
            display: grid;
        ...
        `,r.appendChild(o),r.appendChild(this.glslOutput),this.errorDiv=document.createElement("div"),this.errorDiv.style.cssText=`
            ...
            color: #f44336;
            font-size: 11px;
        `,this.element.appendChild(e),this.element.appendChild(i),this.element.appendChild(r),this.element.appendChild(this.errorDiv)}generate(){let e=this.mathInput.value.trim();this.errorDiv.style.display="none";let t;if(this.config.generator?t=this.config.generator.validate(e,this.config.context):t=e?{valid:!0}:{valid:!1,error:"Expression is empty"},!t.valid){this._showError(t.error||"Invalid expression");return}try{let i;this.config.onGenerate?i=this.config.onGenerate(e,window.notebook,this.config.context):this.config.generator&&(i=this.config.generator.generate(e,window.notebook,this.config.context)),this.glslOutput.value=i,l.info("Generated GLSL from math expression")}catch(i){this._showError(i.message),l.error("Failed to generate GLSL:",i)}}_showError(e){this.errorDiv.textContent=`Error: ${e}`,this.errorDiv.style.display="block"}getElement(){return this.element}getMath(){return this.mathInput.value.trim()}getGLSL(){return this.glslOutput.value.trim()}setMath(e){this.mathInput.value=e}setGLSL(e){this.glslOutput.value=e}getValues(){return{math:this.getMath(),glsl:this.getGLSL()}}};M();var Ht=class{constructor(e){this.renderer=e,this.modal=null,this.controls=[],this.controlsContainer=null,this.dimensionSelector=null,this.generator=new Ee,this.workflow=new le(this.generator),this._createModal()}_createModal(){let e=document.createElement("div");e.className="field-equations-editor-content";let t=document.createElement("div");t.className="field-equations-dimension-row";let i=document.createElement("label");i.textContent="Number of Dimensions:",i.style.cssText="font-size: 14px; color: #4CAF50; font-weight: bold;",this.dimensionSelector=document.createElement("select"),this.dimensionSelector.className="field-equations-dimension-select";for(let r=2;r<=6;r++){let o=document.createElement("option");o.value=r,o.textContent=`${r}D`,this.dimensionSelector.appendChild(o)}this.dimensionSelector.addEventListener("change",()=>{this._updateControls(parseInt(this.dimensionSelector.value))}),t.appendChild(i),t.appendChild(this.dimensionSelector),this.controlsContainer=document.createElement("div"),this.controlsContainer.className="field-equations-controls",e.appendChild(t),e.appendChild(this.controlsContainer),this.modal=new Ie({title:"Field Equations Editor",customContent:e,applyButtonText:"Apply to Renderer",onApply:async(r,o)=>{let s=await this.workflow.executeInteractive(this.controls,r,this.renderer,this.modal);if(s.success){let a=document.getElementById("dimensions");a&&a.setValue&&a.setValue(this.controls.length);let c=window.manager?.get("dimension-inputs");if(c&&s.expressions&&c.setValue(s.expressions),window.equationOverlay&&window.equationOverlay.isVisible&&window.equationOverlay.isVisible()){let u=this.renderer.coordinateSystem.getVariableNames();window.equationOverlay.updateEquations(this.renderer.expressions,u)}}return s}}),this._updateControls(2)}_updateControls(e,t=null){this.controlsContainer.innerHTML="",this.controls=[];let i=t||this.renderer.expressions||[],r=["x","y","z","w","u","v"];for(let o=0;o<e;o++){let s=r[o]||`x${o}`,a=i[o]||"",c=new Gt({generator:this.generator,context:{dimension:o,totalDims:e},mathPlaceholder:`d${s}/dt = ...`,glslPlaceholder:"Generated GLSL...",initialMath:a,initialGLSL:""}),u=document.createElement("div");u.className="field-equation-label",u.textContent=`Dimension ${o}: d${s}/dt`,this.controlsContainer.appendChild(u),this.controlsContainer.appendChild(c.getElement()),this.controls.push(c)}l.info(`Created ${e} field equation controls`)}show(){let e=this.renderer.dimensions||2;this.dimensionSelector.value=e;let t=null;if(window.manager){let i=window.manager.get("dimension-inputs");i&&(t=i.getValue())}t||(t=this.renderer.expressions),this._updateControls(e,t),this.modal.show()}hide(){this.modal.hide()}isOpen(){return this.modal.isOpen()}destroy(){this.modal&&this.modal.destroy(),this.controls=[]}};window.MathParser={setCustomFunctions:zi,getCustomFunctions:Ui};window.showDocs=function(n,e){n.stopPropagation(),window.appModal?.show("docs",e)};window.showFieldEditor=function(n){n.stopPropagation(),window.getFieldEquationsEditor?.()?.show()};Qr();$(document).ready(async function(){window.logger=l,l.hookConsole(),l.info("Logger initialized");try{let a=nn();l.info(`Initializing CAS engine: ${a}`);let c=await rn(a),u=new Ot(c);u.load(),Yi(u),_r(u),window.notebook=u,window.casEngine=c,l.info(`Notebook initialized with ${c.getName()} engine`)}catch(a){l.error("Failed to initialize math system:",a),alert("Failed to initialize math engine: "+a.message);return}function n(a){let c=document.getElementById("canvas"),u=null;function d(){if(c.width=window.innerWidth,c.height=window.innerHeight,u){u.resize(c.width,c.height);let h=c.width/c.height,p=u.bbox,m=(p.min[0]+p.max[0])/2,f=(p.min[1]+p.max[1])/2,x=(p.max[1]-p.min[1])*h;p.min[0]=m-x/2,p.max[0]=m+x/2,u.updateConfig({bbox:p})}}d(),window.addEventListener("resize",d);try{let p=new URLSearchParams(window.location.search).get("storage")||"float";u=new bt(c,{storageStrategy:p}),a(u,c)}catch(h){l.error("Failed to initialize renderer",h),alert("Failed to initialize WebGL renderer: "+(h.message||h))}}function e(a,c){let u=Yr(a,function(d){let{manager:h,state:p,saveSettings:m}=d,f=h.getSettings();a.updateConfig(f),a.start(),o(a,h),window.renderer=a,window.loadPreset=At,window.logger=l,window.saveSettings=m,window.manager=h;let b=Zr(),x=new It(l);b.registerTab(x);let g=new Pt;b.registerTab(g);let y=new kt;b.registerTab(y),window.appModal=b,window.notebookEditor=en,window.fieldEquationsEditor=null,window.getFieldEquationsEditor=()=>(window.fieldEquationsEditor||(window.fieldEquationsEditor=new Ht(a)),window.fieldEquationsEditor),l.info("N-Dimensional Vector Field Renderer initialized!"),l.info("Available presets: "+Object.keys(window.presets).join(", ")),l.info('Try: loadPreset("3d_lorenz")')})}function t(a,c){let u=!1,d=0,h=0,p=null;function m(){p&&clearTimeout(p),p=setTimeout(()=>{window.saveSettings&&window.saveSettings()},500)}c.addEventListener("mousedown",g=>{g.target===c&&(u=!0,d=g.clientX,h=g.clientY)}),c.addEventListener("mousemove",g=>{if(u){let y=g.clientX-d,_=g.clientY-h,S=a.bbox,F=S.max[0]-S.min[0],E=S.max[1]-S.min[1],L=F/c.width;S.min[0]-=y*L,S.max[0]-=y*L,S.min[1]+=_*L,S.max[1]+=_*L,a.updateConfig({bbox:S}),d=g.clientX,h=g.clientY}}),c.addEventListener("mouseup",()=>{u=!1,m()}),c.addEventListener("mouseleave",()=>{u=!1,u||m()}),c.addEventListener("wheel",g=>{g.preventDefault();let y=g.deltaY>0?1.1:.9,_=c.getBoundingClientRect(),S=g.clientX-_.left,F=g.clientY-_.top,E=a.bbox,L=E.max[0]-E.min[0],D=E.max[1]-E.min[1],A=E.min[0]+L*(S/c.width),B=E.max[1]-D*(F/c.height),N=D*y,v=c.width/c.height,C=N*v,w=(A-E.min[0])/L,T=(E.max[1]-B)/D;E.min[0]=A-C*w,E.max[0]=A+C*(1-w),E.min[1]=B-N*(1-T),E.max[1]=B+N*T,a.updateConfig({bbox:E}),m()});let f=0;c.addEventListener("touchstart",g=>{if(g.touches.length===1)u=!0,d=g.touches[0].clientX,h=g.touches[0].clientY;else if(g.touches.length===2){u=!1;let y=g.touches[1].clientX-g.touches[0].clientX,_=g.touches[1].clientY-g.touches[0].clientY;f=Math.sqrt(y*y+_*_)}g.preventDefault()}),c.addEventListener("touchmove",g=>{if(g.touches.length===1&&u){let y=g.touches[0].clientX-d,_=g.touches[0].clientY-h,S=a.bbox,F=S.max[0]-S.min[0],E=S.max[1]-S.min[1],L=F/c.width;S.min[0]-=y*L,S.max[0]-=y*L,S.min[1]+=_*L,S.max[1]+=_*L,a.updateConfig({bbox:S}),d=g.touches[0].clientX,h=g.touches[0].clientY}else if(g.touches.length===2){let y=g.touches[1].clientX-g.touches[0].clientX,_=g.touches[1].clientY-g.touches[0].clientY,S=Math.sqrt(y*y+_*_);if(f>0){let F=f/S,E=a.bbox,L=E.max[1]-E.min[1],D=(E.min[0]+E.max[0])/2,A=(E.min[1]+E.max[1])/2,B=L*F,N=c.width/c.height,v=B*N;E.min[0]=D-v/2,E.max[0]=D+v/2,E.min[1]=A-B/2,E.max[1]=A+B/2,a.updateConfig({bbox:E})}f=S}g.preventDefault()}),c.addEventListener("touchend",()=>{u=!1,f=0,m()});function b(g){let y=a.bbox,_=y.max[0]-y.min[0],S=y.max[1]-y.min[1],F=(y.min[0]+y.max[0])/2,E=(y.min[1]+y.max[1])/2,L=S*g,D=c.width/c.height,A=L*D;y.min[0]=F-A/2,y.max[0]=F+A/2,y.min[1]=E-L/2,y.max[1]=E+L/2,a.updateConfig({bbox:y}),m()}function x(g,y){let _=a.bbox,S=_.max[0]-_.min[0],F=_.max[1]-_.min[1],E=S*g*.1,L=F*y*.1;_.min[0]+=E,_.max[0]+=E,_.min[1]+=L,_.max[1]+=L,a.updateConfig({bbox:_}),m()}$("#zoom-in").on("click",()=>b(.9)),$("#zoom-out").on("click",()=>b(1.11)),$("#pan-up").on("click",()=>x(0,1)),$("#pan-down").on("click",()=>x(0,-1)),$("#pan-left").on("click",()=>x(-1,0)),$("#pan-right").on("click",()=>x(1,0)),$("#pan-up-left").on("click",()=>x(-1,1)),$("#pan-up-right").on("click",()=>x(1,1)),$("#pan-down-left").on("click",()=>x(-1,-1)),$("#pan-down-right").on("click",()=>x(1,-1)),$("#pan-reset").on("click",function(){let g=a.bbox,y=g.max[0]-g.min[0],_=g.max[1]-g.min[1];a.updateConfig({bbox:{min:[-y/2,-_/2],max:[y/2,_/2]},reinitializeParticles:!1}),m()}),$("#mobile-zoom-in").on("click",()=>b(.9)),$("#mobile-zoom-out").on("click",()=>b(1.11)),$("#mobile-center").on("click",function(){let g=a.bbox,y=g.max[0]-g.min[0],_=g.max[1]-g.min[1];a.updateConfig({bbox:{min:[-y/2,-_/2],max:[y/2,_/2]},reinitializeParticles:!1}),m()})}function i(a,c){let u=document.getElementById("grid-canvas"),d=u.getContext("2d"),h=$("#cursor-position"),p=document.getElementById("show-grid"),m=p?.getValue?p.getValue():!0,f=0,b=0;function x(){u.width=c.width,u.height=c.height,S()}function g(w,T){let R=a.bbox,P=R.max[0]-R.min[0],V=R.max[1]-R.min[1],z=R.min[0]+w/c.width*P,I=R.max[1]-T/c.height*V;return{x:z,y:I}}function y(w,T){let R=a.bbox,P=R.max[0]-R.min[0],V=R.max[1]-R.min[1],z=(w-R.min[0])/P*c.width,I=(R.max[1]-T)/V*c.height;return{x:z,y:I}}function _(){let w=a.bbox,P=(w.max[0]-w.min[0])/10,V=Math.pow(10,Math.floor(Math.log10(P))),z=P/V,I;return z<1.5?I=1*V:z<3.5?I=2*V:z<7.5?I=5*V:I=10*V,I}function S(){if(!m){d.clearRect(0,0,u.width,u.height);return}d.clearRect(0,0,u.width,u.height);let w=a.bbox,T=_(),R=Math.floor(w.min[0]/T)*T,P=Math.ceil(w.max[0]/T)*T;d.strokeStyle="rgba(255, 255, 255, 0.1)",d.lineWidth=1,d.font="10px Courier New",d.fillStyle="rgba(255, 255, 255, 0.5)";for(let U=R;U<=P;U+=T){let q=y(U,0);if(d.beginPath(),d.moveTo(q.x,0),d.lineTo(q.x,u.height),d.stroke(),Math.abs(U)<1e-4)continue;let ke=U.toFixed(Math.max(0,-Math.floor(Math.log10(T))));d.fillText(ke,q.x+3,u.height-5)}let V=Math.floor(w.min[1]/T)*T,z=Math.ceil(w.max[1]/T)*T;for(let U=V;U<=z;U+=T){let q=y(0,U);if(d.beginPath(),d.moveTo(0,q.y),d.lineTo(u.width,q.y),d.stroke(),Math.abs(U)<1e-4)continue;let ke=U.toFixed(Math.max(0,-Math.floor(Math.log10(T))));d.fillText(ke,5,q.y-3)}let I=y(0,0);d.strokeStyle="rgba(255, 255, 255, 0.3)",d.lineWidth=2,I.y>=0&&I.y<=u.height&&(d.beginPath(),d.moveTo(0,I.y),d.lineTo(u.width,I.y),d.stroke(),d.fillText("0",I.x+3,I.y-3)),I.x>=0&&I.x<=u.width&&(d.beginPath(),d.moveTo(I.x,0),d.lineTo(I.x,u.height),d.stroke())}function F(){let w=g(f,b),T=c.getBoundingClientRect(),R=f-T.left,P=b-T.top,V=a.readPixelAt(R,P),z=`x: ${w.x.toFixed(3)}, y: ${w.y.toFixed(3)}`;if(V){if(V.hdr){let[I,U,q]=V.hdr;z+=`
HDR: (${I.toFixed(3)}, ${U.toFixed(3)}, ${q.toFixed(3)})`}if(V.ldr){let[I,U,q]=V.ldr;z+=`
LDR: (${I.toFixed(3)}, ${U.toFixed(3)}, ${q.toFixed(3)})`}}h.html(z.replace(/\n/g,"<br>"))}c.addEventListener("mousemove",w=>{f=w.clientX,b=w.clientY,F()}),p&&p.addEventListener("change",function(){m=p.getValue(),S()}),window.addEventListener("resize",x),setInterval(S,100);let E=document.querySelector("#fps-counter .fps-display"),L=document.getElementById("mobile-controls");function D(w){if(w>=1e3){let T=w/1e3;return T%1===0?`${Math.floor(T)}k`:`${T.toFixed(1)}k`}return w.toString()}function A(){if(a&&a.fps!==void 0){let w=a.totalFrames!==void 0?` | ${D(a.totalFrames)} frames`:"",T=`${a.fps}${w}`;E&&(E.textContent=`FPS: ${T}`),L&&L.setFPS&&L.setFPS(T)}}setInterval(A,500);let B=$("#histogram-panel"),N=document.getElementById("histogram-canvas"),v=N.getContext("2d");$("#histogram-header").on("click",function(){B.toggleClass("collapsed"),$("#histogram-expand").text(B.hasClass("collapsed")?"\u25B6":"\u25BC")});function C(){if(!a)return;let w=a.getBufferStats();if(!w||!w.histogram||w.histogram.length===0)return;let T=N,R=v,P=window.devicePixelRatio||1;T.width=T.clientWidth*P,T.height=T.clientHeight*P,R.scale(P,P);let V=T.clientWidth,z=T.clientHeight;R.fillStyle="rgba(0, 0, 0, 0.3)",R.fillRect(0,0,V,z);let I=Math.max(...w.histogram);if(I===0)return;let U=V/w.histogram.length;R.fillStyle="#4CAF50",w.histogram.forEach((q,ke)=>{let Pi=q/I*(z-4),sn=ke*U,an=z-Pi;R.fillRect(sn,an,U-1,Pi)}),$("#hist-avg").text(w.avgBrightness.toFixed(1)),$("#hist-max").text(w.maxBrightness.toFixed(0)),w.maxVelocity!==void 0&&w.maxVelocity!==null?$("#hist-vel").text(w.maxVelocity.toFixed(2)):$("#hist-vel").text("--")}setInterval(C,1e3),x(),F()}function r(a){document.addEventListener("keydown",c=>{if(c.key==="Escape"){if(window.mobilePanelManager&&window.mobilePanelManager.getCurrentPanel()){c.preventDefault(),window.mobilePanelManager.hideAllPanels(),l.verbose("Mobile panel closed via Escape key");return}let u=$(".floating-panel:visible");if(u.length>0){c.preventDefault(),u.hide(),l.verbose("Floating panel closed via Escape key");return}}if(c.key==="c"&&!c.ctrlKey&&!c.altKey&&!c.metaKey&&!c.shiftKey){if(c.target.tagName==="INPUT"||c.target.tagName==="TEXTAREA")return;c.preventDefault(),a.clearScreen(),l.info("Screen cleared via keyboard shortcut (C)")}})}function o(a,c){let u=null;$("#animation-script-input").on("change",function(d){let h=d.target.files[0];if(!h)return;let p=new FileReader;p.onload=function(m){try{let f=JSON.parse(m.target.result);u=new Lt(a,c);let b=u.loadScript(f);$("#anim-name").text(b.name);let x=b.timeline[b.timeline.length-1].time,g=Math.ceil(x*b.fps);$("#anim-param").text(`${b.timeline.length} keyframes`),$("#anim-range").text(`0s \u2192 ${x}s`),$("#anim-frames").text(g),$("#anim-fps").text(b.fps),$("#anim-duration").text(`${x.toFixed(1)}s`),$("#animation-info").show(),$("#animation-run-btn").prop("disabled",!1),$("#animation-download-btn").prop("disabled",!0),l.info(`Animation script loaded: ${b.name}`)}catch(f){alert("Failed to load animation script: "+f.message),l.error("Animation script load failed",f)}},p.readAsText(h)}),$("#animation-run-btn").on("click",async function(){if(u)try{$("#animation-run-btn").prop("disabled",!0),$("#animation-pause-btn").prop("disabled",!1),$("#animation-stop-btn").prop("disabled",!1),$("#animation-script-input").prop("disabled",!0),$("#animation-progress").show(),await u.run((d,h,p)=>{let m=d/h*100;$("#progress-bar").css("width",m+"%"),$("#progress-text").text(`Frame ${d} / ${h}`),$("#progress-param").text(`Time: ${p.toFixed(2)}s`)}),l.info("Animation complete!"),alert("Animation complete! "+u.frames.length+" frames captured."),$("#animation-download-btn").prop("disabled",!1)}catch(d){l.error("Animation failed",d),alert("Animation failed: "+d.message)}finally{$("#animation-run-btn").prop("disabled",!1),$("#animation-pause-btn").prop("disabled",!0),$("#animation-stop-btn").prop("disabled",!0),$("#animation-script-input").prop("disabled",!1)}}),$("#animation-pause-btn").on("click",function(){u&&(u.isPaused?(u.resume(),$(this).text("\u23F8 Pause"),l.info("Animation resumed")):(u.pause(),$(this).text("\u25B6 Resume"),l.info("Animation paused")))}),$("#animation-stop-btn").on("click",function(){u&&(u.stop(),l.info("Animation stopped"),$("#animation-run-btn").prop("disabled",!1),$("#animation-pause-btn").prop("disabled",!0),$("#animation-stop-btn").prop("disabled",!0),$("#animation-script-input").prop("disabled",!1))}),$("#animation-download-btn").on("click",async function(){if(!(!u||u.frames.length===0))try{$(this).prop("disabled",!0),$(this).text("\u{1F4BE} Downloading..."),await u.downloadFrames(),$(this).text("\u2713 Downloaded"),setTimeout(()=>{$(this).text("\u{1F4BE} Download Frames (ZIP)"),$(this).prop("disabled",!1)},2e3)}catch(d){l.error("Download failed",d),alert("Download failed: "+d.message),$(this).text("\u{1F4BE} Download Frames (ZIP)"),$(this).prop("disabled",!1)}}),l.info("Animation panel initialized")}function s(){let a="accordionState",c={};try{let u=localStorage.getItem(a);u&&(c=JSON.parse(u))}catch(u){console.warn("Failed to load accordion state",u)}$("#controls h2").each(function(u){let d=$(this),h=d.next(".accordion-section");if(h.length===0)return;let p=d.text().trim();(c.hasOwnProperty(p)?c[p]:u>0)?(d.addClass("collapsed"),h.addClass("collapsed")):h.css("max-height",h[0].scrollHeight+"px"),d.on("click",function(){let b=$(this),x=b.next(".accordion-section"),g=b.hasClass("collapsed");g?(b.removeClass("collapsed"),x.css("max-height",x[0].scrollHeight+"px"),x.removeClass("collapsed")):(x.css("max-height",x[0].scrollHeight+"px"),x[0].offsetHeight,x.css("max-height","0"),b.addClass("collapsed"),x.addClass("collapsed")),c[p]=!g;try{localStorage.setItem(a,JSON.stringify(c))}catch(y){console.warn("Failed to save accordion state",y)}});let f=()=>{h.hasClass("collapsed")||h.css("max-height",h[0].scrollHeight+"px")};typeof ResizeObserver<"u"&&new ResizeObserver(f).observe(h[0])}),l.info("Accordion initialized with "+$("#controls h2").length+" sections")}s(),n(function(a,c){t(a,c),i(a,c),r(a),e(a,c)})});})();
